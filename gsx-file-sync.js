...
class GSXFileSync {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (is401 && attempt < maxRetries) {
          console.log(
            `[GSX Sync] ${operationName} failed with auth error, attempting token refresh (attempt ${attempt + 1}/${maxRetries})...`
          );
          ...
          if (refreshResult.success) {
            ...
            throw new Error(
              `${operationName} failed: Token expired and refresh failed. Please update your token in Settings.`
            );
          }
        }
  ...
  get defaultSyncPaths() {
    if (!this._defaultSyncPaths) {
      this._defaultSyncPaths = [
        {
          local: path.join(app.getPath('documents'), 'OR-Spaces'),
          remote: 'OR-Spaces',
          description: 'Clipboard Spaces data',
        },
  ...
  async initialize() {
    try {
      ...
      console.log(
        '[GSX Sync] Raw token from settings:',
        typeof token,
        token ? `(length: ${token.length})` : 'NULL/UNDEFINED'
      );
      ...
      if (!token || token.trim() === '') {
        ...
        if (refreshUrl) {
          ...
          const errorMsg =
            'GSX not configured. Please add your Refresh URL in Settings (GSX File Sync Configuration section).';
          console.error('[GSX Sync] ERROR:', errorMsg);
          throw new Error(errorMsg);
        }
      }
      ...
      const _filesApiUrl = filesApiUrls[environment] || filesApiUrls.production;
      ...
      try {
        ...
        if (sdkError.message && sdkError.message.includes('401')) {
          ...
          helpfulMessage =
            'Token expired and refresh failed. Please update your token in Settings.\nYou may need to generate a new token from OneReach.';
  ...
  async syncDirectory(localPath, remotePath, options = {}) {
    ...
    try {
      ...
      const syncOptions = {
        ...
        onFileProgress: options.progressCallback
          ? (fileName, bytesTransferred, fileNumber) => {
              options.progressCallback({
                type: 'file',
  ...
  async syncORSpaces(options = {}) {
    ...
    try {
      await this.executeWithTokenRefresh(async () => {
        await this.client.pushLocalPathToFiles(indexPath, `${baseRemotePath}/index.json`, options);
    ...
    for (const space of spaces) {
      ...
      for (const item of spaceItems) {
        ...
        try {
          ...
          await this.executeWithTokenRefresh(async () => {
            await this.client.pushLocalPathToFiles(itemLocalDir, itemRemotePath, options);
      ...
      if (await this.pathExists(spaceNotebookPath)) {
        try {
          await this.executeWithTokenRefresh(async () => {
            await this.client.pushLocalPathToFiles(spaceNotebookPath, `${spaceRemotePath}/README.ipynb`, options);
  ...
  async syncSingleItem(itemId, spaceId, spaceName) {
    try {
      ...
      await this.executeWithTokenRefresh(async () => {
        await this.client.pushLocalPathToFiles(itemLocalDir, itemRemotePath, { isPublic: false, ttl: null });
  ...
  async syncIndex() {
    try {
      ...
      await this.executeWithTokenRefresh(async () => {
        await this.client.pushLocalPathToFiles(indexPath, `${baseRemotePath}/index.json`, {
          isPublic: false,
          ttl: null,
        });
      }, 'syncIndex');
  ...
  async syncMultiple(syncPaths = this.defaultSyncPaths, options = {}) {
    ...
    for (const syncPath of syncPaths) {
      try {
        const result = await this.syncDirectory(syncPath.local, syncPath.remote, options);
        results.push(result);
      } catch (error) {
  ...
  showSyncErrorAlert(errorMessage, remotePath) {
    ...
    if (errorMessage.includes('401') || errorMessage.includes('Unauthorized') || errorMessage.includes('expired')) {
      ...
      errorMessage.includes('discovery') ||

...
- [x] **Upfront classify migrated** (commit `19d11f9`) — `normalizeIntent` runs its LLM step via `runMetaTask(CLASSIFY_INTENT)`, fast-skip stays inline, fails open to the raw transcript on any non-settle (identical to its long-standing contract → bulletproof on the every-request path). Taxonomy fixed: CLASSIFY_INTENT = NormalizeIntent, DISAMBIGUATE = the halt rephrase-vs-gap triage.
- [ ] The rephrase-*question* builder (`_buildRephraseResponse`) stays inline (secondary halt-path op) and `evaluate-response` stays inline on the per-response path by design (no ledger entry per result). Migrate only if a concrete need arises. *Core meta-work (classify / disambiguate / evaluate-buildability) is now task-based; these two are the deliberate remainder.*

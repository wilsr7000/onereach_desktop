...
async function testOneReach() {
    ...
    try {
        ...
        Object.entries(styleResult.data.cssVariables || {}).slice(0, 5).forEach(([k, v]) => {
            console.log('     ', k + ':', v);
        });
        ...
        const checks = [
            ...
            ['Headlines', (copyResult.data.headlines?.primary?.length || 0) + (copyResult.data.headlines?.secondary?.length || 0) > 0],

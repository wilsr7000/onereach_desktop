...
contextBridge.exposeInMainWorld('videoEditor', {
  ...
  // Thumbnails
  generateThumbnails: (videoPath, options) =>
    ipcRenderer.invoke('video-editor:generate-thumbnails', videoPath, options),
  generateThumbnail: (videoPath, timestamp) =>
    ipcRenderer.invoke('video-editor:generate-thumbnail', videoPath, timestamp),
  getTimelineThumbnails: (videoPath, options) =>
    ipcRenderer.invoke('video-editor:timeline-thumbnails', videoPath, options),
  ...
  saveWaveformCache: (videoPath, cacheData) =>
    ipcRenderer.invoke('video-editor:save-waveform-cache', videoPath, cacheData),
  loadWaveformCache: (videoPath) => ipcRenderer.invoke('video-editor:load-waveform-cache', videoPath),
  saveWaveformImage: (videoPath, imageKey, dataUrl) =>
    ipcRenderer.invoke('video-editor:save-waveform-image', videoPath, imageKey, dataUrl),
  loadWaveformImage: (videoPath, imageKey) =>
    ipcRenderer.invoke('video-editor:load-waveform-image', videoPath, imageKey),
  ...
  saveThumbnailCache: (videoPath, cacheData) =>
    ipcRenderer.invoke('video-editor:save-thumbnail-cache', videoPath, cacheData),
  ...
  saveThumbnailStrip: (videoPath, tierName, dataUrl) =>
    ipcRenderer.invoke('video-editor:save-thumbnail-strip', videoPath, tierName, dataUrl),
  loadThumbnailStrip: (videoPath, tierName) =>
    ipcRenderer.invoke('video-editor:load-thumbnail-strip', videoPath, tierName),
  ...
  extractSpeakerAudio: (videoPath, segments, options) =>
    ipcRenderer.invoke('video-editor:extract-speaker-audio', videoPath, segments, options),
  ...
  replaceAudioWithElevenLabs: (videoPath, options) =>
    ipcRenderer.invoke('video-editor:replace-audio-elevenlabs', videoPath, options),
  ...
  exportWithADRTracks: (videoPath, exportData) =>
    ipcRenderer.invoke('video-editor:export-adr-tracks', videoPath, exportData),
  exportWithAudioReplacements: (videoPath, replacements) =>
    ipcRenderer.invoke('video-editor:export-with-audio-replacements', videoPath, replacements),
  ...
  concatenateClips: (clips, sources, options) =>
    ipcRenderer.invoke('video-editor:concatenate-clips', clips, sources, options),
  ...
  downloadDubbedAudio: (dubbingId, languageCode) =>
    ipcRenderer.invoke('video-editor:download-dubbed-audio', dubbingId, languageCode),
  ...
  extractAudioSegment: (videoPath, startTime, endTime) =>
    ipcRenderer.invoke('video-editor:extract-audio-segment', videoPath, startTime, endTime),
  ...
  identifySpeakers: (transcriptionResult, options) =>
    ipcRenderer.invoke('video-editor:identify-speakers', transcriptionResult, options),

  // ==================== ELEVENLABS STUDIO PROJECTS ====================

  // Create a Studio project
  createStudioProject: (name, options) =>
    ipcRenderer.invoke('video-editor:elevenlabs-create-studio-project', name, options),
  ...
  cloneVoice: (name, audioFilePaths, options) =>
    ipcRenderer.invoke('video-editor:elevenlabs-clone-voice', name, audioFilePaths, options),
  ...
  generateAudioStream: (text, voice, options) =>
    ipcRenderer.invoke('video-editor:elevenlabs-generate-stream', text, voice, options),
  ...
  getHistoryItemAudio: (historyItemId) =>
    ipcRenderer.invoke('video-editor:elevenlabs-get-history-audio', historyItemId),

  // Delete a history item
  deleteHistoryItem: (historyItemId) =>
    ipcRenderer.invoke('video-editor:elevenlabs-delete-history-item', historyItemId),

  // Delete multiple history items
  deleteHistoryItems: (historyItemIds) =>
    ipcRenderer.invoke('video-editor:elevenlabs-delete-history-items', historyItemIds),
  ...
  generateScreengrabs: (videoPath, options) =>
    ipcRenderer.invoke('video-editor:generate-screengrabs', videoPath, options),
  openExportFolder: (folderPath) => ipcRenderer.invoke('video-editor:open-folder', folderPath),
  addWatermark: (videoPath, watermarkPath, options) =>
    ipcRenderer.invoke('video-editor:watermark', videoPath, watermarkPath, options),
  ...
  createVideoFromAudio: (audioPath, options) =>
    ipcRenderer.invoke('video-editor:create-video-from-audio', audioPath, options),
  ...
  detectScenes: (videoPath, options) => ipcRenderer.invoke('video-editor:detect-scenes', videoPath, options),
  ...
  translateText: (sourceText, options) => ipcRenderer.invoke('video-editor:translate-text', sourceText, options),
  ...
  getSpaceVideo: (itemId) => ipcRenderer.invoke('video-editor:get-space-video', itemId),

  // Save scenes only (without re-encoding video)
  saveScenesOnly: (itemId, scenes) => ipcRenderer.invoke('video-editor:save-scenes-only', itemId, scenes),
  ...
  getReleaseOptions: () => ipcRenderer.invoke('video-editor:get-release-options'),
  ...
  getUploadServiceStatus: (service) => ipcRenderer.invoke('video-editor:get-upload-service-status', service),

  // Authenticate upload service (YouTube/Vimeo OAuth)
  authenticateUploadService: (service) => ipcRenderer.invoke('video-editor:authenticate-upload-service', service),
  ...
  // Get all projects
  getVideoProjects: () => ipcRenderer.invoke('video-editor:get-projects'),
  ...
  getBranches: () => ipcRenderer.invoke('video-editor:get-branches'),
  ...
  loadEDL: (branchId, version) => ipcRenderer.invoke('video-editor:load-edl', branchId, version),
  ...
  openImage: () =>
    ipcRenderer.invoke('dialog:open-file', {
      filters: [{ name: 'Image Files', extensions: ['png', 'jpg', 'jpeg', 'gif', 'webp', 'svg'] }],
    }),
  ...
  attachVideoPlayer: () => ipcRenderer.invoke('video-editor:attach-player'),

  // Sync playback state to detached window
  syncPlayback: (state) => ipcRenderer.invoke('video-editor:sync-playback', state),

  // Update video source in detached window
  updateDetachedSource: (videoPath) => ipcRenderer.invoke('video-editor:update-detached-source', videoPath),

  // Check if video is currently detached
  isDetached: () => ipcRenderer.invoke('video-editor:is-detached'),
  ...
  saveProject: (videoPath, projectData) => ipcRenderer.invoke('save-video-project', { videoPath, projectData }),

  // Load project state from file
  loadProject: (videoPath) => ipcRenderer.invoke('load-video-project', { videoPath }),

  // Delete project file
  deleteProject: (videoPath) => ipcRenderer.invoke('delete-video-project', { videoPath }),
  ...
  api: {
    ...
    items: {
      ...
      move: (itemId, fromSpaceId, toSpaceId) =>
        ipcRenderer.invoke('spaces-api:items:move', itemId, fromSpaceId, toSpaceId),
    },
...
contextBridge.exposeInMainWorld('stateManager', {
  ...
  saveSnapshot: (editorId, snapshot) => ipcRenderer.invoke('stateManager:saveSnapshot', editorId, snapshot),

  // List all snapshots for an editor (metadata only)
  listSnapshots: (editorId) => ipcRenderer.invoke('stateManager:listSnapshots', editorId),

  // Get a specific snapshot with full state
  getSnapshot: (editorId, snapshotId) => ipcRenderer.invoke('stateManager:getSnapshot', editorId, snapshotId),

  // Delete a snapshot
  deleteSnapshot: (editorId, snapshotId) => ipcRenderer.invoke('stateManager:deleteSnapshot', editorId, snapshotId),
  ...
  getStats: (editorId) => ipcRenderer.invoke('stateManager:getStats', editorId),

  // Clear all snapshots for an editor
  clearSnapshots: (editorId) => ipcRenderer.invoke('stateManager:clearSnapshots', editorId),
});
...
contextBridge.exposeInMainWorld('projectAPI', {
  ...
  analyzeSceneWithVision: (options) => ipcRenderer.invoke('video-editor:analyze-scene-with-vision', options),

  // Find quotes in transcript (AI-powered)
  findQuotes: (options) => ipcRenderer.invoke('video-editor:find-quotes', options),

  // Detect topics in transcript (AI-powered)
  detectTopics: (options) => ipcRenderer.invoke('video-editor:detect-topics', options),

  // Analyze hooks throughout video (AI-powered)
  analyzeHooks: (options) => ipcRenderer.invoke('video-editor:analyze-hooks', options),

  // Rate project with AI against criteria
  rateProject: (options) => ipcRenderer.invoke('video-editor:rate-project', options),
});

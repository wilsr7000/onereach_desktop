...
const { getContentIngestionService, retryOperation } = require('./content-ingestion');
...
let BrowserWindow, ipcMain, globalShortcut, app, clipboard, nativeImage;
try {
...
class ClipboardManagerV2 {
  constructor() {
    ...
    try {
      ...
      log.info('clipboard', 'Preferences loaded', {
    ...
    try {
      ...
      log.error('clipboard', 'setupIPC FAILED - viewer will not work', {
        error: ipcErr.message,
        stack: ipcErr.stack?.substring(0, 300),
      });
    }
  ...
  _subscribeToSpacesAPIEvents() {
    try {
      ...
      spacesAPI.on('item:deleted', ({ _spaceId, itemId }) => {
      ...
      spacesAPI.on('item:added', ({ _spaceId, item }) => {
      ...
      spacesAPI.on('item:updated', ({ _spaceId, itemId, data }) => {
    ...
    try {
      ...
      if (data.colors && data.typography) {
        const hasColorTokens =
          ...
          data.colors.neutral ||
          data.colors.text ||
          data.colors.backgrounds;
        const hasTypographyTokens =
          data.typography.fontFamilies ||
          data.typography.scale ||
          data.typography.fonts ||
          data.typography.headings ||
          data.typography.body;
        const _hasDesignTokens = data.spacing || data.borderRadius || data.shadows || data.borders || data.animations;
      ...
      if (data.messages && Array.isArray(data.messages)) {
        ...
        const hasMessageStructure = data.messages.some(
  ...
  loadFromStorage() {
    ...
    // Track pinned items
    this.pinnedItems = new Set(items.filter((item) => item.pinned).map((item) => item.id));
  }

  loadPreferences() {
    const prefs = this.storage.getPreferences();
    this.spacesEnabled = prefs.spacesEnabled !== undefined ? prefs.spacesEnabled : true;
    this.screenshotCaptureEnabled =
      prefs.screenshotCaptureEnabled !== undefined ? prefs.screenshotCaptureEnabled : false;
  ...
  async addToHistory(item) {
    ...
    if (requestedSpaceId && requestedSpaceId !== 'unclassified') {
      ...
      if (!spaceExists) {
        log.error('clipboard', 'CRITICAL: Space "..." does not exist! Item will be saved to "unclassified" instead.', {
          requestedSpaceId,
        });
    ...
    if (context && !item.source) {
      item.source = this.contextCapture.enhanceSourceDetection(item.content || item.preview || '', context);
    } else if (!item.source) {
  ...
  async maybeAutoGenerateMetadata(itemId, itemType, isScreenshot, fileType = null) {
    try {
      ...
      if (!shouldGenerate) {
        log.info('clipboard', 'Skipping metadata generation for ... (not in enabled types: ...)', {
          itemType,
          detail: autoAIMetadataTypes.join(', '),
        });
        return;
      }
        ...
        if (BrowserWindow) {
          BrowserWindow.getAllWindows().forEach((window) => {
            window.webContents.send('show-notification', {
              ...
              body: result.metadata.description
                ? result.metadata.description.substring(0, 50) + '...'
                : 'Metadata generated',
            });
          });
        }
        ...
        try {
          ...
          if (detectedSubtype) {
            ...
            if (indexEntry && !indexEntry.jsonSubtype) {
              ...
              log.info('clipboard', 'Detected and saved jsonSubtype: ... for item ...', {
                detectedSubtype,
                itemId: item.id,
              });
            }
  ...
  async deleteItem(id) {
    ...
    if (this.manager && this.manager.pendingOperations) {
      ...
      if (pendingOps && pendingOps.size > 0) {
        log.info('clipboard', 'Waiting for ... pending operations to complete before deleting item ...', {
          size: pendingOps.size,
          id,
        });
        try {
          await Promise.race([
  ...
  getSpaceItems(spaceId) {
    ...
    const _uniqueSpaceIds = [...new Set(indexItems.map((item) => item.spaceId))];
    log.info('clipboard', `[getSpaceItems] Unique spaceIds in index:`);

    // Filter items by space from the storage index
    const filteredItems = indexItems.filter((item) => (spaceId === null ? true : item.spaceId === spaceId));
  ...
  async generateVideoMetadata({ transcript, originalTitle, uploader, _description, duration }) {
...
Respond ONLY with valid JSON, no other text.`;
  ...
  createClipboardWindow() {
    log.info('clipboard', 'createClipboardWindow called', {
      hasExistingWindow: !!(this.clipboardWindow && !this.clipboardWindow.isDestroyed()),
    });
    ...
    this.clipboardWindow.webContents.on(
      'did-fail-load',
  ...
  createBlackHoleWindow(position, startExpanded = false, clipboardData = null) {
    ...
    this.blackHoleWindow.webContents.on('did-finish-load', () => {
      ...
      log.info('clipboard', 'Window loaded, startExpanded', {
        startExpanded,
        arg2: 'hasClipboardData:',
        hasClipboardData,
      });
  ...
  setupScreenshotWatcher() {
    ...
    if (fs.existsSync(desktopPath)) {
      info: log.info('clipboard', 'Setting up screenshot watcher for', { desktopPath });
  ...
  async handleScreenshot(screenshotPath) {
    ...
    try {
      ...
      const existingScreenshot = this.history.find(
        (item) =>
          item.type === 'file' &&
      ...
      if (!this.currentSpace || this.currentSpace === null) {
        send: log.info('clipboard', 'No active space, prompting user to select space for screenshot');
      ...
      if (!isVideo && stats.size < 50 * 1024 * 1024) {
        try {
          ...
          log.info('clipboard', 'Screenshot thumbnail size: ... bytes (original: ... bytes)', {
      ...
      if (settings?.autoGenerateScreenshotMetadata && settings?.llmApiKey) {
        ...
        setTimeout(async () => {
          try {
            ...
            log.error('clipboard', 'Error auto-generating AI metadata for screenshot', {
  ...
  setupIPC() {
    ...
    const safeHandle = (channel, handler) => {
      const wrappedHandler = async (...args) => {
        try {
          ...
          log.error('ipc', `IPC handler threw: ${channel}`, {
            error: err.message,
    ...
    ipcMain.on('black-hole:inactive', () => {
      ...
      detail: log.info('clipboard', 'Black hole inactive');
    });
    ...
    safeHandle('clipboard:paste-item', (event, id) => {
      ...
      log.info('clipboard', '📋 Pasting item', {
        detail: {
          id: item.id,
          type: item.type,
          fileName: item.fileName,
      ...
      if (item.type === 'text') {
        ...
        const isTextFile =
          item.fileName &&
          /\.(md|txt|json|js|ts|tsx|jsx|html|css|py|java|cpp|c|h|cs|php|rb|go|rs|swift|kt|xml|yaml|yml|csv|log|sh|bash)$/i.test(
            item.fileName
          );
      ...
      try {
        ...
        if (!this._floatCardCloseHandler) {
          ...
          ipcMain.on('float-card:ready', () => {
            info: log.info('clipboard', 'Window ready');
          });
    ...
    safeHandle('clipboard:add-video-scene', (event, itemId, scene) => {
      try {
        ...
        if (scene.id === undefined) {
          scene.id = metadata.scenes.length > 0 ? Math.max(...metadata.scenes.map((s) => s.id)) + 1 : 1;
        }
    ...
    safeHandle('clipboard:get-videos-with-scenes', (event, spaceId = null) => {
      try {
        ...
        let items = this.storage.index.items.filter(
          (item) => item.type === 'file' && item.fileType?.startsWith('video/')
        );
      ...
      try {
        ...
        if (item.type === 'file' && item.content) {
          ...
          if (item.fileExt && textExtensions.includes(item.fileExt.toLowerCase())) {
            ...
            (item.fileExt && imageExtensions.includes(item.fileExt.toLowerCase()))
          ) {
            ...
            try {
              ...
              if (fs.existsSync(filePath)) {
                ...
                log.info('clipboard', 'Loaded full-resolution image', {
                  fileName: item.fileName,
                  arg2: 'size:',
    ...
    safeHandle('clipboard:get-pdf-data', (event, itemId) => {
      try {
        ...
        log.info('clipboard', 'Loaded PDF', {
          fileName: item.fileName,
          arg2: 'size:',
          pdfBufferCount: pdfBuffer.length,
        });
    ...
    safeHandle('clipboard:edit-image-ai', async (event, options) => {
      try {
        const { imageData, prompt } = options;
...
Respond in a helpful, conversational way.`;
    ...
    safeHandle('clipboard:save-image-as-new', async (event, imageData, options = {}) => {
      try {
        // Extract base64 data
        let base64Data = imageData;
        let _mediaType = 'image/png';

        if (imageData.startsWith('data:')) {
          const matches = imageData.match(/^data:([^;]+);base64,(.+)$/);
          if (matches) {
            _mediaType = matches[1];
            base64Data = matches[2];
          }
        }
    ...
    safeHandle('clipboard:generate-speech', async (event, options) => {
      try {
        ...
        log.info('clipboard', 'Text length: ... chars, estimated ... chunks, ~... min audio', {
          textLength,
          estimatedChunks,
          estimatedMinutes,
        });
        ...
        for (let i = 0; i < textChunks.length; i++) {
          ...
          log.info('clipboard', 'Generating chunk .../..., length: ...', {
            detail: i + 1,
            totalChunks,
        ...
        log.info('clipboard', 'Successfully generated audio: ... chunks, ... KB (... MB)', {
          totalChunks,
          totalSizeKB,
          totalSizeMB,
        });
    ...
    safeHandle('clipboard:create-audio-script', async (event, options) => {
      try {
        ...
        log.info('clipboard', 'Creating audio script for: ..., content length: ...', {
          title,
...
${content}`,
        ...
        await new Promise((resolve, reject) => {
          ffmpeg(videoPath)
            ...
            .on('end', () => {
              ...
              if (event.sender && !event.sender.isDestroyed()) {
                send: event.sender.send('audio-extract-progress', { itemId, percent: 100, status: 'Complete!' });
              }
              resolve();
            })
            .on('error', (err) => {
    ...
    safeHandle('clipboard:transcribe-audio', async (event, options) => {
      try {
        ...
        log.info('clipboard', 'Successfully transcribed: ... words, ... speakers', {
        ...
        if (typeof tempAudioPath !== 'undefined' && tempAudioPath && fs.existsSync(tempAudioPath)) {
          try {
            ...
            /* cleanup temp on error, already closed ok */
          }
        }
        return { success: false, error: error.message };
      }
      ...
      try {
        ...
        if (fs.existsSync(metadataPath)) {
          ...
          if (
            ...
            log.info('clipboard', 'Using Whisper transcription from metadata', {
              arg1: metadata.transcriptSegments.length,
              arg2: 'segments',
            });
            return {
              success: true,
        ...
        if (fs.existsSync(metadataPath)) {
          ...
          if (metadata.transcriptSegments && metadata.transcriptSegments.length > 0) {
            ...
            log.info('clipboard', 'Found', {
              metadataCount: metadata.transcriptSegments.length,
              arg2: 'segments in metadata (source:',
              arg3: source + ')',
            });
            return {
              success: true,
    ...
    safeHandle('clipboard:generate-summary', async (event, options) => {
      ...
      try {
        ...
        const truncatedTranscript =
          transcript.length > 30000 ? transcript.substring(0, 30000) + '\n\n[Transcript truncated...]' : transcript;
...
Write the structured summary now:`;
    ...
    safeHandle('clipboard:identify-speakers', async (event, options) => {
      ...
      try {
        ...
        log.info('clipboard', 'Raw keys - anthropicApiKey', {
          arg1: !!anthropicApiKey,
          arg2: 'openaiApiKey:',
          arg3: !!openaiApiKey,
        });
        ...
        if (!claudeKey && !openaiKey) {
          return {
            success: false,
            error: 'No AI API key configured. Please add a Claude or OpenAI API key in Settings.',
          };
        }
          ...
          try {
            ...
            if (fs.existsSync(metadataPath)) {
              ...
              if (metadata.transcript?.segments?.length > 0) {
                ...
                formattedTranscript = metadata.transcript.segments
                  .map((seg) => `[${seg.startFormatted || formatTime(seg.start)}] ${seg.text}`)
                  .join('\n');
                log.info('clipboard', 'Using timecoded segments', {
                  arg1: metadata.transcript.segments.length,
                  arg2: 'segments',
                });
              }
            }
        ...
        const truncatedTranscript =
          formattedTranscript.length > maxChars
...
${truncatedTranscript}`;
        ...
        log.info('clipboard', 'Transcript length', {
          truncatedTranscriptCount: truncatedTranscript.length,
          arg2: 'needsChunking:',
          needsChunking,
        });
        ...
        async function callClaude(prompt, systemMsg) {
          ...
          log.info('clipboard', '✅ Successfully received response, text length', {
...
${chunks[i]}`;
    ...
    safeHandle('clipboard:check-monitor-now', async (event, itemId) => {
      ...
      try {
        // Find the monitor item - check both history and storage index
        let item = this.history.find((h) => h.id === itemId);
        if (!item) {
          // Try from storage index (has web-monitor fields)
          const storageItem = this.storage.index.items.find((i) => i.id === itemId);
          if (storageItem) {
            item = { ...storageItem };
            // Also try to load full content for URL recovery
            try {
              const fullItem = this.storage.loadItem(itemId);
              if (fullItem && fullItem.content && !item.url) {
                item.url = this._extractURLFromContent(fullItem.content) || '';
                if (item.url) {
                  try {
                    item.name = item.name || new URL(item.url).hostname;
                  } catch (_e) {
                    /* ignore */
                  }
                }
              ...
              log.error('clipboard', 'Failed to load item from storage', { error: _e.message || _e });
            }
          }
        }
        ...
        // Recover URL from content if still missing
        if (!item.url) {
          try {
            const fullItem = this.storage.loadItem(itemId);
        ...
        if (!item.url) {
          return { success: false, error: 'Monitor has no URL configured. Please remove and re-add it.' };
        }
        ...
        if (!monitorId) monitorId = item.id;
        log.info('clipboard', 'Using monitorId', { monitorId, url: item.url });
    ...
    safeHandle('clipboard:get-video-path', async (event, itemId) => {
      try {
        ...
        const indexEntry = this.storage.index.items.find((i) => i.id === itemId);
        log.info('clipboard', 'Index entry', {
          detail: indexEntry ? { contentPath: indexEntry.contentPath, fileName: indexEntry.fileName } : 'null',
        });
        ...
        if (item?.filePath && fs.existsSync(item.filePath)) {
          filePath: log.info('clipboard', 'Found filePath on item', { filePath: item.filePath });
          const scenes = loadScenes(itemId);
          return {
            success: true,
            filePath: item.filePath,
    ...
    safeHandle('clipboard:generate-metadata-ai', async (event, { itemId, apiKey, customPrompt }) => {
      log.info('clipboard', 'Generate metadata request received', {
        detail: { itemId, hasApiKey: !!apiKey, hasCustomPrompt: !!customPrompt },
      });
    ...
    safeHandle('clipboard:get-audio-data', (event, itemId) => {
      ...
      try {
        ...
        if (filePath && fs.existsSync(filePath)) {
          const isVideo = item.fileCategory === 'video' || (item.fileType && item.fileType.startsWith('video/'));
    ...
    safeHandle('clipboard:open-style-guide-editor', async (event, itemId) => {
      try {
        ...
        const _pathResult = await ipcMain.emit('clipboard:get-json-asset-path', event, itemId);
        const indexEntry = this.storage.index.items.find((i) => i.id === itemId);
        ...
        styleGuideWindow.on('closed', () => {
          /* styleGuideWindow cleaned up */
        });
    ...
    safeHandle('clipboard:open-journey-map-editor', async (event, itemId) => {
      try {
        ...
        journeyWindow.on('closed', () => {
          /* journeyWindow cleaned up */
        });
    ...
    ipcMain.handle('black-hole:add-image', async (event, data) => {
      ...
      try {
        ...
        await retryOperation(() => this.addToHistory(item), { maxRetries: 3, baseDelay: 200 });
    ...
    ipcMain.handle('black-hole:add-file', async (event, data) => {
      ...
      log.info('clipboard', 'ADD-FILE HANDLER - spaceId: ..., fileName: ...', {
        opId,
        detail: data?.spaceId,
      ...
      try {
        ...
        if (['.mp4', '.avi', '.mov', '.wmv', '.flv', '.mkv', '.webm', '.m4v', '.mpg', '.mpeg'].includes(ext)) {
          ...
          ['.mp3', '.wav', '.flac', '.aac', '.ogg', '.wma', '.m4a', '.opus', '.aiff', '.ape', '.amr', '.au'].includes(
            ext
          )
        ) {
          fileType = 'audio';
        ...
        if (fileType === 'pdf') {
          ...
          log.info('clipboard', 'Generated image thumbnail, mimeType', {
            mimeType,
            arg2: 'dataLength:',
            fileDataCount: data.fileData.length,
          });
        }
        ...
        if (data.fileData) {
          log.info('clipboard', 'Preparing file data for storage', {
            fileName: data.fileName,
            arg2: 'size:',
            fileDataCount: data.fileData.length,
            arg4: 'chars',
          ...
          try {
            ...
            log.info('clipboard', 'Temp file created', {
              tempFilePath,
              arg2: 'size:',
              size: tempStats.size,
              arg4: 'bytes',
        ...
        if (!fs.existsSync(storedFilePath)) {
          ...
          if (fs.existsSync(itemDir)) {
            const _files = fs.readdirSync(itemDir);
          ...
          log.info('clipboard', 'File verified at: ..., size: ... bytes', {
            opId,
            storedFilePath,
    ...
    safeHandle('clipboard:complete-screenshot', (event, { screenshotPath, spaceId, _stats, _ext }) => {
      if (fs.existsSync(screenshotPath)) {
    ...
    safeHandle('clipboard:smart-export-space', async (event, spaceId) => {
      ...
      try {
        ...
        smartExportWindow.on('closed', () => {
          /* smartExportWindow cleaned up */
        });
    ...
    safeHandle('clipboard:check-website', async (event, monitorId) => {
      try {
        ...
        // If changed, update the existing monitor item's timeline (don't create new items)
  ...
  setupYouTubeHandlers(safeHandle) {
    ...
    safeHandle('clipboard:verify-item', async (event, itemId) => {
      try {
        ...
        const files = fs
          .readdirSync(itemDir)
          .filter((f) => !f.endsWith('.json') && !f.endsWith('.txt') && f !== '.DS_Store');
    ...
    ipcMain.handle('youtube:download-to-space', async (event, url, spaceId) => {
      ...
      try {
        ...
        const result = await dl.downloadToSpace(url, this, spaceId || this.currentSpace, (percent, status) => {
          info: log.info('clipboard', 'Progress', { percent, status });
    ...
    ipcMain.handle('youtube:get-active-downloads', async (_event) => {
      try {
    ...
    ipcMain.handle('youtube:fetch-transcript-for-item', async (event, itemId, lang = 'en') => {
      ...
      try {
        ...
        if (result.success && result.transcript) {
          ...
          log.info('clipboard', 'Transcript saved for item', {
            itemId,
            arg2: 'length:',
    ...
    ipcMain.handle('youtube:get-transcript-whisper', async (event, url, lang = 'en') => {
      ...
      try {
        ...
        log.info('clipboard', 'Transcription result', {
          detail: result.success ? `${result.wordCount} words, ${result.speakerCount} speakers` : result.error,
        });
        return result;
      } catch (error) {
    ...
    ipcMain.handle('youtube:process-speaker-recognition', async (event, url) => {
      ...
      try {
        ...
        // Download audio first
        if (event.sender && !event.sender.isDestroyed()) {
          event.sender.send('youtube:speaker-recognition-progress', {
            percent: 10,
        ...
        log.info('clipboard', 'Speaker recognition result', {
          detail: result.success ? `${result.speakerCount} speakers, ${result.wordCount} words` : result.error,
        });
        return result;
      } catch (error) {
  ...
  async generateAIMetadata(itemId, apiKey, customPrompt) {
    try {
      ...
      log.info('clipboard', 'Processing item', {
        detail: {
          id: item.id,
          type: item.type,
          fileType: item.fileType,
      ...
      // Check if this is HTML content or generated document
        ...
        if (item.metadata?.type === 'generated-document') {
          ...
          const generatedAt = item.metadata?.generatedAt
      ...
      if (imageData) {
        ...
        const visionPrompt =
          customPrompt ||
          `Analyze this ${contentType} and generate comprehensive metadata including:
...
Return the metadata as a JSON object.`;
   ...
   */
  /**
   * Extract a URL from plain-text content like "Website Changed: ...\n\nURL: https://..."
   * Used by migration to recover URLs from change-notification items.
   */
  _extractURLFromContent(content) {
    if (!content || typeof content !== 'string') return null;

    // Try "URL: <url>" pattern first (change notification format)
    const urlLineMatch = content.match(/URL:\s*(https?:\/\/[^\s<>"{}|\\^`\[\]]+)/i);
    if (urlLineMatch) return urlLineMatch[1].replace(/[.,;:!?)]+$/, '');

    // Try bare URL anywhere in content
  ...
  /**
   * Extract a hostname/name from plain-text content like "Website Changed: onereach.ai"
   */
  _extractNameFromContent(content) {
    if (!content || typeof content !== 'string') return null;

    const nameMatch = content.match(/Website Changed:\s*(.+)/i);
  ...
  migrateWebMonitorItems() {
    try {
      log.info('clipboard', 'Checking for web-monitor items needing migration...');

      const indexItems = this.storage.index.items || [];
      let migratedCount = 0;
      const itemsToRemove = [];

      // Also load the WebsiteMonitor's persisted data for reference
      let persistedMonitors = [];
      try {
        const monitorPath = require('path').join(
          require('os').homedir(),
          'Library',
          'Application Support',
          'onereach-ai',
        ...
        const data = require('fs').readFileSync(monitorPath, 'utf8');
        persistedMonitors = JSON.parse(data);
      } catch (_e) {
        // No persisted monitors file
      }

      // Collect all items in web-monitors space
      const webMonitorItems = indexItems.filter((i) => i.spaceId === 'web-monitors');

      // Group change-notification items by URL to deduplicate
      const urlGroups = new Map(); // url -> [items]
      const _properMonitors = []; // items that already have valid URLs

      for (const item of webMonitorItems) {
        // Try to get the URL from the item itself or its content
        let url = item.url || '';
        let name = item.name || '';

        // If URL is missing, try to extract from content file
        if (!url) {
          try {
            const fullItem = this.storage.loadItem(item.id);
            const content = fullItem.content || '';

            // Try JSON content first (proper monitor items)
            try {
              ...
              url = this._extractURLFromContent(content) || '';
              name = this._extractNameFromContent(content) || name;
            }

            // Also check metadata
            if (!url && fullItem.metadata) {
              url = fullItem.metadata.url || fullItem.metadata.sourceUrl || '';
              name = name || fullItem.metadata.name || '';
            }
          } catch (_loadError) {
            log.error('clipboard', 'Failed to load item for migration', { itemId: item.id });
          }
        }

        // Try to derive name from URL if still missing
        if (url && (!name || name === 'Website')) {
          try {
        ...
        if (url) {
          ...
          urlGroups.get(url).push({ item, url, name });
        } else {
          // No URL recoverable - this item is orphaned
          log.warn('clipboard', 'Orphaned web-monitor item with no URL, marking for removal', { itemId: item.id });
          itemsToRemove.push(item.id);
        }
      }

      // For each URL group, keep ONE proper monitor item and remove duplicates
      for (const [url, group] of urlGroups) {
        // Sort by timestamp descending (newest first)
        group.sort((a, b) => (b.item.timestamp || 0) - (a.item.timestamp || 0));

        // Find if a persisted monitor exists for this URL
        const persistedMonitor = persistedMonitors.find((m) => m.url === url);

        // Keep the first (newest) item as the canonical monitor
        const canonical = group[0];
        const canonicalItem = canonical.item;

        // Ensure it has proper web-monitor fields
        canonicalItem.type = 'web-monitor';
        canonicalItem.url = url;
        canonicalItem.name = canonical.name || 'Website';
        canonicalItem.monitorId =
          canonicalItem.monitorId || (persistedMonitor ? persistedMonitor.id : canonicalItem.id);
        canonicalItem.lastChecked =
          canonicalItem.lastChecked || (persistedMonitor ? persistedMonitor.lastChecked : null);
        canonicalItem.status = canonicalItem.status || 'active';
        canonicalItem.changeCount = canonicalItem.changeCount || group.length - 1; // other items were change notifications
        canonicalItem.timeline = canonicalItem.timeline || [];
        canonicalItem.settings = canonicalItem.settings || { aiDescriptions: false };

        migratedCount++;
        log.info('clipboard', 'Fixed monitor item', { itemId: canonicalItem.id, url, name: canonicalItem.name });

        // Mark duplicate items for removal
        for (let i = 1; i < group.length; i++) {
          itemsToRemove.push(group[i].item.id);
        }
      }

      // Remove duplicate/orphaned items from index
      if (itemsToRemove.length > 0) {
        log.info('clipboard', 'Removing duplicate/orphaned web-monitor items', { count: itemsToRemove.length });
        const removeSet = new Set(itemsToRemove);
        this.storage.index.items = indexItems.filter((i) => !removeSet.has(i.id));
        migratedCount++;
      }
      ...
      if (migratedCount > 0) {
        // Save the updated index
        this.storage.saveIndex();
        log.info('clipboard', 'Web monitor migration complete', {
          migratedCount,
          removedDuplicates: itemsToRemove.length,
        });
      } else {
        log.info('clipboard', 'No web-monitor items needed migration');
      }
    } catch (error) {
      log.error('clipboard', 'Web monitor migration error', { error: error.message || error });
    }
  }
      ...
      try {
        ...
        for (const file of generatedFiles) {
          if (file.endsWith('.png')) {
            ...
            if (thumbStats.size > 5000) {
              // More than 5KB suggests real thumbnail
              break;
            } else {
        ...
        if (thumbnailPath && fs.existsSync(thumbnailPath)) {
          info: log.info('clipboard', 'Reading thumbnail from', { thumbnailPath });
          ...
          this.getPDFPageCount(filePath)
            .then((pageCount) => {
              info: log.info('clipboard', 'Async page count result', { pageCount });
  ...
  async generatePDFPageThumbnail(filePath, pageNumber = 1) {
    ...
    detail: log.info('clipboard', 'Generating thumbnail for page', { pageNumber });
    ...
    try {
      ...
      const _outputPath = path.join(tempDir, 'page.png');
      ...
      try {
        ...
        if (thumbnailPath && fs.existsSync(thumbnailPath)) {
          ...
          // Clean up
          try {
            rmSync: fs.rmSync(tempDir, { recursive: true, force: true });
          } catch (_e) {
            // Ignore
          }
  ...
  generatePDFThumbnail(fileName, fileSize) {
    try {
      ...
      const maxChars = 20;
      const displayName = fileName.length > maxChars ? fileName.substring(0, maxChars - 3) + '...' : fileName;
...
<svg width="200" height="260" viewBox="0 0 200 260" xmlns="http://www.w3.org/2000/svg">
  ...
  async generateTextPreview(filePath, item) {
    ...
    try {
      ...
      if (thumbnailFile) {
        ...
        log.info('clipboard', 'Found thumbnail', {
          thumbnailFile,
          arg2: 'size:',
          arg3: fs.statSync(thumbnailPath).size,
          arg4: 'bytes',
        });
  ...
  async generateAudioPreview(_filePath, _item) {
  ...
  registerShortcut() {
    ...
    globalShortcut.register(shortcut, () => {
      createClipboardWindow: log.info('clipboard', 'Clipboard viewer shortcut triggered');
  ...
  findMonitorByURL(url) {
    ...
    for (const [_id, monitor] of this.websiteMonitor.monitors) {
      if (monitor.url === url) {
        return monitor;
      }
    }
  ...
  async createWebsiteMonitorFromURL(url) {
    ...
    if (existingMonitor) {
      ...
      const existingItemInHistory = this.history.find((h) => h.type === 'web-monitor' && h.url === url);
      const existingItemInStorage = this.storage.index.items.find(
        (i) => i.type === 'web-monitor' && i.spaceId === 'web-monitors'
      );
      ...
      log.info('clipboard', 'Creating new monitor for', { url });
  ...
  /**
   * Sync WebsiteMonitor in-memory state from clipboard items.
   * This ensures monitors survive app restart by re-registering from storage.
   */
  async syncWebsiteMonitorFromClipboard() {
    try {
      const WebsiteMonitor = require('./website-monitor');
      if (!this.websiteMonitor) {
        this.websiteMonitor = new WebsiteMonitor();
      ...
      // Find all active web-monitor items in clipboard storage
      const monitorItems = (this.storage.index.items || []).filter(
        (i) => i.type === 'web-monitor' && i.url && i.status !== 'paused'
      );

      let registered = 0;
      for (const item of monitorItems) {
        const monitorId = item.monitorId || item.id;
        if (!this.websiteMonitor.monitors.has(monitorId)) {
          this.websiteMonitor.monitors.set(monitorId, {
            id: monitorId,
            url: item.url,
      ...
      if (registered > 0) {
        log.info('clipboard', 'Synced monitors from clipboard to WebsiteMonitor', { registered });
        // Persist the synced state
        await this.websiteMonitor.saveMonitors();
      }

      return this.websiteMonitor.monitors.size;
    } catch (error) {
      log.error('clipboard', 'Failed to sync WebsiteMonitor from clipboard', { error: error.message || error });
      return 0;
    }
  }

  startWebsiteMonitoring() {
    // Sync monitors from clipboard on startup (delayed to let app settle)
    setTimeout(async () => {
      try {
        const count = await this.syncWebsiteMonitorFromClipboard();
        log.info('clipboard', 'WebsiteMonitor startup sync complete', { activeMonitors: count });
      } catch (e) {
        log.error('clipboard', 'WebsiteMonitor startup sync failed', { error: e.message || e });
      }
    }, 10000); // 10 second delay for app to settle

    // Check websites every 30 minutes
    this.websiteCheckInterval = setInterval(
      async () => {
        try {
          // Ensure monitors are synced from clipboard
          if (!this.websiteMonitor) {
            await this.syncWebsiteMonitorFromClipboard();
          }

          // Check if there are any active monitors
          const monitorCount = this.websiteMonitor?.monitors?.size || 0;
          if (monitorCount === 0) {
            return; // No monitors, skip silently
          }

          log.info('clipboard', 'Running periodic web monitor check', { monitorCount });
          ...
          log.info('clipboard', 'Periodic check complete', { changedCount, monitorCount });
        } catch (error) {
          log.error('clipboard', 'Error in periodic web monitor check', { error: error.message || error });
        }
      },
      30 * 60 * 1000
    ); // 30 minutes
  ...
  async handleWebsiteChange(result) {
    ...
    // Find the monitor item in history (match by monitorId first, then by URL)
    let monitorItem = this.history.find((h) => h.type === 'web-monitor' && h.monitorId === monitor.id);
    if (!monitorItem && monitor.url) {
      monitorItem = this.history.find((h) => h.type === 'web-monitor' && h.url === monitor.url);
    }

    if (!monitorItem) {
      log.warn('clipboard', 'Monitor item not found in history', { monitorId: monitor.id, url: monitor.url });
      return;
    }
    ...
    const recentChanges = (monitorItem.timeline || []).filter((c) => new Date(c.timestamp).getTime() > oneDayAgo);

    if (recentChanges.length >= 5) {
      // Auto-pause this monitor - too dynamic
      log.info('clipboard', 'Auto-pausing ...: ... changes in 24h', {

...
async function runTests() {
  ...
  for (const test of testCases) {
    ...
    console.log(`${status} ${test.agent.id}: "${test.phrase}" - ${detail} (expected: ${test.shouldBid ? 'bid' : 'no bid'})`);
  ...
  if (timeResult.success) passed++; else failed++;
  ...
  console.log(`  Result: ${weatherNeedsInput ? '✓ needs input' : '✗ should need input'} - ${weatherResult.needsInput?.prompt || weatherResult.message}`);
  if (weatherNeedsInput) passed++; else failed++;
  ...
  if (weatherResult2.success) passed++; else failed++;

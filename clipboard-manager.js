...
<body>
  ...
  saveItemToFileSystem(item) {
    ...
    const metadata = {
      ...
      source: item.source || this.detectItemSource(item),  // Auto-detect source
      author: os.userInfo().username || 'Unknown',  // Current system user
      version: '1.0.0',  // Version tracking
      tags: item.tags || this.autoGenerateTags(item),  // Auto-generate initial tags
      // AI-related metadata
      ai_generated: item.ai_generated || false,  // Whether content was generated by AI
      ai_assisted: item.ai_assisted || false,  // Whether content was co-authored with AI
      ai_model: item.ai_model || '',  // AI model used (e.g., "GPT-4", "Claude 3", "Gemini Pro")
      ai_provider: item.ai_provider || '',  // AI provider (e.g., "OpenAI", "Anthropic", "Google")
      ai_confidence: null,  // Confidence score if applicable (0-1)
      ai_prompt: item.ai_prompt || '',  // The prompt used to generate content
      ai_context: item.ai_context || ''  // Additional context about AI usage
    };

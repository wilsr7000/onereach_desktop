...
function isVideoCallLink(location) {
  ...
  return lower.includes('http') || lower.includes('zoom.') ||
         lower.includes('meet.google') || lower.includes('teams.microsoft') ||
...
function isGenericLocation(location) {
  ...
  const generic = [
    'office', 'room', 'conference', 'meeting room', 'tbd', 'tba',
    'virtual', 'online', 'remote', 'phone', 'call', 'home',
    'desk', 'lobby', 'reception', 'building', 'hq', 'headquarters'
  ];
...
function _looksLikeCity(value) {
  ...
  const notCities = [
    'living room', 'bedroom', 'kitchen', 'bathroom', 'garage', 'basement',
    'office', 'desk', 'couch', 'sofa', 'bed', 'room', 'upstairs', 'downstairs',
    'home', 'house', 'apartment', 'condo', 'work', 'school',
    'here', 'there', 'inside', 'outside', 'unknown', 'none', 'n/a', 'tbd',
  ];
...
const weatherAgent = {
  id: 'weather-agent',
  name: 'Weather Agent',
  description: 'Provides weather with calendar awareness - knows your location and checks if upcoming events need travel weather',
...
- Work: (set your work city)`);
  ...
  async _fetchWeather(location, task, options = {}) {
    ...
    try {
      ...
      log.error('agent', 'Weather fetch error', {
        location,
        error: error.message,

...
- Last Topic: None`
  ...
  async execute(task, _context) {
    ...
    if (/^help[\s!.,?]*$/i.test(lower) || lower === 'help me') {
      return {
        ...
        needsInput: {
          prompt:
            'What would you like help with? I can help with time, weather, music, or just tell you what I can do.',
          agentId: this.id,
  ...
  _getHelpForTopic(topic, isAdvanced) {
    ...
    if (lower.includes('music') || lower.includes('play') || lower.includes('volume')) {
      const basic =
        "You can say 'play music', 'pause', 'skip', 'next', 'volume up', 'volume down'. I'll ask about your mood to pick good music.";
    ...
    if (lower.includes('undo') || lower.includes('cancel') || lower.includes('repeat') || lower.includes('command')) {
      return {
        success: true,
        message:
          "Say 'cancel' to stop what I'm doing, 'repeat' to hear my last response again, or 'undo' within 60 seconds to reverse an action.",
      };
    }

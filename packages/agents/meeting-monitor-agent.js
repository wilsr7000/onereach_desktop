...
const meetingMonitorAgent = {
  ...
  description:
    'Monitors active recordings in real time. Reads the live transcript and health metrics to detect audio/video issues, communication problems, and technical failures. Alerts the user with suggestions to fix problems during meetings.',
  voice: 'onyx',
...
It is a background system agent that auto-starts when recording begins.`,
  ...
  stopMonitoring() {
    ...
    if (this.memory && alertHistory.length > 0) {
      const summary = alertHistory
        .map((a) => `- [${new Date(a.timestamp).toLocaleTimeString()}] ${a.type}: ${a.message}`)
        .join('\n');
      ...
      this.memory.save().catch((err) => console.warn('[meeting-monitor-agent] memory save:', err.message));
    }
  },
...
If no issue is detected, respond: {"issue": "NO_ISSUE", "confidence": 1.0, "evidence": "", "suggestion": ""}`,
  ...
  _emitAlert(alert) {
    ...
    const recentSameType = alertHistory.find((a) => a.type === alert.type && now - a.timestamp < alertCooldownMs * 2);

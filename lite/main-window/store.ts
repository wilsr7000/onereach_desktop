...
export class TabStore {
  ...
  /**
   * The accountId the cache was populated for. Used to invalidate on
   * user switch -- the KV key is always `'default'` (server scopes
   * by accountId server-side via the SDK), but if the active accountId
   * changes we must re-read because the SDK is now using a different
   * server-side bucket.
   */
  ...
  /**
   * Active accountId, or null when signed-out (and a resolver was
   * provided so callers short-circuit reads/writes). Returns the
   * sentinel `'__legacy__'` when no resolver was provided so caching
   * still works in unit tests that don't wire auth. The KV key
   * itself is always `'default'` -- the SDK passes accountId
   * server-side for proper isolation.
   */
  ...
  private async readBlob(): Promise<TabsBlob> {
    ...
    if (accountId === null) {
      // Signed-out: empty tabs. Don't read shared data -- historically
      // that leaked one user's tabs to every install.

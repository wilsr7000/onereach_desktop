...
describe('seedKernelMenu', () => {
  ...
  it('registers exactly the kernel entries (no extras)', () => {
    ...
    const ids = ['top:app', 'top:help', 'app:about', 'app:quit', 'help:report-bug'];
    for (const id of ids) {
  ...
  it('seeds two top-level menus: app first, help last (per ADR-016)', () => {
    seedKernelMenu(handlers);
    const tops = registry.getChildren();
    expect(tops.map((e) => e.id)).toEqual(['top:app', 'top:help']);
  });

  it('app menu has About then Quit', () => {
    seedKernelMenu(handlers);
    const appMenu = registry.getChildren('top:app');
    expect(appMenu.map((e) => e.id)).toEqual(['app:about', 'app:quit']);
  });
  ...
  it('registers Dev Tools menu only when DevTools handlers are provided', () => {
    ...
    expect(registry.getChildren().map((e) => e.id)).toEqual(['top:app', 'top:dev-tools', 'top:help']);

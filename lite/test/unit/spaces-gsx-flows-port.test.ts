...
// ── views (2026-09-05): a flow's Action Desk view rides the account token ──
describe('FetchGsxFlowsPort — views', () => {
  it('lists views on the account token with the projection Action Desk asks for, accepting both link spellings', async () => {
    const { fetch, calls } = scripted({
      '/refresh_token': () => ({ body: { token: 'FLOW acct' } }),
      'discovery.edison.api.onereach.ai/api/v2': () => ({ body: { url: 'https://hub.example' } }),
      '/views': () => ({ body: [
    ...
    // The endpoint's token already reads "FLOW …" and goes out as-is — never "FLOW FLOW …".
    expect(list?.auth).toBe('FLOW acct');
...
describe('FetchGsxFlowsPort — bots (writes)', () => {
  it('createBot POSTs Studio\'s new-bot shape to /bots/new on the account token and returns the id', async () => {
    const bodies: string[] = [];
    const fetchSpy = async (url: string, init?: { method?: string; headers?: Record<string, string>; body?: string }) => {
      if (url.includes('/refresh_token')) return { ok: true, status: 200, text: async () => JSON.stringify({ token: 'FLOW acct' }) };
      ...
      if (url === 'https://hub.example/bots/new' && init?.method === 'POST') {
        ...
        expect(init.headers?.['Authorization']).toBe('FLOW acct');
        return { ok: true, status: 200, text: async () => JSON.stringify({ id: 'bot-9', isDeleted: false, accountId: ACCT }) };
      }
      return { ok: false, status: 404, text: async () => 'no route' };
    };
    const port = new FetchGsxFlowsPort({ env: 'edison', accountId: ACCT, fetch: fetchSpy });
  ...
  it('accepts the id nested under bot, and refuses an answer without one', async () => {
    ...
    const fetch = async (url: string) => {
      if (url.includes('/refresh_token')) return { ok: true, status: 200, text: async () => JSON.stringify({ token: 't' }) };
      if (url.includes('discovery.')) return { ok: true, status: 200, text: async () => JSON.stringify({ url: 'https://hub.example' }) };
  ...
  it('an account without the refresh flow (404) cannot create a bot — the 404 surfaces before any hub call', async () => {
    const { fetch, calls } = scripted({ '/refresh_token': () => ({ status: 404, body: 'no handler' }) });
    const port = new FetchGsxFlowsPort({ env: 'edison', accountId: 'dd96413e-nope', fetch });
    await expect(port.createBot({ label: 'X', description: '' })).rejects.toMatchObject({ name: 'GsxFlowsError', status: 404 });
    expect(calls.filter((c) => c.url.includes('hub.example'))).toHaveLength(0);
  });

  it('deleteBot soft-deletes (temporarily: true) on the account token', async () => {
    ...
    expect(seen).toEqual({ method: 'DELETE', body: JSON.stringify({ temporarily: true }), auth: 'FLOW acct' });
  });
});

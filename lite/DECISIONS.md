...
- **Scope (deliberately narrow for v1)**:
  - **Edison + staging + dev** supported (production deferred — bare `onereach.ai` / `my.onereach.ai` domains need extended matchers, tracked as `auth-multi-env-production` in PORTING.md). The data layer (`Map<Environment, …>` per-env sessions/tokens/bundles, per-env `persist:lite-auth-<env>` cookie partitions, per-env KV keys `lite-auth-sessions/<env>:<accountId>`) has always been multi-env-capable; the multi-env-mvp chunk just adds the missing `ENVIRONMENT_CONFIGS` entries + bumps `SUPPORTED_ENVIRONMENTS`.
  - **Edison stays primary.** Every consumer that needs an "active account / token" (KV, Files, Discovery, Neon, Spaces, Bug-report, Health, Tools, IDW) reads from edison via the `getAuthApi().getSession('edison')` / `getToken('edison')` pattern. Non-edison sessions exist solely so IDW tabs whose URLs point at those envs can clone the right cookies into the tab partition via `injectTokenIntoPartition(env, partition)`. Making the active env user-selectable is a separate chunk.
  - **Auto-sign-in on first IDW touch.** When an IDW tab loads a OneReach URL whose env has no captured session, `prepareTabAndLoad` in `lite/main-window/window.ts` automatically calls `signIn(env)` to open that env's studio sign-in window, then re-attempts cookie injection. Sign-in coalesces in-flight per env (the store's `inFlight: Map<Environment, Promise>` predates this work), so opening three staging tabs at once triggers one auth window.
...
## ADR-050: GSX-first binary assets -- bytes to the account file bucket, fileKey in the graph

- **Status**: accepted (implemented)
- **Date**: 2026-08-05
- **Context**: Sharing a Space between apps requires every binary asset to be readable by every client of the account -- full app, Lite, and agents all resolve binaries by `fileKey` against the account's GSX file bucket. Lite's v1 upload dialog instead stored file bytes as a base64 `data:` URL in the `:Asset.content` graph property ("base64 stub for v1"). Those assets were invisible to other apps, bloated the graph, and risked node-property size limits. Signing into Lite with a GSX account already connects the organizational graph twin AND the GSX file account; the stub path simply never used the latter.
- **Decision**: (1) New `items.createBinary` on the Spaces platform API: raw bytes cross IPC as an ArrayBuffer; the main process uploads them to `lite-spaces/assets/<uuid>-<sanitized-name>` via `getFilesApi().upload` and creates the `:Asset` with the resulting `fileKey` -- inline base64 never enters the graph. If the graph create fails after upload, the orphaned file is best-effort deleted. Orchestration lives in `lite/spaces/create-binary.ts` behind an injected-deps seam; pure helpers (key scheme, name sanitization, data-URL parsing, 100MB cap) in `lite/spaces/binary-asset.ts`. (2) A boot-time migration sweep (`lite/spaces/gsx-migration.ts`, `spaces.gsxMigrate` span + per-item events) lifts existing stub assets into GSX: find `content STARTS WITH 'data:'`, upload, `SET a.url = key, a.size, a.content = null`. Idempotent (converted nodes stop matching), soft-failing (signed-out aborts quietly; a bad asset is counted + skipped), capped per run. (3) The renderer upload branch now reads `File.arrayBuffer()` and calls `createBinary`; the `readFileAsBase64` stub path is deleted. Previews already resolve `fileKey` via `items.resolveFileUrl` -> signed GSX URL, and the AI enricher already prefers `fileKey` download -- both work unchanged.
- **Rejected alternatives**: renderer-side upload via a files bridge (would expose upload auth surface to renderers; main-process orchestration keeps the write path in one place); storing bytes in KV (same cross-app invisibility as the graph stub); migrating lazily on first open (leaves unshareable assets indefinitely; the sweep converges the whole account in bounded background passes).
- **Meta-test**: `spaces-binary-asset.test.ts` (key scheme, sanitization, data-URL parser), `spaces-create-binary.test.ts` (upload-before-create ordering, orphan cleanup, caps), `spaces-gsx-migration.test.ts` (idempotence, soft-fail, abort-on-no-auth, per-run cap); `event-name-conformance` covers `spaces.gsxMigrate.*`.
- **Live-validation finding (2026-08-05)**: the first real `files.upload` in Lite failed with "Cross account requests allowed to SUPER_ADMIN only" -- the Files service rejects raw `mult` cookie tokens for account-scoped writes (KV's service accepts them, which is why the gap survived every mocked test and KV worked live). Fix: `lite/files/token-minter.ts` mints an account-scoped `FLOW` token from `https://em.edison.api.onereach.ai/http/<accountId>/refresh_token` (full-app parity: `lib/edison-sdk-manager.js`), cached 50 min, shared across concurrent ops, invalidated on 401/403, re-minted per account. Wired as `ensureToken` on `SdkFilesClient` (minted bearer wins; raw binding is the signed-out fallback). This also unblocks bug-report attachments and AI Run Times TTS caching, which rode the same broken path. End-to-end proof: a seeded inline-stub asset was lifted into GSX by the running app (`spaces.gsxMigrate.finish {scanned:1, migrated:1, failed:0}`), graph node verified converted (content cleared, `a.url = lite-spaces/assets/<uuid>-...`, size intact).

## ADR-051: Per-space visibility -- gating is an option, not a mandate

- **Status**: accepted (implemented)
- **Date**: 2026-08-05
- **Context**: With assets GSX-backed (ADR-050), Spaces are truly shareable across apps -- but `LIST_SPACES` showed every Space to every account member, so membership (`[:HAS_ACCESS]`, ADR-048 Phase 4 v2) was an audience annotation, not an access gate. Mandatory gating would break the account-wide "everything visible" model existing users rely on; no gating leaves no way to keep a Space private to its members.
- **Decision**: Visibility is a per-space option: `s.visibility` is `'open'` (default -- every pre-existing Space keeps exactly its old behavior) or `'restricted'` (members-only). Enforcement lives in the Cypher read surface via two shared predicates: SPACE_VISIBLE (sidebar list, in-space item list) and ASSET_VISIBLE (direct get, search, home feed -- an asset is visible when uncategorized or in >= 1 open-or-accessible Space; an item in both a restricted and an open Space is visible, because it genuinely lives in the open one). The viewer is resolved main-side from the session (lowercased email, accountId fallback -- the same identity convention as the renderer's whoAmI probe) and injected into the SDK client as a constructor seam, so no public API signatures changed. Flipping a Space to restricted auto-MERGEs the flipper's `[:HAS_ACCESS]` in the same transaction (no lock-yourself-out). UI: a toggle chip in the Space header on every Space kind, a lock pill in the sidebar, and a member-management strip on restricted Spaces (the shared dashboard keeps its richer row; the member prompt/remove/chips gained an injectable refresh seam).
- **Known limitation (documented, deliberate)**: the events/commit timeline can still name items from restricted Spaces (titles in activity rows); gating it needs commit->space joins and is deferred until someone actually needs it.
- **Meta-test**: `spaces-visibility.test.ts` -- query-shape guards (every gated read carries the predicate + `$viewerId`; UPDATE_SPACE carries the write-flag + auto-grant MERGE), viewer normalization, enum validation, row mapping. Live-verified against the real graph: member sees / stranger blocked / signed-out blocked / auto-grant edge MERGEd (6/6).

## ADR-052: GSX window automation -- deterministic scripts + AI repair, graded by the script's own assertions

- **Status**: accepted (implemented)
- **Date**: 2026-08-07
- **Context**: Product wants API support for opening GSX windows (Designer, Flows, Files) inside Lite and navigating aspects of their UI -- both as pre-created deterministic scripts and as a hybrid where an LLM adapts the scripts, with an evaluation feedback loop as the load-bearing piece. The full app has the raw ingredients but no coherent loop: `lib/gsx-autologin.js` (window + partition + token), `lib/browsing-api.js` (snapshot/act), `lib/autopilot-script-cache.js` (LLM actions -> cached script -> assertion-driven invalidation). Lite had no GSX window at all ("GSX window content" sat in the Deferred Queue).
- **Decision**: New `lite/gsx/` module. (1) **Scripts are the contract**: versioned JSON step lists (`navigate`/`waitFor`/`click`/`fill`/`assertVisible`/`assertUrl`/`assertText`/`wait`) with `{param}` substitution and selector-plus-textFallback resilience (the `sso-skip.ts` idiom). The LLM EDITS scripts, it never free-drives the page -- replay is deterministic and model-free on the happy path. (2) **Assertions are the eval**: every script carries its own assertions; every run is graded (`pass`/`fail`/`error`/`repaired-pass`/`repaired-fail`) into a persisted `GsxRunRecord` corpus with per-script stats. (3) **The loop**: fail -> live interactive-element snapshot -> `lite/ai` chat repairs the steps -> validated through the same gate as human scripts -> re-run -> pass promotes a `learned` variant that shadows the read-only seed; 3 consecutive failures demote back to the seed. Every transition is a typed event (ADR-032). (4) **Windows**: standalone BrowserWindow per env on `persist:lite-gsx-<env>`, auth cookies injected before loadURL (ADR-042), NO preload (ADR-038 -- driven entirely from main via `executeJavaScript`), navigation contained to `*.onereach.ai`, Chrome-UA disguise. (5) Electron-free `api.ts`/`store.ts`/`runner.ts`/`repair.ts` with the window port, chat, accountId, and persist dir all injected -- the whole loop unit-tests with fakes.
- **Rejected alternatives**: free-driving LLM agent per navigation request (cost + nondeterminism + unauditable actions on an authed surface); porting `lib/browsing-api.js` wholesale (1970 lines of session/HITL machinery Lite doesn't need yet); WebContentsView tab in the main window (Designer wants a large dedicated window, and full's GSX precedent is a standalone window); giving the automation window `preload-lite.js` (the punch-list Neon finding makes that a live risk).
- **Meta-test**: `gsx-api.test.ts` (Rule 12 conformance + error contract), `gsx-runner.test.ts` (step compilation, param substitution, grading, assertion-vs-action stop semantics), `gsx-repair.test.ts` (prompt contract, response parsing/validation), `gsx-store.test.ts` (the full loop with fakes: repair promotion, learned invalidation, stats, persistence); `event-name-conformance` covers `gsx.*`.

## ADR-053: GSX teach mode -- record a walkthrough, save a parameterized template

- **Status**: accepted (implemented)
- **Date**: 2026-08-07
- **Context**: ADR-052's scripts must be authored by hand (or repaired by the LLM after failing). Product wants "teach the system how to navigate" -- the user demonstrates once, and the system remembers it via labels/descriptions as a TEMPLATE that can later click different elements (record clicking flow "Billing Bot", replay with `{flowName: "Support Bot"}`). Constraint: GSX windows deliberately carry no preload (ADR-038/052), so the page cannot push events to main.
- **Decision**: `lite/gsx/recorder.ts` + store orchestration. (1) **Record**: an injected, idempotent page recorder captures capture-phase clicks and final `change` values with ranked selector candidates (data-testid > id > aria-label > name > data-* > class > nth-of-type) and the element's human label (aria-label / `<label>` / placeholder / title); the buffer write-throughs to sessionStorage so the click that triggers a navigation survives page teardown; main drains on a poll (default 400ms), synthesizes `navigate` events from URL deltas, re-installs after navigations, and sanitizes every drained event (page world is untrusted). Password inputs are never recorded. (2) **Templatize**: `stopRecording` builds deterministic steps (accountId/env back-substituted to `{accountId}`/`{env}`; trailing `assertUrl` pins the end state so even a raw recording self-evaluates), then the AI module generalizes: content-specific literals become named `{params}` derived from element labels, structural clicks stay literal, assertions are kept or strengthened. Model output passes the same `validateScript` gate as everything else; on any generalization failure the deterministic recording is saved unchanged -- a walkthrough is never lost to a flaky model call. (3) The product is an ordinary `learned` script in the ADR-052 eval loop (graded runs, AI repair, seed-floor demotion when shadowing a seed). API: `startRecording` / `getRecording` / `stopRecording` / `cancelRecording` on `GsxApi` + `window.lite.gsx.*`; new codes `GSX_NOT_RECORDING`, `GSX_EMPTY_RECORDING`; span `gsx.record.*` + instants `gsx.record.cancelled` / `gsx.record.generalized`.
- **Rejected alternatives**: preload-based event push (violates the no-preload boundary that keeps `window.lite.*` away from automated pages); Electron debugger/CDP input recording (heavyweight, records coordinates not semantics -- labels are what make params nameable); recording into a video/screenshot trace for pure-LLM replay (nondeterministic replay, no eval floor); deterministic-only templatization with positional `{param1}` names (labels exist; unnamed params are unusable at the call site).
- **Known limitation (documented, deliberate)**: the poll can misorder a drain against a same-tick navigation, and cross-origin navigations drop the sessionStorage buffer; both degrade to a slightly sparser recording, which generalization tolerates.
- **Meta-test**: `gsx-recorder.test.ts` (script safety properties incl. password guard + sessionStorage survival, step conversion, back-substitution, generalization prompt/parse contract), `gsx-store.test.ts` teach-mode suite (record->save, generalize + garbage fallback, empty/not-recording errors, sanitization); `event-name-conformance` covers `gsx.record.*`.

## ADR-054: UI-automation agents -- taught walkthroughs become named, self-describing, invokable agents in the "GSX Build" Space

- **Status**: accepted (implemented)
- **Date**: 2026-08-07
- **Context**: Teach mode (ADR-053) produces templates, but the UX product wants is agent-shaped: the user records and NAMES a walkthrough ("open-designer"); the SYSTEM describes what it does; anyone who knows the agent's name can call it via API, passing free-form details from which params are extracted. Agents-as-assets already exists in Spaces (parent :Agent nodes, items.createAgent, OKF definitions) -- taught agents should live there too, in a core "GSX Build" Space.
- **Decision**: `lite/gsx/agent.ts` + store orchestration. (1) **Creation** (`stopRecordingAsAgent(windowId, {name, hint?, publish?})`): ONE model call describes (title + 1-3 sentence description written for a prospective caller), generalizes (content literals -> `{params}` named from element labels), and documents every param (one-line description written so a later extraction is unambiguous). Deterministic floor: no AI -> slug-derived title, params scanned from `{placeholders}`, agent fully functional with structured params. The template is an ordinary learned script `agent.<name>` in the ADR-052 eval loop. (2) **Invocation** (`invokeAgent(name, {details?, params?})`): a second, cheap model call maps the caller's free-form details onto the documented params ("open the billing bot flow" -> `{flowName: "Billing Bot"}`); structured params merge OVER extraction; anything still missing throws `GSX_MISSING_PARAMS` naming the gaps -- extraction never invents values (prompt rule + lenient parser that degrades to missing). (3) **Publication**: best-effort OKF (frontmatter + description + invocation snippet + behavior contract) into the "GSX Build" Space (find-or-create, violet, `items.createAgent` with `agentType: 'workflow'`); wired as an injected seam in `main.ts` so gsx never imports spaces; `spaceItemId` links the registry entry to the Space asset. Publication failure is decoration failure -- the agent works locally regardless. Registry persists in `gsx-automation.json` (schema-tolerant `agents` field).
- **Rejected alternatives**: registering taught agents in the task-exchange auction (heavyweight, wrong invocation model -- these are direct-call tools, not bidding participants); making the Space the source of truth for invocation (Spaces is presentation/sharing; invocation must work signed-out and offline); letting the extraction model fill unstated params with guesses (silent wrong-element clicks on an authed surface; loud MISSING_PARAMS is the only safe degradation); user-authored descriptions (the system watched the walkthrough -- it writes better, more consistent metadata, and the user only supplies the name + optional hint).
- **Meta-test**: `gsx-agent.test.ts` (name contract, param scanning, fallback meta, create/extract prompt + parse contracts incl. never-extract-unasked-params, OKF rendering), `gsx-store.test.ts` agent suite (create->describe->publish, details extraction, structured-over-extracted precedence, MISSING_PARAMS, no-AI floor, soft publish failure, persistence + delete cascade); `event-name-conformance` covers `gsx.agent*`.

## ADR-055: Checklists as first-class graph nodes -- Gawande's doctrine encoded as schema, required gates on ticket status

- **Context**: Tickets needed pre-flight/post-flight discipline (release reviews, launch readiness). A checklist could have been a JSON blob on the ticket, but The Checklist Manifesto's core claim is that checklists are REUSABLE, VERSIONED artifacts owned by a team, not per-ticket scratch text -- and the book's operational parameters (DO-CONFIRM vs READ-DO mode, an explicit pause point, killer items, brevity) are schema, not prose.
- **Decision**: `(:Checklist)` node per Space with doctrine-derived required params: `mode` (DO-CONFIRM | READ-DO), `pausePoint` (WHEN it runs -- mandatory; a checklist without one is just a list), `items` JSON (each `{text, killer?}`; killer = the kill-the-flight items the book says checklists exist for), `version`/`revisedAt` (living document), denormalized `itemCount`. Items hard-capped at 12 with a remediation citing the 5-9-item/60-90-second doctrine -- REJECT, never truncate (silently dropping a killer item defeats the artifact). Tickets connect via typed edges `PREFLIGHT_CHECKLIST` / `POSTFLIGHT_CHECKLIST` with `obligation: required | recommended` on the edge. Run state lives on the edge as `checkedIdx` (native int list; check/uncheck are atomic Cypher list ops, completion = `size(checkedIdx) = itemCount` -- no JSON read-modify-write races). **The gate**: `updateTicket` calls `assertTicketStatusAllowed` before writing status -- incomplete REQUIRED preflight blocks leaving `open`; incomplete REQUIRED postflight blocks entering `done`; the error (`SPACES_CHECKLIST_REQUIRED`) names the blocking checklist. `recommended` NEVER blocks (a gate that blocks on advice gets worked around, poisoning trust in required ones); transitions to `blocked` are never gated (parking a ticket is always allowed). Both node and edge types registered in the graph `(:Schema)` registry via additive `ensureChecklistSchema` MERGE at init. All reads visibility-gated per ADR-051; per-item attribution is edge-level (`lastCheckedBy`/`lastCheckedAt`), traded for atomicity.
- **Rejected alternatives**: checklist-as-ticket-property (not reusable, not versioned, no shared team artifact); a `CHECKLIST` edge with a `phase` property (relationship TYPES are the graph's signal -- two named edges make pre/post queryable without property filters and match how the schema registry documents relationships); per-item check nodes (full attribution but N+1 writes and non-atomic completion); gating on `recommended` with an override flag (the book's lesson inverted -- advice must stay advice); blocking `done -> reopened` on postflight (re-opening is a correction, not a completion).
- **Meta-test**: `spaces-checklists.test.ts` (doctrine validation incl. reject-not-truncate cap, mode/pausePoint mandatory, gate blocks/allows matrix, recommended-never-blocks, gate-runs-before-write ordering, atomic-list-op + visibility-predicate query shapes, schema-registry coverage); `spaces-visibility.test.ts` sweep auto-covers the new reads; `event-name-conformance` covers `spaces.checklists.*`.
- **Addendum (2026-08-08): revise + delete + the Space-manager library.** The user asked for create/view/edit/check in the Space manager as well as tickets; create/check existed (ticket attach flow), view/edit/delete did not. (1) **Revise** (`UPDATE_CHECKLIST`): version bumps, `revisedAt` stamps, and — the doctrine call — **every attached ticket's run state resets in the same statement** (both edge types, `checkedIdx = []`, attribution cleared). `checkedIdx` is positional, so a check made against v1's item list says nothing about v2's; carrying runs across a revision would let a ticket pass a gate it never actually ran. Same validators as create (cap-12 REJECT, mode + pausePoint mandatory). (2) **Delete** (`DELETE_CHECKLIST` + `COUNT_CHECKLIST_ATTACHMENTS`): refused with `SPACES_CHECKLIST_ATTACHED` while ANY ticket holds the checklist — deleting out from under a REQUIRED gate would silently un-gate those tickets; detach everywhere first. (3) **Library UI**: a Checklists section on the shared-space dashboard — list (mode, items + killer count, vN, pause point, used-by count), expandable read-only item view, one editor panel for create AND revise (the revise panel states plainly that saving resets runs), delete behind the confirm. Runs still happen ONLY on tickets at their pause points — the library manages the artifacts, not the runs. Rejected: silent-detach-then-delete (un-gates tickets nobody chose to un-gate); preserving run state across revisions (positional indexes lie after an edit); a separate edit-items-in-place API without a version bump (defeats the living-document contract).

## ADR-057: Asset versions -- edits snapshot the replaced state, restores are edits, history follows the asset into every Space

- **Context**: Assets were edited destructively -- `UPDATE_ITEM` did `SET a.content = $content`, so the prior state was gone; the only trace was a `LAST_EDITED` edge + `updatedAt`. There was no way to see what an asset looked like before an edit or to go back. Soft-delete/restore recovers a DELETED asset, not an earlier EDIT; the checklist `version` counter doesn't retain prior item lists. Users need real versioning: edits go back in time.
- **Decision**: `(:AssetVersion)` node per replaced state, linked `(a)-[:HAS_VERSION]->(v)`. The asset node stays the SINGLE source of the CURRENT state -- which is exactly why multi-space membership is free: the asset is one node with `BELONGS_TO` edges, so a new version is instantly what EVERY containing Space sees (no per-space copies to reconcile). `UPDATE_ITEM` snapshots the outgoing state as a version IN THE SAME STATEMENT as the SET (no lost-snapshot window), gated by a client-side `$doSnapshot` computed from a pre-read hash so a no-op edit creates no version. Each version carries `seq` (1-based, denormalized onto `a.version`), the pre-edit title/description/content/fileKey/mimeType, a `contentHash`, and `editedBy`/`editedAt`. The RESULTING state's hash is matched against older snapshots -- when you edit back to a state you've had before, the new version records `currentMatchesSeq` ("this equals v3"), surfaced as an amber `= vN` badge. History is pruned past `MAX_ASSET_VERSIONS` (50). **Restore** (`RESTORE_ASSET_VERSION`) snapshots the CURRENT state first (stamped `restoredFromSeq`), then copies the target snapshot onto the asset -- so a restore is an ordinary, itself-restorable edit and history NEVER rewrites. Every edit now also writes an `item:edited`/`item:restored` Commit, so the ACTIVITY panel finally shows edits (2026-08-08 logging review). **AI change summary**: after each edit, `main.ts` fires a fire-and-forget, deadline-raced (45s) Claude call that diffs old vs new content into a one-sentence `changeSummary` written onto the newest version via `ANNOTATE_LATEST_VERSION` -- best-effort, never blocks or fails the edit, degrades to the version title. All version reads are ADR-051 `ASSET_VISIBLE`-gated. UI: a History section on the detail pane (Current marker + one row per snapshot with who/when, the AI summary, restore/duplicate badges, View + Restore), and a read-only version viewer modal.
- **Rejected alternatives**: per-space version copies (the asset is one node in N spaces; copies would diverge and multiply the write); versioning via the existing `:Commit` log (commits are attribution/activity, not state -- they never stored content); overwriting-with-undo-buffer in the renderer (lost on reload, not shared, not in the graph); making the user write the change summary (the model watched the diff -- it writes a more consistent one, and it's optional decoration anyway); blocking the edit on the AI summary (a wedged model call must never hold up a save -- the extractAssetMetadata hang lesson).
- **Meta-test**: `spaces-versions.test.ts` (hashAssetState no-op-edit rule; UPDATE_ITEM snapshot-only-on-change + prune + item:edited commit shape; RESTORE snapshots-present-first + restoredFromSeq; all version reads carry `ASSET_VISIBLE`; schema-registry write covers AssetVersion + HAS_VERSION); `spaces-visibility.test.ts` sweep auto-covers the new reads; `event-name-conformance` covers `spaces.items.versions.*`.

## ADR-058: Read all space members, not just :Asset -- Lite renders what any graph writer puts in a Space

- **Context**: Lite's Spaces is a reader of a SHARED graph — WISER Playbooks, agents, and other apps all write to it, and the app's own copy promises "a Space made in WISER Playbooks, an asset added by an agent" appears in Lite. But every content read hard-filtered `(a:Asset)`, while WISER writes `:Playbook` and `:Note` labels (no `:Asset`). Result (2026-08-08 bug report): a user's email `:Note` and 234 `:Playbook` nodes were invisible — the WISER Playbooks Space showed 0 items despite holding 7. Confirmed against Aura: `BELONGS_TO`-to-Space label sets are `:Playbook` ×234, `:Asset` ×40, `:Note` ×1, `:Checklist` ×1.
- **Decision**: content reads match members by a label SET, not the single `:Asset` label. `SPACE_MEMBER = (a:Asset OR a:Playbook OR a:Note)` — Lite's own typed assets plus the two foreign renderable kinds — deliberately EXCLUDING the managed `:Checklist` (surfaced in its own library, not the item grid) and graph plumbing (`:Space`/`:Person`/`:Commit`/`:AgentType`/`:AgentEndpoint`). `kind` derives from the label when the node isn't a typed `:Asset`: `:Playbook -> 'playbook'`, `:Note -> 'text'` (its `type` is a note SUBtype like 'Basic', not a Lite ItemKind, so it must not leak through as the kind), else `coalesce(a.type, a.assetType, 'other')`. Applied to LIST_ITEMS_IN_SPACE, GET_ITEM (id lookup now label-guarded, not `:Asset`-pinned, so a `:Note`/`:Playbook` opens in the detail pane), HOME_RECENT_ITEMS (foreign writes show in the timeline), SEARCH_ITEMS (they're findable), and LIST_ITEMS_UNCATEGORIZED. All five stay ADR-051 visibility-gated. Left as `:Asset`-only on purpose: AssetVersion/tag/metadata mutations (Lite-authored assets only), ticket + agent-library + entity-count queries, and the itemCount OPTIONAL MATCH.
- **Rejected alternatives**: a migration that stamps `:Asset` onto foreign nodes (we don't own WISER; new writes reappear); dual-writing labels (same); a per-label UNION (multiplies query count and the visibility-sweep surface); showing EVERY BELONGS_TO node regardless of label (pulls `:Checklist` and any future plumbing into the grid — the allow-list is safer than a deny-list); a renderer-side filter (the rows never cross the SDK, so the fix belongs in Cypher).
- **Meta-test**: `spaces-sdk-client.test.ts` ADR-058 block (all five reads carry the member set; `:Checklist` excluded; label->kind derivation; reads stay `$viewerId`-gated); `spaces-visibility.test.ts` sweep still passes (predicates intact). Live-verified: the WISER Playbooks Space now lists the user's email `:Note` + the 6 `:Playbook` nodes.

## ADR-059: Universal notes -- Lite conforms to the registry Note contract (v1.1.0) as a full read/write citizen

- **Context**: The user's direction: "we want notes to be universal standard across all apps and agents." The standard ALREADY exists — the graph's `(:Schema {entity:'Note'})` registry node (v1.1.0, validator + instructions) governs `:Note` entities written by WISER Playbooks / OR-Mobile and agents: id `note_<UUID>` MERGEd on id, subtype labels (Basic→BasicNote, …), body in the shared Edison KV store (`notes:<account>` / `note:<id>` — verified reachable through Lite's own KV client), graph node as index + mirrors, `BELONGS_TO` + `CONTAINS` membership in lockstep, `schema_version` + `updated_by_actor_type`/`updated_by_source` stamps on every write, and newer-wins conflict on `updated_at` (epoch ms) where a stale writer skips the property merge AND the KV write. ADR-058 made Lite read these; edits still dead-ended (UPDATE_ITEM matched `:Asset` only → silent no-op) and a blind graph write would have desynced WISER's KV mirror.
- **Decision**: `lite/spaces/note-standard.ts` owns the contract (constants, id/kv helpers, preserve-unknown KV body merge, `UPDATE_NOTE_GUARDED`, `createNoteCypher`). (1) EDIT: `updateItem` routes items carrying `:Note` through the conformant path — guarded stamp write FIRST (newer-wins gate; 0 rows → `SPACES_NOTE_CONFLICT`, nothing mutated, KV skipped per the registry rule), then for dual-label `:Asset:Note` the ADR-057 version machinery (snapshot still captures the PRE-edit state), then the KV body merge-write which re-reads the body and preserves every foreign field (`basicNoteData`, provenance, space pointers). KV failure after the graph write surfaces `SPACES_NOTE_KV_WRITE_FAILED` (retry re-syncs; merge re-reads so nothing is lost). Kind changes on notes are refused (`SPACES_NOTE_TYPE_LOCKED` — `type` is the universal SUBTYPE, not a Lite kind). (2) CREATE: text born in a Space becomes a dual-label `:Asset:Note:BasicNote` citizen — universal (WISER/agents see + edit it) AND fully Lite-capable (versions, tags, metadata) — with both membership edges, registry stamps, KV pointer + body (OR-Mobile-compatible shape, `sourceApp: 'Onereach.ai Lite'`). Signed-out creates degrade to graph-only (null KV pointer). Uncategorized text stays a plain `:Asset` draft (notes are Space residents). (3) MEMBERSHIP: MOVE/ADD/REMOVE/SOFT_DELETE relaxed to the ADR-058 member label set and maintain `CONTAINS` in lockstep for `:Note` members. KV store + account identity are INJECTED (`noteKv`/`noteAccount` in the client config) — spaces never imports lite/kv; account is the session email VERBATIM (KV collections are case-sensitive; OR-Mobile writes it un-lowercased, unlike ADR-051's lowercased viewerId).
- **Rejected alternatives**: editing the graph node only (desyncs WISER's KV mirror — the KV body is what WISER reads); Lite-created notes as pure `:Note` (loses ADR-057 versions/tags; the dual label costs nothing — extra labels don't break `MERGE (n:Note {id})` writers); last-write-wins without the guard (the registry mandates newer-wins; silent clobbers across apps are the exact class of bug ADR-058 fixed the read half of); importing lite/kv directly in sdk-client (boundary violation; injection keeps tests hermetic); migrating WISER's existing notes to `:Asset` (we don't own WISER; new writes reappear unlabeled).
- **Known seams (documented, deferred)**: moving/removing a note does NOT rewrite the KV body's denormalized `spaceId`/`spaceName` (multi-space membership makes a single pointer ill-defined; WISER's own UI keys off the graph edges); Lite soft-delete sets `deletedAt`, which WISER ignores — a "deleted" WISER note disappears from Lite but stays visible in WISER (cross-app delete semantics need a registry amendment); non-Basic subtypes (Checklist/Ticket/… scalar mirrors) render but Lite only authors Basic.
- **Meta-test**: `spaces-note-standard.test.ts` (27 tests: id/label/pointer shapes, preserve-unknown merge, guard + stamps in both timestamp conventions, conflict skips KV, dual-label runs ADR-057, signed-out + graph-only degradations, create shape, membership lockstep); legacy create/membership shape tests re-anchored. Live-verified against Aura + the shared KV (see commit).

## ADR-060: Recent is graph-truthful -- cross-writer activity ordering + member-true counts

- **Context**: User ask (2026-08-10): "can recent show most recent edited or created at the graph level, so if somebody else edited or added something recently it shows in the app under recent." It couldn't, twice over. (1) HOME_RECENT_ITEMS ordered by STRING-compared timestamps; Lite writes ISO strings while WISER/GSX/the note standard write epoch ms, and '2026…' > '17864…' lexicographically — so a foreign writer's fresh edit could NEVER outrank a stale Lite item. (2) The sidebar RECENT ranked Spaces by the Space node's OWN updatedAt, which only changes on rename — member adds/edits by anyone never bumped it. Same family: LIST_SPACES/SPACE_ITEM_COUNT/UNDELETE_SPACE/UNCATEGORIZED_COUNT still counted `(a:Asset)` only, so WISER-heavy spaces showed itemCount 1 while rendering 7 (post-ADR-058 grids count ≠ badge).
- **Decision**: a TOTAL Cypher normalization macro `tsMs(expr)` folds every observed timestamp shape into epoch ms — regex-gated branches for 10-digit epoch seconds (widened ×1000), all-digit epoch ms, and strict-ISO (`datetime().epochMillis`), ELSE 0 — total by construction because the Edison flow maps Cypher errors to empty 200s, so ONE malformed row must degrade to "old", never blank the list. HOME_RECENT_ITEMS computes `activityMs = tsMs(coalesce(updated…, created…))` per member and ORDERs BY it (the LIMIT now cuts the true freshest N; client-side re-sorting could never fix a mis-ordered server cut). LIST_SPACES projects `lastActivityMs = max(member activity, space self)`; `toSpace` maps positive values to ISO `Space.lastActivity`; the renderer's `sortSpaces('recent')` prefers it over the node's own stamps. All four count sites now count the ADR-058 member label set, live rows only. Cache TTL (60s) + manual ⟳ bound how quickly another writer's change surfaces.
- **Rejected alternatives**: client-side over-fetch + normalize (unsound — the string sort partitions by FORMAT, so the fresh foreign item can sit beyond ANY fetch horizon); unguarded `datetime()` parse (one legacy garbage value empties the whole read via Edison); migrating all timestamps to one format (we don't own the other writers; new writes reappear); ordering by `_history`/Commit events (not all writers produce them — property stamps are the one universal signal).
- **Meta-test**: ADR-060 block in `spaces-sdk-client.test.ts` (both queries carry all normalization branches incl. seconds-widening + ELSE 0; ORDER BY activityMs replaces the string coalesce; lastActivityMs projected and mapped, 0 stays unset; all count sites member-labeled + live-only) and `spaces-renderer.test.ts` (a Space with fresh member activity outranks a freshly-renamed idle Space). Live-validated against Aura pre-commit: LIST_SPACES ranks WISER Playbooks first (WISER edit minutes prior, itemCount 1→7) and HOME_RECENT_ITEMS interleaves ISO- and epoch-writers in true time order.
- **Addendum (same day) — every timeline row knows its Space**: user follow-up: the Recent feed "should subtly indicate the space the asset is from." Item rows already chip via `otherSpaces[0]`, but commits written without `spaceId` (older edit/restore paths) rendered chip-less event rows forever. HOME_RECENT_EVENTS now resolves the Space AFTER the LIMIT (so the fallback expansion never touches the full commit scan) via `(c)-[:TOUCHED]->()-[:BELONGS_TO]->(other:Space)`, gated by OTHER_SPACE_VISIBLE — the same rule as otherSpaces chips, so restricted Space names never leak; a commit-recorded `spaceId` also picks up its real display name by id. Renderer unchanged (the subtle "in ● Space" chip already renders whenever the row carries a space). Live-verified: 25/25 recent events attribute (previously the item:edited/item:restored rows had none). Tests pin the post-LIMIT placement, the coalesced projections, and the visibility gate.

## ADR-061: WISER Meeting -> Lite, Tier 2 -- completed meetings mirror into the shared graph

- **Context**: user asked how hard bringing WISER Meeting into Lite would be; chose the recommended tiered path (2 -> 1 -> 3). The recorder (full app) saves meetings to the LOCAL OR-Spaces store, and its OmniGraph sync is off without a configured neo4jPassword — verified: the shared Aura graph held ZERO :Meeting/:Transcript nodes. Enabling the full spaces-sync instead was rejected: it would push ~60 personal local spaces into the shared account graph (space-universe flood).
- **Decision**: a targeted bridge, `lib/meeting-graph-bridge.js` (full app), pushes ONLY completed-meeting artifacts through the same Edison `/omnidata/neon2` proxy Lite uses, shaped for Lite's existing read paths: `(:Asset:Meeting {type:'text'})` with summary/decisions/action-items markdown, `(:Asset:Transcript {type:'transcript'})` with the full transcript (180k-char cap, <50-char noise gate — agent-readable per the meeting vision), `(:Asset:Recording {type:'other'})` metadata stubs (file stays on host; no upload), all `BELONGS_TO`+`CONTAINS` an auto-ensured open "WISER Meetings" Space (reuses any live same-named space, else deterministic `space-wiser-meetings`), `HAS_TRANSCRIPT`/`HAS_RECORDING` edges, dual-convention epoch-ms stamps (ADR-060 ranks fresh meetings atop Recent), and idempotent `item:added` Commits (`meeting-add-<id>`) so the Home feed announces them with space chips. Hooked fire-and-forget in `recorder:complete-meeting` AND `recorder:post-meeting-analyze` (id-keyed MERGEs — the second push only enriches with the summary). Contract: best-effort, NEVER throws — a graph outage cannot block meeting completion. Zero Lite-side code: ADR-058/059/060 render it all.
- **Rejected alternatives**: enabling the full app's spaces-sync (floods the shared graph with local spaces); Lite-side polling/import of OR-Spaces (wrong direction — the graph is the shared truth); uploading recordings (241MB blobs through a Cypher proxy; the transcript is the agent-readable value — revisit with a real file-upload path); waiting for the Tier-3 recorder port (this ships the platform value now).
- **Known seams**: creds are the same baked dev-account record as lite/neon (punch-listed; rotate together); meetings land in "WISER Meetings" rather than a per-meeting-space mapping (local spaces have no shared-graph identity — users can move items via Lite, which handles all member kinds post-ADR-059); the bridge activates on the next FULL-app release (it ships with v5.0.x cadence, not lite's).
- **Meta-test**: `test/unit/meeting-graph-bridge.test.js` (13 tests: proxy wire contract with in-body creds, existing-space reuse vs deterministic MERGE, dual-label node shapes + both membership edges + dual-convention stamps, deterministic commit hashes, HAS_TRANSCRIPT/HAS_RECORDING links, transcript gates/cap, recording stubs announce-free, never-throws, no-id rejection). Live-verified end to end: synthetic meeting pushed to real Aura -> Lite dev app showed "WISER Meetings" ranked #1 in RECENT (3 items), all three artifacts in the Home feed with space chips, WISER Meeting as an ACTIVE THIS WEEK contributor; synthetic data fully cleaned after.

## ADR-062: The meeting ring -- live meetings ring Lite through the graph, no polling loops, no notification service

- **Context**: user direction: people should join WISER meetings from Lite via web NOW (Tier 1), with a ring ("when I start a meeting and invite someone the app rings") — explicitly without client loops and without notification-infrastructure complexity. Strict zero-loop + instant + zero-infra is a pick-two; the design resolves it by reusing two things that already exist: the shared graph as the signaling bus, and Lite's spaces cache-refresh stream + window-focus events as the receiver. Ring relevance correlates with presence, presence correlates with app events — so event-driven checks make rings feel instant exactly when someone can answer, and a worst-case one-heartbeat delay when idle is honest.
- **Decision**: (1) SENDER (full app): `recorder:livekit-create-room` fire-and-forgets `announceMeetingLive` — an ephemeral `(:MeetingLive {id: live_<room>, title, joinUrl, host, startedAt, lastSeenAt})` signal node, deliberately NOT `:Asset` and with NO Space membership (a doorbell, not an artifact — nothing else renders it). The join URL is composed main-side (published guest page + this install's link-signing public key), title = de-salted room slug. Announces sweep stale signals (>2h); `recorder:session-end` deletes eagerly; read-side TTL (30 min) covers crashed hosts. recorder.html untouched (live concurrent edits). (2) RECEIVER (Lite): `listLiveMeetings` (LIMIT 5, TTL-guarded, quiet span) checked ONLY on existing events — the cache `onUpdate` stream and `browser-window-focus` — with a 20s floor; NO new timers. Unseen ids ring once per app-run (a still-live meeting SHOULD re-ring after relaunch): `shell.beep()` + native `dialog.showMessageBox` (Join / Dismiss) + an OS `Notification` when Lite is unfocused (click = join) — zero new windows, bundles, preloads, or IPC channels. Join = `getMainWindowApi().openTab(joinUrl)`, which auto-raises via the a8f1bfa machinery. (3) TIER 1: every tab session gets a `setDisplayMediaRequestHandler` (`useSystemPicker` on macOS 15+ = the native screens-and-windows picker; primary-screen fallback), WeakSet-idempotent; mic/cam ride the normal OS TCC prompts (entitlements already shipped).
- **Rejected alternatives**: a dedicated poll loop (user constraint); LiveKit standing "ring rooms" (per-client 24/7 connections on the shared — and still leaked-key — project); a push/notification service (the complexity being avoided); ringing via space-item writes (artifacts as signals pollute Recent + spaces); a custom ring window (native dialog ships today with zero surface).
- **Meta-test**: bridge tests (announce shape: no `:Asset`/membership, startedAt coalesce, sweep, dedupe id, teardown, never-throws, prettyRoomTitle) + lite tests (query shape TTL/LIMIT/no-Asset, row mapping, ttl default+forward). LIVE-VERIFIED BY THE USER: synthetic announce → Lite dev build rang on the focus check (`meeting ring … reason: focus` in logs) → the user themselves clicked Join → the real published guest page rendered in a Lite tab (link-verification hard-fail shown for the keyless test link, as designed). Test signals cleaned.

## ADR-063: Chrome-parity for third-party tabs -- close every gap that sends a user back to Chrome

- **Context**: user report: "when a third-party chat client is open in the app many things like pop ups and others don't work as they would in Chrome... people will not use it and go back to Chrome." Audit found the gaps concentrated in the tab `WebContentsView`s: (1) the tab window-open handler (`buildPopupHandler`, an earlier OAuth fix) routed every non-OAuth, non-same-site open to `shell.openExternal` — LITERALLY opening Chrome; same-site target=_blank became floating popup windows; (2) no right-click menu at all; (3) no download handler on tab sessions (the downloads module's `attachToSession` existed for exactly this and was never wired); (4) no PDF viewer (`plugins` off → PDFs download); (5) the default user agent advertises Electron, which Google sign-in hard-refuses. A silent-clobber hazard shaped the fix: `setWindowOpenHandler` is last-writer-wins, and the first parity attempt was silently overwritten by the later legacy handler — diagnosed only via a permanent route log.
- **Decision**: `lite/main-window/browser-parity.ts` + `attachChromeParity(view.webContents)` in `attachTab`, the ONE place window-open policy lives (a loud comment forbids setting another handler after it). Behavior table mirrors what Chrome actually does, pinned by pure unit-tested functions: **sized `window.open`** (a features string) → in-app popup window in the SAME partition (opener/postMessage/cookies work; requested size honored, clamped 320..1600) with full ADR-038 hardening (contextIsolation+sandbox+no-preload+plugins); **featureless opens** (target=_blank anchors, cmd+click, plain window.open) → app TABS — disposition deliberately untrusted; **OAuth-allowlisted URLs** → popup even featureless (a tab can't give the IdP `window.opener`); **mailto/tel/sms/facetime/webcal** → the OS; **arbitrary custom schemes** → blocked + logged (an app-launch primitive third-party pages don't get; Chrome prompts, we decline). Popups recurse the full parity via `did-create-window`. Session-scope (WeakSet-idempotent): Chrome-shaped UA (Electron/app tokens stripped, real Chromium version kept — the auth window's ADR-041 approach generalized) + `downloads.attachToSession`. Per-contents: Chrome-shaped right-click menu (nav with history-aware enablement, link open-in-new-tab/copy, image copy, edit roles by editability/selection). `plugins: true` on tab views for the built-in PDF viewer. Every route decision logs one info line (`window.open routed`) — the support tool that caught both live-drive bugs.
- **Rejected alternatives**: keeping the OS-browser else-branch (the complaint itself); trusting `disposition` (measured lying risk + the legacy handler's same-site heuristic mis-popping anchors); `allowpopups`/unmanaged child windows (no partition/hardening control); shelling arbitrary custom schemes (Chrome prompts; we have no prompt surface — deny+log until one exists); a preload-injected `window.prompt` polyfill (ADR-038 forbids preloads in third-party tabs; `prompt()` stays a known gap).
- **Meta-test**: `main-window-browser-parity.test.ts` (14: route table incl. the anchor-as-scripted-open regression, UA shape, popup hardening incl. no-preload, size clamps, handler seams, context-menu table). Live-verified in the dev app via a local harness page in a REAL tab: Chrome-shaped UA on-page, `window.open` popup with `window.opener` SET, target=_blank landing as an app tab (route log `route:tab`), mailto → OS default, right-click menu with correct enablement. Downloads/PDF wiring unit-covered.

## ADR-064: Presence -- "who is doing what" as a TTL'd graph signal pointing at temporal logs

- **Context**: user vision: a user's schema should reflect what they currently have open — which tools, which WISER playbook, which Space, whether they're in an IDW / a meeting / the builder — feeding a Live Activity page in the main app showing who is doing what and when they last did it; TTL-based so a stuck client can't look present forever; and (user's refinement mid-build) "it should be temporal, so maybe the graph just points to logs."
- **Decision**: the MeetingLive doctrine generalized, split across three stores by role. (1) GRAPH = the NOW-snapshot: one `(:Presence {id: presence_<personId>_<appId>})` per user+app, MERGE-based, carrying facet properties (tool, spaceId/Name, playbookId/Title, meetingRoom, idwName, lastAction/lastActionAt), `lastSeenAt` heartbeats, and a `kv_collection`/`kv_ref` POINTER; `PRESENCE_OF -> Person` for graph shape — with the hard rule that a presence write NEVER sets properties on the `:Person` (ADR-060 recency must not see heartbeats; pinned by test). NOT an `:Asset`, never a Space member — nothing else renders it. TTL on read (active < 3m, idle < 30m, invisible after), sweep on write (> 24h deleted): stuck clients self-erase. (2) KV = the TEMPORAL trail the graph points at: `presence:logs / log:<personId>_<appId>` holding `{entries: [{at, ...facetPatch}]}`, appended ONLY on meaningful beats (facet changes / actions — bare heartbeats never touch KV), capped 200 entries / 24h so it self-prunes. (3) COMMITS = the durable what-happened log that already exists; the Live Activity page merges presence (now) + the Commit rail (history) and lazily loads a beacon's KV trail on expand. Writers: `lib/presence-beacon.js` (shared, reuses the meeting bridge's proxy transport; registry entry `(:Schema {entity:'Presence'})` self-registers) with main-app wiring (60s unref heartbeat, meeting facets from the recorder hooks, `userEmail`-gated identity) + a Lite beacon in `lite/spaces/main.ts` (ZERO new timers — heartbeats ride the same cache-refresh stream as the ADR-062 ring; facets on Spaces open, meeting-tab join, item create/edit; identity = the ADR-051 viewerId convention, email else accountId — the email-only first draft silently produced no beacon on this very install). Identity is never manufactured: no id, no beacon. Every write is never-throws, and failures LOG (a silent catch cost a debug round on day one).
- **Rejected alternatives**: facet history on the graph node or per-beat event nodes (unbounded churn in the graph — temporal data belongs in logs, the user's exact call); a websocket/push presence service (the complexity this whole design line avoids); stamping activity onto the `:Person` (poisons cross-writer recency); Lite-side polling timers (the no-new-loops doctrine holds).
- **Meta-test**: `test/unit/presence-beacon.test.js` (11: MERGE shape + sweep + pointer, the Person-untouched rule, facet allow-list + lastActionAt stamping, null-clears, heartbeat-vs-meaningful KV behavior, log append/prune/cap, newest-first reads, TTL classification, identity gating, never-throws). LIVE-VERIFIED cross-app: the dev Lite build beaconed on Spaces open -> the real graph held the beacon (+PRESENCE_OF) -> the dev MAIN app's new Tools -> Live Activity page showed it (green active dot, "Onereach.ai Lite - just now", tool: spaces, Show-trail) beside the Recent Activity rail correctly carrying the real ADR-061 meeting commit from an hour earlier.

## ADR-069: Attention-tiered Spaces sidebar -- the menu is a working set, not an inventory

- **Context**: user concern: "spaces will become so plentiful that a menu will be unmanageable. how do we deal with that in our minds?" Evidence already live: machine-minted id-named spaces ("Space a7671f80") sitting at equal sidebar rank with real work. The agreed frame (Cap Chew creed applied to navigation): the GRAPH is the inventory; the sidebar shows what is WARM (data speaks), keeps what the human MARKED (human leaves marks), folds the dormant tail, and search reaches everything -- past a certain scale humans don't scan menus, they type three letters.
- **Decision**: four mechanisms on the existing ADR-060/065 substrate, zero new data model beyond one edge. (1) PINNED tier: per-viewer `(:Person {id:$viewerId})-[:PINNED {pinnedAt}]->(:Space)` edge (the VIEWED/HAS_ACCESS idiom) -- cross-device, identity-true, MATCH-only on the Person (no bare-node minting), toggled from the space context menu, surfaced as a sidebar section that stays hidden until the first pin. `LIST_SPACES` returns `pinned` per row. (2) ACTIVE working set + More… drawer: the Spaces section defaults to `sortMode: 'recent'` (ADR-060 lastActivityMs -- activity is the resting order; A→Z stays one select away) and folds past 8 rows behind a "More… (n)" row (never folding a tail smaller than 2; session-sticky expansion; drawer state and pin flags join the render fingerprint so toggles repaint). (3) ⌘K jump palette: portaled overlay (z 2800 -- above menus/toast, below the identity gate) filtering all belonging spaces; ranking = pinned first, then activity, prefix-beats-substring; Enter jumps via the existing scope activation. (4) Rename nudge: `looksMachineNamed()` (empty / "Space <hex>" / uuid / bare-hex -- false positives are the failure mode, so the detector is deliberately narrow) puts a quiet dashed "name?" pill on the row that opens the existing inline rename.
- **Rejected alternatives**: hierarchical space folders (organizing the inventory instead of shrinking the working set -- moves the unmanageability one level down); auto-archival state on the Space node (dormancy is a DISPLAY tier; a shared-graph writer must not flip another user's data lifecycle); pinning in localStorage (device-local marks contradict the one-identity model just shipped across desktop/phone/watch/web); weighting others' activity equal to the viewer's own (deferred: a busy teammate could colonize the sidebar -- revisit with per-viewer touch weighting).
- **Meta-test**: `spaces-attention-tiers.test.ts` (15: PINNED cypher contract incl. MATCH-not-MERGE on Person + visibility gate + edge-only unpin; palette ranking incl. pinned-first, prefix-over-substring, cap-12; machine-name detector both directions; nudge in real row DOM; pin/unpin context-menu labels + handler firing; ADR-060 recent anchor). Existing renderer fixtures extended with the `togglePin` handler; CSS rides the structure guard.

## ADR-070: The NEON access standard -- one chokepoint per surface, every query wears its caller tag

- **Context**: the 2026-08-17 Lambda-concurrency incident ("something is calling the neon instant like crazy!!") was undiagnosable from the graph side: SHOW TRANSACTIONS and the Aura query log showed anonymous Cypher from six-plus surfaces (Lite, full app x3 transports, two web apps, two iOS clients, the flow-builder pipeline and its DEPLOYED flows), and the caller census had to be reconstructed from repo archaeology under incident pressure. User directive: "make sure nothing else calls NEON. Lets centralize all NEON calls. Even with web apps."
- **Decision**: centralization by CONTRACT, not by a shared package (the repos are separate; the ecosystem's proven pattern for cross-repo lockstep is IdentityKit -- one contract, per-surface implementations, pinned by tests). The standard, applied at every surface's single chokepoint: (1) ONE chokepoint per surface -- every graph call routes through it, bypasses are defects; (2) every Cypher leaves the process wearing `/* caller:<tag> */` as its first line -- legal Cypher, proxied untouched, visible verbatim in SHOW TRANSACTIONS and the Aura query log, so the next storm names itself in one look (tags: onereach-lite, onereach-desktop-omnigraph, wiser-meeting-bridge [+ opts.caller override for transport sharers], wiser-playbooks-web, gpsfl-web, or-mobile-ios, chorequest-ios, flow-pipeline-register, flow/<step-name> for deployed flow logic); (3) chokepoints carry timeout + bounded retry + failure backoff or breaker (the incident's client-side lessons: watcher backoff, cache outage backoff, harness circuit breaker); (4) already-tagged queries pass through unchanged (idempotent prefix), so tags survive transport nesting.
- **Rejected alternatives**: a literal shared npm package (build-infra project across five repos; the contract gets the observability TODAY); tagging via HTTP headers or body fields (the flow forwards only cypher/parameters -- the comment rides the one channel that reaches the database); a new central proxy service (the neon flow already is the choke; the missing piece was attribution, not another hop); KV-call tagging (KV has no comment channel -- deferred to a flow-side change, noted as a gap).
- **Known seams**: deployed Edison flow logic (knowledge-twin library/steps/**) is tagged in SOURCE but runs untagged until each flow is redeployed; the Aura password itself is still baked everywhere, so any holder can bolt around every chokepoint -- rotation (punch-listed) is the real fence; the full app's omnigraph has a direct-bolt path (_executeDirect) which now tags but predates the proxy-only doctrine.
- **Meta-test**: lite/test/unit/neon-caller-tag.test.ts pins the Lite chokepoint's tag (prefix on the wire, idempotence, config override); web-app builds verified; bridge wire-contract tests (26) green with the tag.
- **Addendum (2026-08-17, same day): neon2/keyvalue2 migration + the KV half of the standard.** The platform moved both flow paths -- `omnidata/neon` → `omnidata/neon2` and `/keyvalue` → `/keyvalue2` -- and the old paths 404 (`no handler`). Every constant across all six repos repointed the same day (probe-verified: neon2 answers the same `{result:{status,records}}` wire; keyvalue2 the same GET/PUT/POST-list wire). Two consequences worth remembering: (1) neon2 holds the graph credential SERVER-SIDE -- `neo4jPassword` in the request body is now optional (verified live), which let `web/live-activity.html` drop the baked Aura password it had been shipping to a public URL; sending a password still works (back-compat) but new code should stop; (2) the KV half of the standard: KV gets the SAME one-chokepoint-per-surface treatment as Cypher, minus the tag (no comment channel). Full app: `lib/kv-client.js` -- all three URL shapes (shared account / explicit account / refresh_token-derived session account) plus transport (15s abort-timeout, one retry on 5xx or network error, never on 4xx), with the error contract "missing → null, failure → THROW" (the old any-failure→null pattern let a KV hiccup publish empty blobs over good data). Routed: presence-beacon, gps-for-life/ics-sync, capture-signaling, recorder (room payloads), tickets-client (keeps its FLOW-token wire, rides the transport). Lite already conforms (lite/kv). riff and knowledge-twin KV consolidation DEFERRED deliberately -- their KV-owning files carry another session's uncommitted in-flight work; consolidating under it would collide. Deployed flow steps keep inline KV by design (Lambda-bundled, no shared module) -- the contract for them is timeout + non-fatal handling, not a shared client. Meta-test: `test/unit/kv-client.test.js` (12: URL builders, unwrap, missing-vs-failure contract, retry semantics, wire shapes).

## ADR-071: Exclusive expansion -- selection is the expansion; one open space, ever

- **Context**: user: "The spaces manager spaces menu is so busy" — and, on the first proposed cure (ADR-069-style occlusion via opacity tiers): "My eye cant distinguish between them maybe they only expand when clicked and colapse when others are clicked." Since 2026-08-08 every space tree rendered OPEN by default and Recent duplicated a second expanded explorer — the sidebar was N simultaneous trees plus per-row chevrons, counts, AI/lock badges, and nudge pills, all lit at once. The failed intermediate matters: gradations of light (dimming tiers) were indistinguishable to the eye. Hierarchy must be STRUCTURAL, not tonal.
- **Decision**: at most ONE space tree is open, ever (`expandedSpaceId: string | null`, resting state null). Clicking a space activates its scope AND opens its tree; whichever was open closes in the same motion (`syncExclusiveExpansion` in `setActiveScope`; chevron `toggleSpaceTree` keeps the no-scope-change variant with the same exclusivity). Non-space scopes (Home, Uncategorized) close everything. Recent becomes a FLAT jump list — no chevrons, no trees (the tree lives solely in the Spaces section). Rows rest as name-only lines: chevron, count, AI badge, lock pill, and the name? nudge STAY IN THE DOM (tests and screen readers see them) but surface via CSS only on :hover, :focus-visible, .is-active, or .is-expanded. All expansion mutations stay in-place (outside the render fingerprint, per the 2026-08-08 perf doctrine); a repaint reproduces the single open tree from `expandedSpaceId`. Bonus: with trees resting closed, the boot-burst lazy hydration (ADR at 2026-08-17) now fires ~zero tree fetches at launch. ADR-069's data model (PINNED edge, activity ordering, ⌘K palette, More… fold) is untouched.
- **Rejected alternatives**: opacity/dimming tiers (tried first — "my eye cant distinguish between them"; light levels are decoration, open-vs-closed is information); multiple simultaneous trees with smarter folding (the busy-ness WAS the simultaneity); removing the resting-row affordances from the DOM instead of CSS (breaks the pinned renderer contracts — count span must exist even when empty — and screen-reader discovery); auto-expanding on hover (accidental churn while scanning).
- **Meta-test**: spaces-renderer-integration 'sidebar explorer trees' rewritten to the exclusive contract (rest-collapsed; open A then B collapses A; re-toggle closes; source pin `if (expandedSpaceId === space.id)` + `syncExclusiveExpansion(scopeId)`); spaces-review-fixes pins Recent-is-flat (no buildSpaceChildren/chevron in renderRecentSpaces); chevron-stays-firstElementChild and count-always-exists pins unchanged and green. Full lite suite 3495/3495.

## ADR-073: One current playbook per space — deferred plurality, cheap switching

- **Date**: 2026-08-18
- **Status**: Accepted (with a written revisit trigger)
- **Question**: should a Space's current playbook become an array, so several plans can be current at once?
- **Decision**: NO for now. A Space keeps exactly ONE current playbook. Instead, make DESIGNATING one cheap: "Set current playbook…" now sits on every Space's right-click menu (asset picker: playbook-shaped first, current one marked, any kind promotable), alongside the existing detail-pane button.
- **Why**: the reported pain ("how does a user assign a current playbook?", and the Data Bricks space holding four WISER playbooks with none designated) was DISCOVERABILITY, not capacity — there was no cheap way to switch, so the singular model looked like a limit it wasn't. "Current" is a focus word: it answers "what are agents working on right now?". If five plans are current, the word stops carrying information and the concept collapses into "the playbooks in this Space", which the asset list already is.
- **What plurality would actually cost** (assessed, not guessed): the storage is an EDGE (`[:CURRENT_PLAYBOOK]`), so multiple is nearly free — stop deleting the old edge in SET_CURRENT_PLAYBOOK. Tickets ALREADY carry `playbookId`, so per-plan attribution exists. The real cost is semantic + UI: the shared dashboard renders one plan section, agents need a selection policy (priority? concurrent? round-robin?), and the empty state / onboarding copy assumes the singular.
- **Revisit trigger**: if, after living with one-click switching, users still want two plans live simultaneously, implement it as **"active playbooks"** (rename away from "current", which implies singular) with a ticket lane per playbook on the dashboard — NOT as a quietly pluralized field, which would leave "current" meaning nothing and agents without a policy.
- **Meta-test**: `spaces-renderer` "space context menu — every action is wired (coverage contract)" walks every action entry at every depth, fires each, and asserts it reached a handler — the gap that let Delete ship missing from this menu.

## ADR-074: Read-only members — a role on the grant edge, enforced by inventory

- **Date**: 2026-08-18
- **Status**: Accepted
- **Decision**: a member's standing rides `[:HAS_ACCESS]` as a `role` property (`'writer'` | `'reader'`), NOT a second edge type. `[:HAS_ACCESS]` continues to mean SIGHT; `role` decides whether the holder may also CHANGE. Absent role reads as `'writer'`, so every grant made before this ADR keeps working untouched. A Space's `createdBy` always writes, edge or not — otherwise marking yourself read-only would lock you out of your own Space with no way back.
- **Why a property, not an edge type**: every visibility predicate (SPACE_VISIBLE, ASSET_VISIBLE, OTHER_SPACE_VISIBLE, and their `other`/`vs` variants) matches on HAS_ACCESS. A second edge type would double each of them and create two places where a person's standing is recorded — the classic source of "revoked here, still allowed there".
- **Enforcement**: `SPACE_WRITABLE` / `ASSET_WRITABLE`, the write-side mirrors of the visibility predicates, in the WHERE clause of every mutation. Sight is deliberately not enough: a reader on the only Space holding an asset sees it and cannot touch it.
- **The inventory IS the feature**: `spaces-write-guard.test.ts` enumerates every mutating Cypher constant and fails on any carrying neither guard nor a written exemption. A permission enforced on 38 of 39 queries is a false sense of one. Exemptions carry reasons (pins are per-viewer preference; schema registry writes are documentation; `CREATE_ASSET_UNCATEGORIZED` has no Space and no node yet to guard).
- **Found en route (pre-existing, unrelated to roles)**: `UPDATE_ITEM`, `ADD_TAG` and `HARD_DELETE_ASSET` carried NO authorization whatsoever — any signed-in viewer could edit or hard-delete any asset by id. The inventory surfaced them; they are now guarded.
- **Schema**: `ENSURE_ACCESS_ROLE_SCHEMA` publishes the contract (edge shape + role values + the "absent = writer" default) into the graph's own `(:Schema {_RelationshipTypes})` registry at boot, per the ADR-055 pattern, so the full app / agents / future services discover it instead of reverse-engineering our Cypher.
- **Known gap (phase 2)**: `CREATE_AGENT_ENDPOINT{,_MERGED}` bind an `:Agent` by id with no Space in scope, so they are exempt today; the agent-asset creates that precede them ARE guarded. Gate them on the agent's owning Space once agents carry one reliably.
- **Still to build**: the UI to SET a member's role (the API and enforcement exist; the member row shows role but has no toggle yet).

## ADR-075: Keychain traffic control — quit drains keytar before Node teardown

- **Date**: 2026-08-20
- **Status**: Accepted
- **The crash**: report `Onereach.ai Lite-2026-08-20-112705.ips` — a Lite process quit while a keytar call was on the libuv threadpool; `node::Environment::RunCleanup()` drained the pool with the runtime half-dismantled; keytar's completion threw a C++ exception (`__cxa_throw` in `keytar.node`), which nothing above native code can catch → `std::terminate` → SIGABRT → the macOS "quit unexpectedly" dialog for an app that was exiting on purpose. Same native-throw family as the 2026-08-12 keytar SIGABRT in the logging incident, here in its teardown form.
- **Decision**: traffic control, not exception handling. `lite/keychain/api.ts` keeps a registry of in-flight keychain calls; every `require('keytar')` wraps the module in `trackKeychainBackend()` (a Proxy, so methods we never anticipated are covered too); `before-quit` AND `will-quit` in `main-lite.ts` consult `keychainQuitGate()`, which holds the quit (`event.preventDefault()`) while calls are pending, drains them (2s cap per round, 3 rounds total), then re-quits. Once will-quit passes its gate, `armKeychainFuse()` makes any LATER keychain call reject without reaching keytar — past the last checkpoint a new call would be exactly the in-flight native work whose completion aborts teardown, and "rejected and logged" beats "aborted the process and lost anyway".
- **What reproduction taught (the first cut was insufficient)**: SIGTERM-at-450ms against the dev build aborted 5/5 before the fix. A `process.once('SIGTERM')` handler turned out to be dead weight in steady state — once Chromium's browser main is up it owns the POSIX sigaction and routes TERM/INT through Electron's normal quit (before-quit/will-quit DO fire; verified by breadcrumb). The handlers stay only for the pre-Chromium-init sliver. A before-quit-only gate still aborted intermittently — calls issued after its pending==0 check reached keytar during teardown — hence the will-quit second checkpoint plus the fuse. After all three: 10/10 clean exits across kills at 0.45–2.5s, zero crash reports.
- **Reproduction gotcha**: a9f85c1 (same day, concurrent stream) defaults all three stores to an INERT backend in agent shells (`CLAUDECODE` set + unpackaged), so a kill-test run from a Claude shell exercises no keytar at all and passes vacuously — its boot line says `[LITE] keychain inert`. Force the real vault with `LITE_KEYCHAIN=1` when reproducing: the final proof here is 10/10 clean at the 450ms crash timing with the real backend confirmed per-run (no inert line). The drain tolerates the inert default trivially — an inert backend never has pending calls.
- **Quit breadcrumbs**: the gate writes one-liners to stderr (`[lite] quit: before-quit`, `…draining N keychain call(s)`) because the async file logger races a quitting process and loses — stderr is the shutdown record that survives.
- **Why a cap**: the logging incident showed securityd can wedge for 45s+. A quit that hangs is worse than a rare teardown abort, so past the cap we quit anyway and the residual risk is confined to "quitting while securityd is actively wedged".
...
## ADR-084: Space sight is explicit permission only — `OWNS` is membership, not access

- **Date**: 2026-09-02
- **Status**: Accepted. Supersedes the "Sight follows OWNS" clause of ADR-083; the rest of ADR-083 (private→restricted, constraints, registry annotations) stands.
- **What went wrong**: ADR-083 read the other writers' `(:Person)-[:OWNS]->(:Space)` edge as per-Space membership and let it grant sight. The live graph says otherwise: the Playbooks writer stamps `OWNS` on **every** Space for **every** account member — of 109 live Spaces with OWNS edges, robb holds 102, Rich 99, and Rich's edges were all created within seconds on the day he first signed in. So the rule made every member see every Space. It shipped in lite-v0.0.79. robb: "You can't give access to spaces to people who don't have permission… this has been an ongoing issue… all space access should be handled through the API."
- **Rule**: a Space (and everything in it) is visible to a viewer **only** when (1) the viewer created it — Lite stamps `createdBy`; the Playbooks writer stamps `created_by_user`; both mean creator — or (2) a live `[:HAS_ACCESS]` grant exists (ADR-065, roles per ADR-074). Nothing is inferred: not `OWNS`, not `visibility`, not account membership. The creator by either stamp may write; a grant writes unless `role = 'reader'`. Applied in SPACE_VISIBLE, OTHER_SPACE_VISIBLE, ASSET_VISIBLE, SPACE_WRITABLE, ASSET_WRITABLE and the live-meetings audience. `spaces-schema-audit.test.ts` fails the build if `[:OWNS]` reappears in any runtime query.
- **Consequence, stated plainly**: 93 of the 100 Playbooks-created Spaces carry `created_by_user = system@playbooks.app` and no grant — they are visible to nobody in Lite until someone with a real creator stamp or a grant exists. That is the correct outcome of "explicit only"; the fix for those Spaces is a grant (Add people…) or a real creator stamp from the writer that made them, never a wider predicate. Under this rule robb sees 24 Spaces, Rich 4.
- **The count**: the Spaces home no longer phrases the visible count as the account's total ("You can see all 114 Spaces in this account"); it says what it is — the Spaces you created or were given access to.
- **The structural fix (proposed, not done here)**: one access authority that every writer calls. Today Lite, WISER Playbooks and GSX-Desktop each write their own permission vocabulary straight into the graph and each re-derives a predicate; every widening to "the other app's signal" has leaked (ADR-051 open→everyone, ADR-083 OWNS→sight). Either a shared predicate package consumed by all apps, or a Spaces access service in front of the graph, with `HAS_ACCESS {role}` as the single grant edge and `createdBy` as the single creator stamp — and other writers stop minting `OWNS` as if it meant access.
- **OR Mobile**: mirrors `LIST_LIVE_MEETINGS`'s WHERE; it must drop the `OWNS` branch too (no mobile session was live to ping on 2026-09-02).
- **Addendum (0.0.82, same day)**: the `:Agent` catalog respects asset permissions. A Lite-built agent's name and description are derived from the asset that `[:REPRESENTS]` it, so the account-wide Home card and library search were describing agents built inside restricted Spaces to every member. `AGENT_VISIBLE`: a directory entry with no representing asset stays visible (the 20k+ platform / tracker-synced catalog); a represented agent is visible only when that asset is — the viewer's own uncategorized asset, or one in a Space the viewer can see. Live: robb 20,620 (his own "50/50 Risk Analyst" included), Rich and signed-out 20,619. `MEMBER_LIBRARY_SEARCH` stays account-wide (names are not asset content). The Spaces home counts are viewer-scoped too (0.0.81).

## ADR-085: Spaces inside one or more Spaces — permissions land on the inner Space; inheritance is a per-edge opt-in

- **Date**: 2026-09-02
- **Status**: Accepted. Extends ADR-084 with one new, explicitly requested signal; leaves ADR-084's two signals and its "nothing inferred" stance intact.
- **The ask (robb, verbatim)**: "add in NEON schema that option to support spaces inside one or more spaces. this would be an edge but permissions would land on the inside space for scoping default with the option to inherit top level space permissions." This is the explicit yes that ADR-084 and the standing rule ("never add a signal to the predicate without robb's explicit yes") require before a predicate widens.
- **Schema**: `(child:Space)-[:NESTED_IN {inheritsPermissions: boolean, createdAt, createdBy, updatedAt}]->(parent:Space)`. A Space may sit inside several parents (a DAG, not a tree). No self-edges; no cycles (the write refuses when the parent already sits below the child, checked eight hops deep). Registered for other writers in `(:Schema {entity: '_RelationshipTypes'}).lite_relationships` and in `Space.lite_sight_rule` (`ENSURE_LITE_SCHEMA_ANNOTATIONS`).
- **Sight and write**: by default the edge changes NOTHING about permission — the inner Space keeps ADR-084's two signals (creator by either stamp, or a live `HAS_ACCESS` grant). Only when the edge's `inheritsPermissions` is true does the parent's *explicit* standing flow down: sight through `NESTED_SIGHT`, write through `NESTED_WRITE` (a `reader` on the parent stays a reader on the child). Chains inherit only through consecutive opted-in edges, bounded to six hops; a deleted Space anywhere on the chain breaks it. Applied in `SPACE_VISIBLE`, `OTHER_SPACE_VISIBLE`, `ASSET_VISIBLE`, `SPACE_WRITABLE`, `ASSET_WRITABLE` (so everything that reads through the macros — the agent catalog, the home counts, item reads — follows automatically). Deliberately NOT applied to `LIST_LIVE_MEETINGS`: OR Mobile mirrors that WHERE and cannot be changed from here.
- **Who may do what**: nesting a child under a parent is a write to BOTH Spaces (you change what the parent holds and how the child is exposed) — `NEST_SPACE` carries the write guard on both ends, in the shape 0.0.83 set for add/move. Either Space's writer may unnest. Only a writer of the CHILD may turn inheritance on or off: exposure is the inner Space's call.
- **Surfaces**: `nestSpace` / `unnestSpace` / `setNestInheritance` / `listChildSpaces` / `listParentSpaces` on `SpacesApi`, IPC `lite:spaces:{nest,unnest,nestInheritance,listChildSpaces,listParentSpaces}`, the renderer's "Put inside a Space…" picker (with the inherit checkbox, off by default) and the Inside / Contains chip row on the Space header (unnest ×, inherit toggle on the parent chips). Listings gate both ends: a child you cannot see is not named, and vice versa.
- **Tests**: `spaces-schema-audit.test.ts` pins that every predicate carries the opt-in branch, that the branch requires `inheritsPermissions = true` on every edge, is bounded, breaks on a deleted Space, evaluates the parent by the same two explicit signals, and never mentions OWNS; `spaces-write-guard.test.ts` names the three mutations; `spaces-visibility.test.ts` gates the two listings; `spaces-nesting.test.ts` pins the queries and the client contract.
- **TTL (same day, robb: "make sure the schema and graph supports TTL on space permissions, also the UI when sharing")**: member grants already carry `HAS_ACCESS.expiresUnixMs` (ADR-052) — every predicate, the nested branches included, treats a lapsed grant as absent, and the sharing UI offers the same four presets (24 h / 7 d / 30 d / 90 d, or no limit) when adding a person and when changing a member's access. The inherit opt-in gets the same treatment: `NESTED_IN.inheritsUntilUnixMs` (epoch ms, optional) lapses inheritance when it passes — both nested predicates require it to be unset or in the future on every edge of the chain; the picker asks "For …" once the inherit box is ticked, turning inheritance on from a parent chip asks the same, and the chip reads "inherits · expires in 6d". A TTL without the opt-in is dropped.
- **Consequence, stated plainly**: with inheritance OFF (the default) nesting is pure organization — a person who can see the parent still sees nothing of the child unless they were given access to the child. With it ON, the child's writer has explicitly chosen to trust the parent's grant list, including future additions to it. There is no "inherit by default" mode and none should be added without another explicit yes.

## ADR-086: The Agent Registry — a search and registry manager over NEON, with an admin role and a submission checklist

- **Date**: 2026-09-02
- **Status**: Accepted.
- **The ask (robb)**: "add a menu item in LITE it needs to be a full agent registry, you can find the checklist in there for submission. This is where an admin can fully manage the agents in the platform available and the IDW and skills and knowledge models they belong to. Its a search and registry manager. Connect to NEON to populate should look solid."
- **What the graph holds (discovered, not assumed)**: 20,669 `:Agent` nodes — the Playbooks writer's catalog (types simple / gsx / micro-ui / planning / role, categories analysis / social / export / writing / creative / utilities / transform, `enabled`, `deleted` tombstones on ~8,600) and 35 GSX-Desktop library agents (`builtin`, `keywords`, `capabilities`, `executionType`, `(:Library)-[:CONTAINS]->`, `(:Person)-[:ENABLED]->`); three seeded `:IDW` nodes; a `KnowledgeModel` entity declared in the `:Schema` registry with no nodes yet; no `Skill` label (skills are the `AgentEndpoint:Skill` reachability kind and `capabilities`); 46 checklists, none about submission. The registry's own `_Manifest` and the `Agent` contract ARE the submission checklist: required `id`/`name`, documented `description`/`type`/`status` (active | inactive | deprecated)/`version`, provenance on every write, and the declared relationships `USES_KNOWLEDGE`, `HAS_CAPABILITY`, `APPLIES_TO_IDW`, `REACHABLE_VIA`.
- **Surface**: IDW menu → "Agent Registry…" opens a window (`lite/registry/`): facets (source, type, category, availability, listing, reachability, IDW, knowledge model), server-side search over name / description / keywords / category / id with paging, and a management panel for the selected agent — availability on the platform (`enabled`/`active`), record fields, MCP / RESTful / Skill endpoints, what it belongs to (IDWs via `APPLIES_TO_IDW`, knowledge models via `USES_KNOWLEDGE`, skills via `HAS_CAPABILITY` plus declared capabilities), where it is used, and the submission checklist with listing actions. Reads are the account-wide directory (the same stance as the agent library search — inventory, not Space content).
- **Admin**: a registry admin is a `Person` whose `role` is admin or owner (case-insensitive). No one held it, so the registry offers a first-admin claim that works ONLY while the graph has no admin at all (`CLAIM_FIRST_ADMIN` is guarded by that absence); admins grant or revoke others (never themselves). Writes: an admin or the agent's own creator; listing on the platform and minting knowledge models / capabilities are admin-only. Every write stamps `_Manifest` provenance (`updated_by_app_id = onereach-lite`, app name, user, `updatedAt`) and Lite's additions are namespaced (`lite_listing`, `lite_listed_at`, `lite_listing_checks`).
...
## ADR-090: The Calendar shows the account's scheduled flows (2026-09-05)

**Problem.** GSX → Calendar opened `https://calendar.<env>.onereach.ai/`, a link copied from the full app's menu. The host does not exist in any environment (no DNS; Tickets is dead the same way), so the GSX window failed with GSX_NAVIGATION_FAILED every time.

**Decision.** Calendar is now Lite's own window (`lite/calendar/`): the signed-in account's scheduled flows on a month grid, a day pane, a detail pane, and "Open in GSX Designer" through Lite's contained GSX window. The GSX menu keeps the label and routes the click to it (`GsxMenuDeps.openCalendar`).

**What "scheduled" means, verified against the platform.** A flow is scheduled when its canvas carries the platform's **Schedule execution** step (template `5d352cdd-6524-47d9-b612-061349152ddc`). Its runtime code hands the platform trigger primitive `this.triggers.on({ schedule: [{ cron, timezone, start, end }], eventName })` exactly what the step's data carries: `scheduleEvents[].scheduleEventData` with `expressions` (Quartz cron: min hour dom month dow year, optionally seconds first), `timeZone`, `startExpression` / `endExpression`, `isReccuring`, `isRunAtActivation`. The Event Manager fires them, and it publishes no listing (every route wants an account-scoped path it does not expose). At activation the platform records the armed triggers on the **deployment**: `GET datahub/deployments?query={"dateDeactivated":0}` lists active deployments whose `data.triggers[].params` carry `name: timer/<flowId>/<stepId>/next`, `hasSchedule: true`, `timeoutTime` (the next fire recorded then). So the Calendar reads two things and shows both: the authored schedule from the flow definition, and whether the flow is active and armed from the deployments.

**How it reads GSX** (`datahub.ts`, plain fetch — no `@or-sdk` dependency in the kernel): the account token from `em.<env>.api.onereach.ai/http/<account>/refresh_token` (the same source the full app's SDK manager and the podscan tools use; cached, re-minted once on 401/403), the `data-hub-pg` service URL from discovery `api/v2?serviceName=data-hub-pg`, then `/bots?query={"isDeleted":false}`, `/flows?query={"botId":…,"isDeleted":false}` and `/deployments`. The newer `@or-sdk/base` in the main app rejects that token ("wrong keyId"); the older SDK the podscan tools resolve accepts it, and so does plain fetch with `Authorization: <token>` — which is what Lite sends.

**Evaluation** (`cron.ts`, pure, tested): 5–7 Quartz fields, lists, ranges, steps, month and weekday names, `L`, `n#k`; instants produced in the event's time zone with the two-pass offset method (a wall time that does not exist on a DST day is skipped); start/end bounds intersect the window; one-shot events fire at their start. Occurrences are capped (20,000) and the window at 400 days; the grid collapses a (flow, event) day into one chip with its count and a series description ("every 5 min, 00:00–23:55 (288 runs)").
...
7. *Grey means it will not fire (robb: "if it's not running it should just be grey").* Every scheduled run is drawn; colour = the flow is armed, plain grey = it is not. The authored END date is advisory because the platform ignores it for an armed flow (the _ReportingAdapters "5min" flow: window ended 2020-02-01, 13 executions in the hour checked); `expandOccurrences` honours the start, produces runs past the end flagged `pastWindow`, and the tooltip says so. Before this, that flow drew nothing on the grid while firing every five minutes. Cap raised to 60,000 occurrences per window (two 288-a-day series over a month ± 7 days).
8. *Rollup (robb: "roll up the agent flows, show a number", "the day square will need more space").* A month cell shows at most three flow lines — name, then `×count` or the single run's time, the event name only when a flow has several series that day — then `+N more · M runs`; cells are ≥ 118 px and the grid scrolls. The grid paints from the datahub first; Space events are drawn when the graph answers. Reason: on 2026-09-06 the NEON endpoint hung for every query (`RETURN 1` = 29 s; the app's other graph spans failed the same way) and the old `Promise.all` held the whole grid for 29 s. Bisecting the events query under that outage was misleading — query shape made no difference; the endpoint did.
9. *Live check (dev app, Dev profile, robb's "HTTP toolkit 1.0.0" in Omni Data).* Filter and account list correct (3 scheduled, 2 armed, 1 not armed); links block answers in ~6 s; log summary in 9.75 s; the disarm → arm round trip is recorded in the session summary with its outcome.
...
## ADR-096: Google sign-in in Lite — FedCM off, plain-Chrome popups, the product token on the opener, Chrome parity on the Home view (2026-09-06)
...
**Root cause 3 — the Home view's popup policy (found on robb's second try, same evening).** The Home tab is not an ordinary tab: `attachRemoteHome` had its own UA lines plus the OAuth-allowlist popup handler (`buildPopupHandler`). The OneReach login page's own Google path is a `window.open` of `sso.global.api.onereach.ai` — not an identity-provider host — so that handler logged `oauth-popup: routed to OS default browser` and handed the page a null window: the button span, Chrome (the OS browser) quietly received the sign-in page. "It works always in Chrome" was literally true. The Home view now gets `attachChromeParity`, the same routing tabs have: a featureful `window.open` is an in-app popup in the Home partition.

**Root cause 4 — the frame.** The Home IDW page embeds the login page in an iframe. In a frame the login page does not run One Tap itself; it posts `GOOGLE_LOGIN_PROMPT_OPEN` to its parent and waits for the parent's answer, which never came (the standalone login tab, where cause 1 was measured, is top-level). The page checks `/onereach/i` on the user agent BEFORE the frame check and, when it matches, opens its SSO popup directly. So the tab and sign-in-window UA now end in ` OnereachDesktop` (`PRODUCT_UA_TOKEN`); popups and the app fallback stay plain Chrome, which is what Google's own pages see. This reverses the first cut's rejection of a UA token; the frame case is why.

**Verified in the dev app (Dev profile, trusted mouse click into the framed button on the Home tab):** SSO popup within ~4 s, `accounts.google.com/v3/signin/identifier`, "Sign in - Google Accounts", popup UA plain Chrome. A synthetic `element.click()` from a script did not open the popup from the frame; a real click did. The return leg after Google is robb's to try.

**Cause 5 — Google's passkey step (robb: "Google says I can access using my passkey, but it does not work").** Once the popup reached Google, Google offered the account's passkey because the page saw WebAuthn support; the ceremony cannot finish here: the passkey lives in iCloud Keychain / a Chrome profile, and Electron reaches neither (Chromium's iCloud Keychain provider sits behind Apple's browser entitlement `com.apple.developer.web-browser.public-key-credential`, granted to browsers). The app's own Touch ID authenticator (ADR-066) holds only passkeys enrolled inside the app, and an unsigned dev build has none (`isUserVerifyingPlatformAuthenticatorAvailable()` → false). `auth/passkey-popup.ts`: every sign-in popup registers, through the DevTools channel (`Page.enable` + `Page.addScriptToEvaluateOnNewDocument` — without `Page.enable` the script registers and never runs, measured), a script that removes `PublicKeyCredential` and rejects `navigator.credentials.get/create` on google.com documents only, before any page script. Google then goes straight to its other methods. Verified: first and later documents in the popup see `typeof PublicKeyCredential === 'undefined'`. A USB security key would still work in principle (Chromium's cross-platform WebAuthn is intact outside google.com popups); nothing else passkey-shaped can, until Electron gains an iCloud Keychain bridge.

**Review fixes (0.0.91 focused review of aa139fa + a5738ea, same day).** (1) The sign-in window's partition header rewriter wrote the window's UA — token included — onto every request in `persist:lite-auth-<env>`, the SSO popup's included, so Google received the token on the wire while the popup's page said plain Chrome; the rewriter now writes plain Chrome, the window's `navigator.userAgent` keeps the token (the login page's check is client-side). (2) Tabs presented the token to every site; on the wire it now reaches *.onereach.ai only (`stripProductTokenForHost`, one `onBeforeSendHeaders` per tab session, the session default UA plain) — page-side JS on third-party sites still reads the token, documented as a fingerprint, not a blocker. (3) `PRODUCT_UA_TOKEN` lives in `product.ts`; both UA builders import it and the parity test pins the tab UA to it. (4) The passkey shim runs only in popups opened FROM a *.onereach.ai page and only on `accounts.google.com` documents, with the standard NotSupportedError wording; a popup whose first document is already accounts.google.com can commit before the script registers (worst case: the old passkey prompt once). (5) The window.open route log carries the origin, never a query string. Follow-up, not in this cut (punch list 12): a Google sign-in completed on the Home tab never refreshes the app's stored session (`AuthStore` watches only the sign-in partition), so the next IDW tab injects the stale token and may bounce to a second sign-in in the sign-in window.

**Rejected.** Allowing One Tap by another route (Electron has no FedCM to allow); patching the page; a per-contents override at creation (measured ineffective for window.open children); navigating the Home tab to the login page top-level to dodge the frame (works only until the IDW changes its wrapper; the page's own in-app path is the contract).

**Not done here.** The installed app is 0.0.90; this ships with the next release (release rule: full delta review first).

- **Amendment (2026-09-06, same day — from the pre-release review):** the bundle folder and the executable do NOT take the new name. On macOS the update is applied by `scripts/install-update.sh` packaged inside the app that is already installed, and every helper shipped through 0.0.90 finds the new bundle inside the downloaded zip by the installed folder's basename (`BUNDLE_NAME=$(basename "$APP_PATH")` → `$EXTRACT_DIR/$BUNDLE_NAME`). A zip whose root is `Onereach Desktop.app` fails all of them at `find_bundle` — verified against the built zip. So electron-builder `productName` stays `Onereach.ai Lite` (folder, executable, zip root), and the name people see rides on `CFBundleDisplayName` (Dock, About) and on a localized `CFBundleName` in `lite/build/InfoPlist.strings` (the menu-bar title — measured: AppKit reads the localized name, not CFBundleDisplayName). The raw Info.plist `CFBundleName` must stay the internal name: Electron finds its helper apps as "<CFBundleName> Helper.app", and overriding it made the packaged app die at launch ("Unable to find helper app"); the DMG title and the artifact prefix say Desktop. Finder still shows the file name. The helper now accepts a lone `*.app` at the zip root, so a folder rename becomes possible once no install older than 0.0.91 remains; `APP_BUNDLE_NAMES` has one entry until then. The earlier "Squirrel swaps contents" reasoning was wrong for Lite: Lite bypasses Squirrel on macOS.

## ADR-098: Every asset kind the full app knows, from one registry (2026-09-09)

**Problem (robb: "add asset needs more asset types, like tools, video, slide, all the types from the main app should be supported. This is a lot of work particularly edit and meta data. But all needed.").** Lite's Spaces held thirteen kinds and its Add-asset dialog offered five tabs; the full Onereach app's Spaces present about twenty effective types (its storage enum `text|html|image|code|file|url|web-monitor|data-source|journey` plus the derived `presentation|spreadsheet|data|design|flow|notebook|style-guide|journey-map|chatbot-conversation|pdf|video|audio|screenshot|meeting|transcript|generated-document`). A deck, a Figma file, an MCP server, a notebook, a brand's tokens all landed in Lite as `document` or `other`, with no fields of their own and no way to edit what they were. Nine places in Lite answered "what kinds exist" independently — the union, a glyph switch, a label map, the reclassify list, a media predicate, a MIME sniffer, and three renderer switches — and the SDK kept a tenth, hand-copied `Set`.

**Decision.** One registry, `lite/spaces/asset-kinds.ts` (pure; imported by main, the SDK, the renderer and the tests): a row per kind with its label, glyph, accent, family, one-line hint, creation modes (paste / upload / link / form, or a hand-built pane), typed metadata fields (key, label, control type, options, required / read-only), file recognition (extensions, MIME prefixes, exact MIMEs), read-side aliases, and the empty-state copy. `ITEM_KINDS` grows by eleven — `tool, presentation, code, data, design, flow, notebook, styleguide, conversation, meeting, monitor` — and every surface derives from the table: glyphs, labels ("Slides", "Web link", "File"), the reclassify dropdown, media-first tiles, upload inference (`inferKindFromFile`: name pattern → extension → MIME prefix → exact MIME → `text/*` is a document), the LIST projection's content-head kinds, the SDK's schema annotation. A test pins the registry to the union, the bridge typings, and the annotation, so a kind cannot be added on one side of the bridge alone.

**Reading other writers' words.** `normalizeKind` maps the full app's `assetType` / `fileCategory` / `jsonSubtype` vocabulary through the aliases (`presentation`, `data-source`→tool, `web-monitor`→monitor, `style-guide`, `chatbot-conversation`, `html`/`pdf`/`generated-document`→document, `spreadsheet`→data, `screenshot`→image, `recording`→video…); WISER's second labels (`:Meeting`, `:Transcript`, `:Recording`) map in the member-kind CASE. Lite still writes `a.type` with its own ids; the alias table is read-side only.

**Creation — the picker.** The dialog's five tabs became two steps. Step one is a filterable, keyboard-walkable picker of every creatable kind grouped by family (Write / Files / Links / Capabilities / Structured, plus "already in your account" for the existing-asset search). Step two is the kind's form under a bar naming the pick, with Change. The five hand-built panes (text, upload for document/file, agent, knowledge, existing) keep their ids and behaviour; every other kind gets a generated pane from its registry row: a Paste body, an Upload dropzone (adopting the private/public/expiry controls), a Link input, or the typed fields. A dropped file skips the picker — its kind is read from the file — and a pasted link or body that looks like another kind ("Looks like a deck — file it as Slides") offers the switch without losing what was typed. Optional details fold under a disclosure so the common path stays a title and a body. Bodies imply metadata on the way in (`deriveBodyMetadata`: lines and language, rows and headers, cells and kernel, colours and fonts, turns and provider, stages, steps) so the tile has something to say before any enrichment runs. `link-embeds.ts` recognises YouTube, Vimeo, Loom, Google Slides/Docs/Sheets, Keynote, Canva, Pitch, Figma, Sketch, XD, GitHub, GSX Designer flow links and direct media files: the kind they imply and the embed form each publishes. Uploads ride the one upload pipeline with a kind override and extra metadata; inline text uploads pick their kind by JSON shape, then extension.

**Edit and metadata — the Details section.** The detail rail gains a "<Kind> details" section above the raw Metadata table: the kind's typed fields, each shown formatted (option labels, Yes/No, "1h 05m", locale dates, comma lists) and edited in place with the control its type deserves — a select, a date picker, a duration parser that reads "45m" or "3:20" (bare numbers are minutes), a comma list. Read-only fields (dimensions, counts, provider facts) show but never edit. A blank save removes the key; an unreadable value shows an inline error and keeps the editor. Writes go through the same metadata callbacks the raw table uses; the raw table hides the keys the section owns. Link-based kinds (web link, linked video/audio/image, slides, design, flow, monitor, tool) get a Link row that edits `sourceUrl` — new in `items.update` (`ItemUpdatePatch.sourceUrl`, http(s) only, capped at `MAX_ITEM_SOURCE_URL_LENGTH`, empty clears; the SDK's UPDATE_ITEM sets it and never snapshots for it).

**Rendering.** `asset-previews.ts` paints tiles for the new kinds from what the list projection carries — code's first lines and language, a mini table from a CSV head, swatches from a token head, chat bubbles, a calendar block for a meeting, a stacked deck for slides, provider tiles for linked media — and the detail blocks embed what can be embedded: the YouTube nocookie player, Google Slides / Canva / Pitch / Figma frames, direct media players, a tool card (type, endpoint, operations, auth method, spec and docs links), a meeting card (when, attendees, recording, transcript), a monitor card (address, cadence, status; checks run in the full app), notebook cells through the renderer's markdown and code previews, style-guide swatch grids and type samples, conversation threads. Kinds whose block IS the content (code, data, notebook, style guide, conversation) get a Preview / Source toggle and the same "✎ Edit" textarea every Markdown body has. The Spaces page CSP now allows `frame-src https:` and `media-src https:` for those embeds; only URLs the link recogniser produced are ever framed, never a raw `sourceUrl`. The styles live in `asset-kinds.css`, shipped beside `spaces.css`.

**Verified.** Unit: the registry invariants, alias and file inference tables, link recognition, field format/parse/controls, the picker (cards, filter, arrows, Enter), generated panes and payloads per mode, body-derived metadata, the Details editor (select, duration, blank-removes, error-keeps, Link row, Escape), tile previews and detail blocks per kind, the asset matrix's coverage contract extended to all 24 kinds; the pre-existing Spaces suites updated only where they pinned the old labels, the old reclassify list, the drop handler's callee, and the four-kind content-head list. Live: see the release notes for this cut.
...
- **Amendment 2 (2026-09-09, correctness + UX lenses of the same review):** the SDK's content-head IN-list is derived from a `contentHead` flag on the registry row (a hand-copied list would have let a new inline-body kind ship without a head in five projections); `spaces-asset-kinds.test.ts` pins the SDK constant to the derivation. `asset-kinds.css` joined `THEMED_SHEETS`, and its bare literals went — the month band and poster overlay paint `--or-on-accent`, raised surfaces `--or-bg-raised`, the deck shadow `--or-shadow-sm`. Four glyphs that read alike at 12 px changed (Document ▯, Design ✎, Web monitor ◎, Conversation ◒) and glyph uniqueness is pinned. The four empty states that still named graph internals now say what to do ("Paste or upload a note and it appears here."), pinned by a test that bans `:Asset.` from that copy.
...
### ADR-100 Amendment 1 — the pre-cut review: linear scans, a worker with a kill timer, sanitised HTML (2026-09-09)

The release session's reviewer drove the built `convert-mcp.js` over real stdio and blocked the cut on one finding, with six more to fix in place.

**The blocker.** The tag and marker strippers were regexes shaped `<tag[\s\S]*?</tag>`, `<[^>]+>` and `\[([^\]]*)\]\(…\)`: when the opener repeats and the closer never comes, every start rescans to the end. Measured: 1 MB of `<script` took 24 s, 400 KB of `![` took 27 s, and 2.1 MB of `<script` through `convert_text` never answered — the server is one process per client, the work is synchronous, and a `Promise.race` cannot fire, so the tool was dead until node was killed. Three things fix it, each sufficient on its own:
1. *Linear scans.* `lite/convert/scan.ts` walks the text once — openers found with an indexOf cursor that only moves forward, and a closer that was not found once is not looked for again (`ForwardSearch`). `html-to-text`, `md-to-text`, the sanitiser and every other regex the audit found with the same shape (`titleFromMarkdown`'s `^\s*` crossing lines, `code-to-md`'s language tells, `IMPORT_PATTERN`'s lazy tail, `html-to-md`'s cell flattening, `md-to-jupyter`'s link tag) now cost what the input costs. `convert-hostile.test.ts` holds a 1.5 s budget for each shape the review measured, at the review's sizes; they run in tens of milliseconds.
2. *A worker with a kill timer.* The MCP server runs every conversion in a worker thread (`worker.ts` → `dist-lite/build/convert-worker.js`, beside the server, which refuses to start without it) with `CONVERT_TIMEOUT_MS` (default 20 s) and a 512 MB heap; over budget the worker is terminated and the caller gets `CONVERT_TIMEOUT`, out of memory it dies alone. Format, path and size are refused in-process. Inside the app the api stays in-process — the caps are its budget.
3. *Caps by engine.* 2 MB for the regex-engine renderers (marked, turndown, highlight.js, the text structurer), 8 MB for the linear strippers, the 25 MB service cap for the data parsers; a converter's cap applies to what reaches it, step by step, and names the converter and the step.

**The rest.** `md-to-html` output is sanitised by default (`sanitize-html.ts`: an allowlist pass, linear, byte-identical when nothing needs to go; `sanitize: false` keeps the raw render for Markdown you wrote yourself) — marked has no sanitiser and `<script>` reached a self-opening styled page. Entities decode in one pass, so `&amp;lt;script&amp;gt;` stays escaped. `py` is read by the code converters, `tsv` is written by `json-to-csv` and the new `delimited` (csv ↔ tsv without a retyping detour through JSON), and `convert_capabilities` says of every format whether anything reads or writes it. `mergeOptions` drops prototype-shaped keys. Options given to a multi-step pipeline say so in a warning, as a strategy does. The tool descriptions and README carry the caps, the budget and "untrusted in, untrusted out".

**Verified.** The full gate; `convert-worker.test.ts` bundles the real worker with esbuild and proves the chain (a hung worker becomes `CONVERT_TIMEOUT` inside its budget; codes cross the thread); the built server driven over stdio with the review's 2.1 MB input answers at once (the cap) and with 1.9 MB answers in well under a second, and a forced timeout leaves the server answering the next call.

**Inherited caveat, not fixed here.** `@modelcontextprotocol/sdk` resolves (1.29.0) but is not declared in package.json; a fresh `npm ci` would break `lite:build` for this entry and for spaces-mcp / local-api-mcp alike. Declaring it touches the lockfile in a shared tree; punch-listed for the next quiet moment.

### ADR-100 Amendment 2 — the second review pass: a length-stable fold, detection on a prefix, the timer first (2026-09-09)

The reviewer verified Amendment 1 over stdio (the blocker and findings 3–7 closed) and found one more blocker in the sanitiser, plus two smaller things.
- **The fold was not length-stable.** The scans and the sanitiser matched on `html.toLowerCase()` and sliced `html` with offsets found in the copy. U+0130 (İ) is the one code point whose lowercase is two UTF-16 units, so every İ moved the copy one unit past the original; after enough of them the drift swallowed a tag and it was emitted verbatim — a live `<script>` behind 25 İ, `stats.sanitized: true`; html-to-text leaked a script body the same way. `asciiLower` (scan.ts) folds A–Z only, one unit each, and is the only fold the scans and the sanitiser use for matching; `asciiLower(s).length === s.length` is pinned, and the İ shapes are in the hostile tests for the sanitiser, md-to-html (both strategies), every html-to-text strategy and each scan helper.
- **`code-to-html` ran out of memory inside its own cap.** highlight.js's automatic detection runs every registered grammar over the whole input and keeps enough of the results to exhaust the 512 MB worker heap from ~1.5 MB (deterministic at 2 MB, measured under `--max-old-space-size=512`; with the language given, 2 MB highlights in half a second). Detection now reads the first 64 KB and the whole input is highlighted once — 2 MB in 1.25 s under the same heap; the worker test converts at the declared cap with detection and passes.
- **The budget starts before the worker exists.** The kill timer used to be armed after `new Worker(…, { workerData })`, so the structured clone of a large input sat outside it. It is armed first; a synchronous failure to construct the worker is reported as CONVERT_FAILED. The JSON-RPC parse and schema check that precede our code are the transport's and are documented as unbudgeted.

## ADR-101: The update feed lives in code; no bundle ships without its descriptor (2026-09-09)

**Problem (robb: "I just got an error cannot check updates … updating in this app NEEDS to work always").** The app in /Applications reported 0.0.92 but was a fast `--dir` package made in the shared repo at 12:31 and copied in at 12:34. electron-builder writes `Contents/Resources/app-update.yml` — the file electron-updater reads for the feed — only when it produces a release target, never for `--dir`. The updater therefore failed every check with `ENOENT … app-update.yml`, on launch and on every Help → Check for Updates. Nothing in the pipeline or the app could notice: the published 0.0.92 zip has the file, the shipped 0.0.91 had it, and the release gate checks the asar, the entitlements and the boot, not the descriptor.

**Decision — three layers, any one of which would have prevented it.**
1. *The updater takes its feed from code.* `lite/updater/feed.ts` holds `LITE_UPDATE_FEED` (github, wilsr7000/Onereach_Lite_Desktop_App, cache dir `onereach-lite-updater`); `RELEASES_URL` derives from it. When the app is packaged, `initAutoUpdater` calls `setFeedURL` with those constants — the provider never depends on the bundle — and, when the bundle has no descriptor, writes one under userData from the same constants and points electron-updater at it, because the download step reads `updaterCacheDirName` from the descriptor. A warning names the missing file and the likely cause (a `--dir` build). Tests cover both paths and the unwritable case.
2. *The packager writes the descriptor into every bundle.* `lite/scripts/after-pack-update-descriptor.cjs` runs as electron-builder's `afterPack` hook — before signing, so the file is inside the code seal — and writes `app-update.yml` when it is absent, for mac (`Contents/Resources`) and win/linux (`resources`). A test runs the hook against a fake `--dir` output and pins its constants to the updater's and to the `publish` block.
3. *The release gate refuses a bundle without it.* `release-lite.sh` aborts when `app-update.yml` is missing or names another repo or cache dir, next to the entitlement tripwire and the asar sanity check.

**The installed app.** The descriptor was copied into the running install by hand (outside its signature seal — harmless for launching an unquarantined app, and the installer verifies the downloaded bundle, not the installed one); a restart is needed because electron-updater caches its first failed read for the life of the process. The next update replaces the bundle with a sealed release build that carries all three layers.

**Rejected.** Fixing only the install (the cause was the pipeline); making `--dir` builds unsigned so they cannot be installed (the fast build is the right tool for side-by-side dev testing — it just has to update like any other); deriving the cache dir name in the hook from the package name (it is part of the install handoff contract with `scripts/install-update.sh`, so it is pinned in one place and mirrored with a test).

### ADR-101 Amendment 1 — the pre-cut review, landed before 0.0.93 (2026-09-09)

The release session reviewed the first cut of this ADR (2b87f92) and held the 0.0.93 re-cut on nine findings. All nine are in; each one is a way the three layers could still have let a broken bundle through.

- **The gate is executed, not grepped.** `lite/scripts/check-update-descriptor.sh <bundle>` is the gate: it passes only when `Contents/Resources/app-update.yml` is exactly the four lines of `lite/updater/feed.ts` (anchored full-line comparison; comments, blank lines and trailing whitespace tolerated — a duplicated or extra key is not). `release-lite.sh` calls it, and when the entitlement tripwire's `find` came back empty it now looks again under `dist-lite` and aborts when nothing is found — an empty `APP_BUNDLE` used to skip the whole gate silently. `updater-feed-guards.test.ts` runs the script against fake bundles (exact, tolerated, missing, drifted, duplicated key, absent bundle) and pins the executable bit on both scripts: the first commit had dropped `release-lite.sh` to 100644.
- **The packaged feed defers to a dev update config.** `applyPackagedFeed` is skipped when `forceDevUpdateConfig` is true — `LITE_DEV_UPDATE_CONFIG` points a real bundle at a local server for the packaged e2e tier, and the production feed must not override it.
- **It never throws.** The whole packaged path runs inside one try; a failure degrades to `{ feedFromCode: false, descriptor: 'error' }` and one log line. A throw there would have taken the IPC handlers and the fallback menu down with it.
- **The bundle descriptor is read and compared, not trusted for existing.** Its provider, owner, repo and `updaterCacheDirName` must equal `LITE_UPDATE_FEED`; a bundle naming another feed (the `gsx-power-user-updater` drift class) gets the userData descriptor exactly as a missing one does (`reason: 'drift'`), and the updater is pointed at it.
- **Downloads re-establish the descriptor.** The userData copy is user-writable and electron-updater reads `updaterCacheDirName` from it at download time, so `downloadUpdate` is wrapped: when the descriptor is ours it is rewritten from code right before delegating; when the state is `unwritable` or `error` the download is refused with a legible error (it reaches the dialog through the lifecycle's error status), and the manual Check for Updates shows "This copy of the app cannot download updates" with Open Releases Page instead of a check that looks fine and fails at download time (`packagedFeedState()` in `init.ts`, read by `index.ts`).
- **The hook writes the pinned constants only, and drift fails the build.** An electron-builder `publish` block that disagrees with the pinned feed throws instead of propagating into the bundle; the app follows `feed.ts`, so a descriptor saying otherwise would be a lie. On win32 the hook writes `publisherName` from `win.publisherName` — that is what NsisUpdater verifies downloads against; without it verification is silently skipped — fails when a signing identity is configured without one, and warns plainly for an unsigned build.
- **One literal for the cache dir.** `install.ts` takes `updaterCacheDirName` from `LITE_UPDATE_FEED` instead of a second copy of the string.

**Verified.** Full gate green in a clean worktree (typecheck, dep-check, 4587 unit + 230 integration tests, 15 of them the packaged-feed behaviours above); a fast `--dir` package from the same tree: the hook line precedes signing, the check script passes the produced bundle, `codesign --verify --deep --strict` is clean, and the descriptor inside the seal is the four lines from `feed.ts`.

### ADR-101 Amendment 2 — the second review pass (2026-09-09)

The release session verified Amendment 1 finding by finding: eight closed, one partial, a few low residuals. All are in.
- **Windows signing predicate (the partial).** The hook now recognises every way app-builder-lib can be told to sign — `certificateFile`/`certificatePassword` in the config as well as the store-certificate options, and `CSC_KEY_PASSWORD`/`WIN_CSC_KEY_PASSWORD` alongside `CSC_LINK`/`WIN_CSC_LINK` — so a certificate-file build cannot ship without `publisherName`.
- **The refusal reaches the person, on every path.** Amendment 1 claimed the download refusal "reaches the dialog through the lifecycle's error status"; it did not — the lifecycle's dialog listens to the updater's `error` event, and a rejected `downloadUpdate` only logged and set a status; a renderer-driven check had no equivalent of the menu guard. Now `checkRunner.check()` takes a `preflight` (index.ts supplies it): an unusable feed refuses every check — menu, renderer IPC, periodic — before `checkForUpdates`, with the dialog when manual; and the download guard raises its refusal as the updater's `error` event as well as rejecting.
- **The `error` state refuses downloads.** The guard used to be installed inside the try, after the point that could fail, so `descriptor === 'error'` in it was unreachable. It is installed first; the packaged path only decides what it does.
- **No stale-bundle fallback.** `release-lite.sh` no longer looks for any `.app` under `dist-lite` when the build's own directory is empty — that could have gated a stale bundle while the release referred to the fresh one. An empty build directory is the failure.
- **Gate and runtime agree on extra keys.** `check-update-descriptor.sh` checks the four keys per line (`grep -x`-style anchoring: exactly one line for the key, exactly one with the value); other keys pass, as `descriptorMatchesFeed` already allowed, so the hook's own Windows output (`publisherName`) and a future channel do not fail the gate. A commented-out or prefixed value still does.
- **Dialogs catch.** The dev-build and refusal message boxes log a failed `.then()` instead of leaving it unhandled.

**Third pass (same day; the reviewer cleared Amendment 2 as safe to ship with four notes).** The download guard's installation sits inside its own try, so `applyPackagedFeed` never throws even for an updater without a callable `downloadUpdate` (descriptor `'error'`). The refusal text and the allow/refuse mapping moved into `init.ts` (`feedRefusalMessage`), so a test composes it with the check runner the way `index.ts` wires it: `'bundle'` and `'written'` run the check, `'unwritable'`/`'error'` refuse every caller, dev and pre-init allow. The hook's signing predicate counts certificate sources only — a password alone signs nothing, and `CSC_KEY_PASSWORD` is the macOS variable, so a Windows build in a mac-signing shell no longer trips on it. Every remaining dialog chain in `index.ts` and `lifecycle.ts` catches.

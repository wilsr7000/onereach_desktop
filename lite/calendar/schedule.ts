...
export function expandOccurrences(flows: ScheduledFlow[], fromMs: number, toMs: number, cap = 20000): { occurrences: Occurrence[]; truncated: boolean } {
  ...
  for (const f of flows) {
    for (const ev of f.events) {
      ...
      for (const expr of ev.cron) {
        ...
        for (const atMs of instants) out.push({ atMs, ...base });
      }
    }
  }

...
This is a sketch, not a commitment. Sequencing and scope will be revised as Phase 1-2 ships and we observe what users (and agents) actually need.

## Shipped

- Phase 0: Module foundation + window shell
- Phase 1: Spaces list in left sidebar
- Phase 2: Items as cards + detail panel + multi-Space chips

See [.cursor/plans/spaces-manager-phases_5ab75078.plan.md](../../.cursor/plans/spaces-manager-phases_5ab75078.plan.md) for the detailed phase breakdown.
...
- **Lite Spaces UI** (this plan) — the first consumer; validates the API surface end-to-end.
...
Phases below are likely directions but not committed. Names and sequencing may collapse or split based on what shipping Phase 1-2 reveals about agent output volume, governance pain points, and user behavior.

- **Approval + Audit queue**: surface agent output landing in Uncategorized; humans approve, reject, or quarantine. Different mental model from "file these notes" — this is "do you accept this work product into the canonical record."
- **Write paths**: create / rename / delete Space; manual Add to Space; remove from Space (for both human and agent-initiated writes).

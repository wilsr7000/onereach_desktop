...
test.describe('Smart Export', () => {
  ...
  test.afterAll(async () => {
    if (testSpaceId) {
      try { await deleteSpace(testSpaceId); } catch {}
    }
    await closeApp(app);
  });
  ...
  for (const format of EXPECTED_FORMATS) {
    test(`${format.toUpperCase()} export produces valid output`, async () => {
      ...
      const result = await mainWindow.evaluate(async ({ format, spaceId }) => {
        try {
          if (window.api?.invoke) {
  ...
  test('HTML export can be saved back to a space', async () => {
    ...
    const htmlContent = '<html><body><h1>Exported Document</h1><p>This was generated by Smart Export.</p></body></html>';
  ...
  test('smart export window closes on Escape key', async () => {
    ...
    const exportPage = windows.find(p => {
      try { return p.url().includes('smart-export') || p.url().includes('export'); } catch { return false; }
    });
    ...
    if (exportPage) {
      ...
      const stillOpen = afterWindows.find(p => {
        try { return p.url().includes('smart-export'); } catch { return false; }
      });
  ...
  test('no unexpected errors during smart export tests', async () => {
    ...
    if (genuine.length > 0) {
      console.log('Errors found:', genuine.map(e => e.message));
    }

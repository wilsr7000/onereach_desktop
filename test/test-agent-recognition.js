...
const { getBidsForTask, selectWinner, AGENT_CAPABILITIES } = require('../packages/agents/agent-bidder');
...
async function runTests() {
  ...
  if (apiKey) {
    ...
    await test('Decompose compound query (time + weather)', async () => {
      ...
      const hasWeatherOrSearch = result.tasks.some(t => 
        t.type === 'weather' || t.type === 'search' || t.type === 'clarify'
      );

...
(function () {
  ...
  async function checkAssets(videoPath, metadata) {
    ...
    const hasAccurateTranscript =
      accurateTranscriptSources.includes(metadata.transcriptSource) &&
    ...
    if (hasAccurateTranscript) {
      ...
      missing.push({
        ...
        description:
          'Upgrade to accurate ElevenLabs Scribe timestamps (current: ' +
          (metadata.transcriptSource || 'unknown') +
          ')',
      });
    } else {
    ...
    try {
      ...
      const _hasPreview = previewResult && previewResult.exists;
  ...
  function showModal(options) {
    return new Promise((resolve) => {
      ...
      const warningHtml =
        isLongOperation || isVeryLongVideo
          ? `
  ...
  async function generateAssets(options) {
    ...
    for (const assetType of orderedAssets) {
      ...
      try {
        if (assetType === 'transcript') {
          ...
          await generateWaveform(
            ...
            (progress) => {
              ...
              updateProgress(
                modal,
                `${asset.icon} Generating ${asset.name}...`,
                overallProgress,
        ...
        completedSteps++;
        updateProgress(modal, `${asset.icon} ${asset.name} complete!`, (completedSteps / totalSteps) * 100, '');
      } catch (error) {
   ...
   */
  async function generateSpectrogramImage(audioBuffer, _duration) {
   ...
   */
  async function generateBarsImage(peaks, _duration) {
    const width = 4000;
    const height = 80;
  ...
  async function checkAndPrompt(videoPath, metadata, callbacks = {}) {
    try {
      ...
      console.log(
        '[Preloader] Missing:',
        missing.map((a) => a.type)
      );
      console.log(
        '[Preloader] Existing:',
        existing.map((a) => a.type)
      );

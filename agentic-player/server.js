 ...
 */
async function aiSelectClips(clips, prompt, context, _settings) {
    ...
    req.on('end', async () => {
      try {
        ...
        if (prompt && settings.openaiApiKey) {
          const aiResult = await aiSelectClips(
            unwatchedClips,
            prompt,
            {
        ...
        if (selectedClips.length === 0) {
          ...
          reasoning = prompt ? `Clips matching: ${prompt}` : 'Playing next clips in sequence';
        }

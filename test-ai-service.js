...
function setupMockSettings() {
...

  {
    ...
    global.settingsManager = {
      ...
      getLLMApiKey() {
        return this._data.anthropicApiKey || this._data.llmApiKey;
      },
      getLLMProvider() {
        return this._data.llmProvider;
      },
      getLLMModel() {
        return 'gpt-4o-mini';
      },
      getAIModelProfiles() {
        return this._data.aiModelProfiles;
      },
...
function testMigrationCompleteness() {
  ...
  for (const file of agentFiles) {
    ...
    test(`Agent file ${file} imports ai-service`, () => {
      ...
      assert(
        content.includes("require('../../lib/ai-service')") || content.includes('require("../../lib/ai-service")'),
  ...
  for (const { path: filePath, importPath } of toolFiles) {
    ...
    test(`Tool file ${filePath} imports ai-service`, () => {
      ...
      assert(content.includes(importPath) || content.includes('ai-service'), `${filePath} should import ai-service`);
    });
  }
  ...
  test('No remaining direct OpenAI chat calls in agent files', () => {
    for (const file of agentFiles) {
      ...
      const content = fs.readFileSync(filePath, 'utf8');
      assert(!content.includes('api.openai.com/v1/chat/completions'), `${file} still has direct OpenAI chat call`);
    }
  });
  ...
  test('No remaining getOpenAIApiKey in agent files', () => {
    for (const file of agentFiles) {
      ...
      if (!fs.existsSync(filePath)) continue;
      const content = fs.readFileSync(filePath, 'utf8');
      assert(!content.includes('getOpenAIApiKey'), `${file} still has getOpenAIApiKey function`);
    }
  });
  ...
  test('main.js design handlers use ai-service (no direct DALL-E calls)', () => {
    ...
    assert(designSection.includes('ai.imageGenerate'), 'Design handlers should use ai.imageGenerate()');
  });
  ...
  test('src/video/index.js uses ai-service for transcription', () => {
    ...
    assert(content.includes("require('../../lib/ai-service')"), 'Should import ai-service');
    assert(!content.includes("hostname: 'api.openai.com'"), 'Should not have direct OpenAI API calls');
  });
  ...
  test('whisperSpeech.ts has deprecation warning on direct fetch fallback', () => {
    ...
    assert(content.includes('@deprecated'), 'Direct fetch fallback should be marked @deprecated');
    assert(content.includes('DEPRECATED'), 'Should have DEPRECATED warning in console.warn');
  });
}
...
async function testLiveAPIs() {
  ...
  if (hasOpenAI) {
    ...
    await testAsync('ai.json() returns parsed JSON (OpenAI)', async () => {
      ...
      const result = await service.json(
        'Return a JSON object with key "status" set to "ok" and key "number" set to 42. Return ONLY the JSON, no markdown.',
        {
          profile: 'fast',
          maxTokens: 50,
          temperature: 0,
  ...
  if (hasOpenAI) {
    await testAsync('ai.imageGenerate() generates an image (OpenAI)', async () => {
      ...
      console.log(
        `    Generated ${result.images.length} image(s), has revised_prompt: ${!!result.images[0].revised_prompt}`
      );
    });
  } else {

...
function mountConversionRoutes(app, conversionService) {
    ...
    try {
      ...
      if (!steps || !Array.isArray(steps) || steps.length === 0) {
        ...
        return res.status(400).json({
          ...
          message:
            'The "steps" field must be a non-empty array of conversion steps. Each step needs at least a "to" field.',
        });
      }
      ...
      log.info('app', 'POST /api/convert/pipeline', {
        stepCount: steps.length,
        pipeline: steps.map((s) => s.to).join(' -> '),
      });
  ...
  log.info('app', 'ConversionAPI routes mounted', {
    routes: [
      '/api/convert',

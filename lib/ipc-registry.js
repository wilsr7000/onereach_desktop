...
const IPC_CHANNELS = {
  ...
  'playbook:search': { type: 'handle', source: 'main.js', description: 'Search playbooks (title + content, ranked)' },
  'playbook:search-agentic': { type: 'handle', source: 'main.js', description: 'Agentic playbook search (LLM walks playbooks one by one)' },
  'playbook:jobs': { type: 'handle', source: 'main.js', description: 'List playbook jobs' },

  // --- Meetings ---
  'meetings:history': { type: 'handle', source: 'main.js', description: 'Meeting history timeline (meetings + transcript summaries)' },
  'meetings:open-history': { type: 'on', source: 'main.js', description: 'Open the Meeting History window' },
  ...
  'ai:pause': { type: 'handle', source: 'main.js', description: 'Pause all AI traffic (kill switch)' },
  'ai:resume': { type: 'handle', source: 'main.js', description: 'Resume AI traffic' },
  'ai:pause-status': { type: 'handle', source: 'main.js', description: 'Get AI pause status' },

  // --- Error Diagnostics (plain-English, copiable "What's wrong?") ---
  'diagnostics:diagnose': { type: 'handle', source: 'main.js', description: 'Diagnose an error and return a user-friendly recommendation (hint table -> Claude Code -> ai-service -> fallback)' },
  'diagnostics:get-recent-logs': { type: 'handle', source: 'main.js', description: 'Pull the recent log-queue slice for an error category' },

  // --- App Issue Agent (two-stage diagnose + propose-fix pipeline) ---
  'issue-agent:status': { type: 'handle', source: 'main.js', description: 'Availability + dev-mode + claude-code install status' },
  'issue-agent:report': { type: 'handle', source: 'main.js', description: 'reportIssue entry point: diagnose and optionally propose a fix patch' },
  'issue-agent:save-patch': { type: 'handle', source: 'main.js', description: 'Write a fix proposal patch to .cursor/patches/' },

  // --- Critical Meeting Alarms ---
  'critical-alarms:status': { type: 'handle', source: 'main.js', description: 'Upcoming scheduled alarms + active rule set' },
  'critical-alarms:snooze': { type: 'handle', source: 'main.js', description: 'Snooze all alarms for an event for N minutes' },
  'critical-alarms:dismiss': { type: 'handle', source: 'main.js', description: 'Cancel all pending alarms for an event' },
  'critical-alarms:reload-rules': { type: 'handle', source: 'main.js', description: 'Re-read and re-parse the agent memory rules file' },
  'critical-alarms:test': { type: 'handle', source: 'main.js', description: 'Fire a synthetic test alarm on all channels' },
  ...
  'action:execute': {
    type: 'handle',
    source: 'action-executor.js',
    description: 'Execute a registered action by ID with optional params',
  },
  'action:list': { type: 'handle', source: 'action-executor.js', description: 'List all actions grouped by category' },
  'action:has': { type: 'handle', source: 'action-executor.js', description: 'Check if an action ID is registered' },
  'action:info': {
    type: 'handle',
    source: 'action-executor.js',
    description: 'Get action metadata including param schema',
  },

  // --- Desktop Autopilot ---
  'desktop-autopilot:status': {
    type: 'handle',
    source: 'lib/desktop-autopilot.js',
    description: 'Get Desktop Autopilot status and capabilities',
  },
  'desktop-autopilot:browser-task': {
    type: 'handle',
    source: 'lib/desktop-autopilot.js',
    description: 'Run a natural-language browser task via browser-use',
  },
  'desktop-autopilot:browser-navigate': {
    type: 'handle',
    source: 'lib/desktop-autopilot.js',
    description: 'Navigate the autopilot browser to a URL',
  },
  'desktop-autopilot:browser-screenshot': {
    type: 'handle',
    source: 'lib/desktop-autopilot.js',
    description: 'Take a screenshot of the autopilot browser',
  },
  'desktop-autopilot:browser-state': {
    type: 'handle',
    source: 'lib/desktop-autopilot.js',
    description: 'Get current browser state (URL, title, tabs)',
  },
  'desktop-autopilot:browser-extract': {
    type: 'handle',
    source: 'lib/desktop-autopilot.js',
    description: 'Extract text content from the current page',
  },
  'desktop-autopilot:browser-close': {
    type: 'handle',
    source: 'lib/desktop-autopilot.js',
    description: 'Close the autopilot browser session',
  },
  'desktop-autopilot:system-applescript': {
    type: 'handle',
    source: 'lib/desktop-autopilot.js',
    description: 'Run AppleScript for macOS automation',
  },
  'desktop-autopilot:system-mouse': {
    type: 'handle',
    source: 'lib/desktop-autopilot.js',
    description: 'Mouse control (move, click, scroll)',
  },
  'desktop-autopilot:system-keyboard': {
    type: 'handle',
    source: 'lib/desktop-autopilot.js',
    description: 'Keyboard control (type text, press keys)',
  },
  ...
  // --- GSX Create (Claude Code-backed; legacy `aider:*` channel names kept for
  //     renderer compatibility; UI lives in gsx-create.html) ---
  'aider:start': { type: 'handle', source: 'main.js', description: 'Start GSX Create engine' },
  'aider:initialize': { type: 'handle', source: 'main.js', description: 'Initialize engine with repo' },
  'aider:run-prompt': { type: 'handle', source: 'main.js', description: 'Run coding prompt' },
  'aider:run-prompt-streaming': { type: 'handle', source: 'main.js', description: 'Run prompt streaming' },
  'aider:add-files': { type: 'handle', source: 'main.js', description: 'Add files to context' },
  'aider:remove-files': { type: 'handle', source: 'main.js', description: 'Remove files from context' },
  ...
  'aider:shutdown': { type: 'handle', source: 'main.js', description: 'Shutdown engine' },
  ...
  'location:get': { type: 'handle', source: 'main.js', description: 'Get best-known location (precise > IP > stored)' },
  'location:report-precise': { type: 'handle', source: 'main.js', description: 'Renderer reports navigator.geolocation coords to main for cross-agent use' },
  ...
  'budget:getStatsByAgent': { type: 'handle', source: 'main.js', description: 'Stats by agent (who spent what)' },
  'budget:getAgentLeaderboard': { type: 'handle', source: 'main.js', description: 'Ranked agent cost leaderboard' },
  'budget:getAgentCosts': { type: 'handle', source: 'main.js', description: 'Detailed cost breakdown for one agent' },
  ...
  // --- Email ---
  'email:get-accounts': { type: 'handle', source: 'main.js', description: 'List email accounts with connection status' },
  'email:add-account': { type: 'handle', source: 'main.js', description: 'Add a new email account' },
  'email:update-account': { type: 'handle', source: 'main.js', description: 'Update an email account' },
  'email:remove-account': { type: 'handle', source: 'main.js', description: 'Remove email account and delete credential' },
  'email:test-connection': { type: 'handle', source: 'main.js', description: 'Test IMAP/SMTP connection' },
  'email:connect-account': { type: 'handle', source: 'main.js', description: 'Connect a configured email account' },
  'email:connect-all': { type: 'handle', source: 'main.js', description: 'Connect all configured email accounts' },
  'email:disconnect-all': { type: 'handle', source: 'main.js', description: 'Disconnect all email accounts' },
  'email:get-inbox': { type: 'handle', source: 'main.js', description: 'Fetch inbox messages' },
  'email:get-message': { type: 'handle', source: 'main.js', description: 'Fetch a full email message by UID' },
  'email:search': { type: 'handle', source: 'main.js', description: 'Search email messages' },
  'email:send': { type: 'handle', source: 'main.js', description: 'Send an email' },
  'email:get-summary': { type: 'handle', source: 'main.js', description: 'Get email summary for agent cache' },
  'email:get-provider-presets': { type: 'handle', source: 'main.js', description: 'Get provider IMAP/SMTP presets' },

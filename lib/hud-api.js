...
// Phase 0 (agent-system upgrade): dual-write mirror into a consolidated
// task-store + durable task lifecycle log. Both are opt-in behind the
// `typedTaskContract` flag so behavior is unchanged until enabled.
...
/**
 * Record a task lifecycle event to the durable timeline. No-op unless the
 * `typedTaskContract` flag is on. Swallows all errors -- recording is
 * always best-effort and must never break the caller.
 */
function _recordLifecycle(taskId, type, data) {
  if (!taskId) return;
  if (!isAgentFlagEnabled('typedTaskContract')) return;
...
async function _runCouncilSubmission(text, opts) {
  ...
  // task. Dual-write into task-store when the typedTaskContract flag is on.
  ...
  // Eligible agents: built-in registry minus bidExcluded. When the
  // Phase 3 `roleBasedVoterPool` flag is on AND the task has a spaceId,
  // also drop specialist agents whose declared `defaultSpaces` do not
  // include the task space. Generalists always vote.
  const fullAgentList = (getAllAgents() || []).filter((a) => !a.bidExcluded);
  const agents = isAgentFlagEnabled('roleBasedVoterPool')
    ? filterEligibleAgents(fullAgentList, task)
    : fullAgentList;
...
async function submitTask(text, options = {}) {
  ...
  // ---- Phase 3: Auto variant selection ---------------------------------
  // When the caller didn't specify a variant, optionally ask a cheap
  // LLM micro-classifier. Gated by `variantSelector` so the cost opts
  // in. If classification yields 'council' and council mode is also on,
  // fall through into the Phase 1 council path. Otherwise we default to
  // the normal single-winner auction.
  let resolvedVariant = variant;
  if (!resolvedVariant && isAgentFlagEnabled('variantSelector')) {
    try {
  ...
  // ---- Phase 1: Council-mode dispatch ----------------------------------
  // When the (caller-supplied or auto-selected) variant is 'council' AND
  // the flag is enabled, bypass the normal single-winner pipeline and
  // run the task through lib/exchange/council-runner.js. Skips transcript
  // filtering and intent normalization -- requests routed here are
  // structured evaluation tasks, not ambient voice input.
  if (resolvedVariant === 'council' && isAgentFlagEnabled('councilMode')) {
    try {
      ...
      // Fall through to normal path on council failure -- never strand the user.
    }
  }

  // ---- Phase 3: Role-based voter-pool filter ----------------------------
  // When `roleBasedVoterPool` is on and the task has a spaceId, populate
  // metadata.agentFilter so the exchange only solicits bids from
  // eligible agents. Generalist agents (no declared defaultSpaces) still
  // bid on everything; specialists bid only in their space. Caller-
  // supplied metadata.agentFilter wins over automatic filtering.
  if (
    isAgentFlagEnabled('roleBasedVoterPool')
    && spaceId
    && !targetAgentId
...
function _cleanupStaleMaps() {
  ...
  // state doesn't outlive the raw Maps. No-op when the flag is off
  // (store will simply be empty).
  if (isAgentFlagEnabled('typedTaskContract')) {

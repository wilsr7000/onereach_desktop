...
// Minimum safety timeout for TTS playback. Long-form briefs can
// easily run 45-90s so 30s was too tight and caused the mic to
// un-mute mid-playback, generating self-feedback.
const SPEECH_SAFETY_MIN_MS = 30_000;
// Hard ceiling so a truly stuck state machine still clears eventually.
const SPEECH_SAFETY_MAX_MS = 300_000;
// Padding added to an estimated duration when caller knows it up front.
const SPEECH_SAFETY_BUFFER_MS = 10_000;

/**
 * Mark that TTS playback has started.
 * The voice listener should mute/clear its input buffer while this is true.
 *
 * @param {number} [expectedDurationMs] Estimated TTS length. When
 *   provided, the safety timeout scales to
 *   max(SPEECH_SAFETY_MIN_MS, expected + buffer), capped at
 *   SPEECH_SAFETY_MAX_MS. This prevents 40s+ briefs from being
 *   auto-cleared mid-playback.
 */
function speechStarted(expectedDurationMs) {
  _isSpeaking = true;
  let timeoutMs = SPEECH_SAFETY_MIN_MS;
  if (typeof expectedDurationMs === 'number' && expectedDurationMs > 0) {
    timeoutMs = Math.min(
      SPEECH_SAFETY_MAX_MS,
      Math.max(SPEECH_SAFETY_MIN_MS, expectedDurationMs + SPEECH_SAFETY_BUFFER_MS)
    );
  }
  clearTimeout(_speakingTimeout);
  _speakingTimeout = setTimeout(() => {
    if (_isSpeaking) {
      log.warn('ipc', `Speech state auto-cleared after ${Math.round(timeoutMs / 1000)}s safety timeout`);
      _isSpeaking = false;
...
async function submitTask(text, options = {}) {
  ...
  // ---- Auto variant selection (opt-in) ---------------------------------
  // The caller controls variant dispatch. When they pass `variant: 'auto'`,
  // we spend a cheap cached LLM call to classify winner/council/probers.
  // When they pass an explicit variant, we honor it. When they pass
  // nothing, we default to winner -- the classic single-agent auction.
  //
  // Why opt-in? Auto-classification on every submit would route voice
  // commands like "give me my daily brief" through an LLM classifier
  // that occasionally misfires and sends them to council mode, which
  // bypasses the winning agent's execute() (and its rich UI) in favor
  // of a multi-agent aggregate. Explicit opt-in keeps voice paths fast
  // and deterministic.
  ...
  // A pending agent question makes short answers MEANINGFUL: when an agent
  // is awaiting input ("Want me to build it?"), "yes" / "no" / "playbook"
  // are exactly what we expect — the mic-noise filter must not eat them.
  // Live fail 2026-08-05: builder asked for consent, the user said "Yes.",
  // the filter rejected it as 'filler or fragment', and the consent never
  // reached routePendingInput. The whole multi-turn surface dead-ended.
  let agentAwaitingInput = false;
  try {
    const { getTranscriptService } = require('./transcript-service');
    agentAwaitingInput = getTranscriptService().hasPending() === true;
  } catch (_e) {
    agentAwaitingInput = false;
  }
  if (agentAwaitingInput) {
    log.info('ipc', 'Filter bypassed: agent awaiting input (short answers are meaningful)', {
      text: text.slice(0, 40),
    });
  }
  if (!skipFilter && !agentAwaitingInput) {
    try {
 ...
 * @param {Object} request - { taskId, prompt, agentId, field, proactive }
 */
function emitNeedsInput(request) {
  ...
  const fullRequest = {
    ...
    // proactive: this needsInput came from a background/scheduled agent the user
    // never invoked. The voice relay must surface it even if the owning tool is
    // otherwise self-prompting (see relay-core.shouldSurfaceNeedsInput / UC5).
...
function _registerIPC() {
  ...
  // Task submission. This is the load-bearing entry point for the voice orb:
  // window.agentHUD.submitTask() -> here -> exchange auction. Every call logs
  // (with the caller's traceId when present) so an orb dispatch is traceable
  // end-to-end in the `ipc` category even when the orb window has devTools off.
  ipcMain.handle('hud-api:submit-task', async (_event, text, options) => {
    const traceId = options?.traceId || '-';
    try {
      log.info('ipc', 'submit-task called', {
        traceId,
  ...
  // Speech state (mic gating)
  ipcMain.handle('hud-api:speech-started', async (_event, expectedDurationMs) => {

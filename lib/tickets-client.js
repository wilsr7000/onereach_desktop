 ...
 *
 * KV endpoint: https://em.edison.api.onereach.ai/http/{accountId}/keyvalue2
...
// URL construction and transport (timeout + one retry) ride the
// lib/kv-client chokepoint; the FLOW-token wire ({id,key,value} bodies,
// Authorization header) is this client's own and stays here.
const { kvUrlForAccount, kvFetch } = require('./kv-client');

function getKVUrl() {
  return kvUrlForAccount(getAccountId());
}
...
async function kvDelete(collection, key) {
  ...
  const resp = await kvFetch(getKVUrl(), {
    method: 'DELETE',
    headers: { 'Content-Type': 'application/json', Authorization: token },
    body: JSON.stringify({ id: collection, key }),
  });

  if (resp.status === 404) return; // already gone — idempotent
  if (!resp.ok) {
    const text = await resp.text().catch(() => '');

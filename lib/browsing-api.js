...
class BrowsingAPI extends EventEmitter {
  ...
  async setViewport(sessionId, viewport) {
    ...
    if (spec.mobile) {
      ...
      try { sess.window.webContents.disableDeviceEmulation(); } catch (_) { /* intentionally empty */ }
  ...
  async checkAuthState(sessionId, opts = {}) {
    ...
    let domain = '';
    try { domain = new URL(url).hostname; } catch (_) { /* intentionally empty */ }
  ...
  async _tryAutoAuth(sessionId, sess, originalUrl, _detection) {
    let domain = '';
    ...
    for (const source of sources) {
      try {
        ...
        if (!newDetection.blocked) {
          try { await this.saveToAuthPool(sessionId); } catch (_) { /* intentionally empty */ }
...
`;
  ...
  async _sendTrustedKey(sess, ref, key) {
    ...
    const _keyCode = key.charCodeAt(0);

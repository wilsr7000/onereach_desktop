 ...
 *
 *   POST /playbook-qa
 *     Body:   { question, mode?, spaceId?, limit?, refresh?, profile?,
 *               viewerId? }  (or X-Viewer-Id header)
 *     Results are scoped to what that viewer may see; access that cannot
 *     be established yields NOTHING (fail closed). Scoping is not auth —
 *     see the security posture note below.
 *     200 ->  { question, mode, answer, results[], evaluated,
 *               totalCandidates } — retrieval finds, quality scores.
 *
 *   GET /health
...
async function _readJsonBody(req, maxBytes = 256 * 1024) {
  return new Promise((resolve, reject) => {
    let size = 0;
    const chunks = [];
    // A malformed or oversized body is the CLIENT's fault: tag the
    // rejection 400 so the router answers instead of 500-ing (or, before
    // 2026-09-01, never answering at all — see _router).
    ...
    req.on('data', (c) => {
      ...
      if (size > maxBytes) {
        req.destroy();
        return reject(clientError('Request body too large'));
      }
      chunks.push(c);
    });
    req.on('end', () => {
      ...
      try {
        ...
        reject(clientError('Invalid JSON body: ' + err.message));
      }
    });
    req.on('error', reject);
  });
...
/**
 * POST /playbook-qa — ask the playbook corpus a question.
 *
 * The whole point of this route: the SAME answer shape whether the asker
 * is the app, a flow, or another agent. Business logic lives in
 * lib/playbook-qa.js; this is transport only, like every other route.
 *
 * Body: { question, mode?: 'auto'|'quality'|'retrieval', spaceId?,
 *         limit?, refresh?, profile? }
 * 200 -> { question, mode, answer, results[], evaluated, totalCandidates }
 */
async function _handlePlaybookQa(req, res) {
  const body = await _readJsonBody(req);
  const question = typeof body.question === 'string' ? body.question.trim() : '';
  if (question.length === 0) return _sendError(res, 400, 'question is required');

  const mode = body.mode || 'auto';
  if (!['auto', 'quality', 'retrieval'].includes(mode)) {
    return _sendError(res, 400, `unknown mode: ${mode}`);
  }

  let qa;
  try {
    qa = require('./playbook-qa');
  } catch (err) {
    return _sendError(res, 503, `playbook-qa not available: ${err.message}`);
  }

  const opts = { mode };
  // Viewer scoping. NOT authentication: this gateway binds to 127.0.0.1
  // and has no auth, so a local caller can claim any id. What this does
  // is stop the agent answering about playbooks the named viewer cannot
  // see — and with no viewer, the resolver falls back to open spaces
  // only. Fail closed lives in playbook-qa.filterByAccess.
  const viewerId =
    (typeof body.viewerId === 'string' && body.viewerId.trim()) ||
    (typeof req.headers['x-viewer-id'] === 'string' && req.headers['x-viewer-id'].trim()) ||
    '';
  if (viewerId.length > 0) opts.viewer = { id: viewerId };
  if (typeof body.spaceId === 'string' && body.spaceId.length > 0) opts.spaceId = body.spaceId;
  if (Number(body.limit) > 0) opts.limit = Number(body.limit);
  if (body.refresh === true) opts.refresh = true;
  if (typeof body.profile === 'string' && body.profile.length > 0) opts.profile = body.profile;

  try {
    const answer = await qa.answerPlaybookQuestion(question, opts);
    _sendJson(res, 200, answer);
  } catch (err) {
    // A corpus sweep can fail late (LLM outage mid-walk). Report it as a
    // failure rather than an empty-but-successful answer — "no playbooks
    // are well written" and "I could not check" are different claims.
    _sendError(res, 500, err.message || 'playbook question failed');
  }
}
...
async function _router(req, res) {
  ...
  try {
    if (req.method === 'GET' && pathname === '/health') {
      return await _handleHealth(req, res);
    }

    if (req.method === 'POST' && pathname === '/submit-task') {
      return await _handleSubmitTask(req, res);
    }

    if (req.method === 'POST' && pathname === '/respond-input') {
      return await _handleRespondInput(req, res);
    }

    if (req.method === 'POST' && pathname === '/select-disambiguation') {
      return await _handleSelectDisambiguation(req, res);
    }

    if (req.method === 'POST' && pathname === '/cancel-task') {
      return await _handleCancelTask(req, res);
    }

    if (req.method === 'POST' && pathname === '/playbook-qa') {
      return await _handlePlaybookQa(req, res);
    }

    const eventsMatch = pathname.match(/^\/events\/([^/]+)$/);
    if (req.method === 'GET' && eventsMatch) {
      return await _handleEvents(req, res, decodeURIComponent(eventsMatch[1]));
    }

    _sendError(res, 404, `Unknown route: ${req.method} ${pathname}`);
  } catch (err) {
    // Every handler is awaited above, so a thrown/rejected body parse or
    // handler error lands HERE and always gets an answer. Client-caused
    // failures (bad JSON, oversized body) carry statusCode 400.
    if (res.headersSent) return;
    _sendError(res, err && err.statusCode ? err.statusCode : 500, (err && err.message) || 'Internal error');
  }
}

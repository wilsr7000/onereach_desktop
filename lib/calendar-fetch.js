...
const { getCircuitBreaker, CircuitOpenError } = require('./calendar-circuit-breaker');
...
async function fetchEventsForRange(startDate, endDate, { includeDetails = true } = {}) {
  ...
  // Phase 5: every Omnical fetch goes through the circuit breaker. If the
  // breaker is open we skip the network call entirely and either serve the
  // stale cache (tagged) or throw so callers can fall back to the local store.
  ...
  try {
    ...
    const reason = error instanceof CircuitOpenError ? 'circuit-open' : 'fetch-failed';
    log.error('calendar-fetch', `Omnical fetch failed (${reason})`, { error: error.message });

    if (_cache.events) {
      log.info('calendar-fetch', 'Returning stale cached events', { reason });
      // Tag the array (non-enumerable so JSON serialisation isn't affected).
      Object.defineProperty(_cache.events, '__stale', { value: true, enumerable: false, configurable: true });
      Object.defineProperty(_cache.events, '__staleReason', { value: reason, enumerable: false, configurable: true });
      return _cache.events;
    }
    throw error;
  }
...
module.exports = {
  ...
  // Phase 5 reliability surface
  getCircuitState: () => getCircuitBreaker().getState(),
  resetCircuit: () => getCircuitBreaker().reset(),

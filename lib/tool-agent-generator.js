...
async function fetchDocsContent(url) {
  try {
    ...
    log.error('agent', 'Error fetching docs', { error: error.message });
    throw error;
  }
}
  ...
  try {
    ...
    if (global.clipboardManager && global.clipboardManager.spacesAPI) {
      ...
      if (!toolAgentsSpace) {
        ...
        log.info('agent', 'Created "Tool Agents" space');
      }
      ...
      log.info('agent', 'Saved agent markdown to Spaces', { agentDef: agentDef.name });
    } else {
      log.warn('agent', 'Spaces API not available, skipping markdown save');
    }
  } catch (error) {
    log.error('agent', 'Error saving to Spaces', { error: error.message });
...
When you match a request, respond with a brief confirmation and open the tool.`;
    ...
    const agentData = {
      ...
      keywords: agentDef.examplePhrases.map((p) =>
        p
          .toLowerCase()
          .replace(/[^a-z0-9 ]/g, '')
          .trim()
    ...
    log.info('agent', 'Registered agent', { createdAgent: createdAgent.id });
    return createdAgent;
  } catch (error) {
    log.error('agent', 'Error registering agent', { error: error.message });
    throw error;
  }
}
...
async function deleteToolAgent(toolId) {
  try {
    ...
    if (toolAgent) {
      ...
      log.info('agent', 'Deleted agent', { toolAgent: toolAgent.id });
    } else {
      log.info('agent', 'No agent found for tool', { toolId: toolId });
    }
    ...
    if (global.clipboardManager && global.clipboardManager.spacesAPI) {
      ...
      if (toolAgentsSpace) {
        ...
        if (agentItem) {
          ...
          log.info('agent', 'Removed agent markdown from Spaces');
        }
      }
    }
  } catch (error) {
    log.error('agent', 'Error deleting agent', { error: error.message });
...
async function generateAgentFromDocs(tool) {
  ...
  log.info('agent', 'Generating agent for', { tool: tool.name });
  log.info('agent', 'Fetching docs from', { tool: tool.docsUrl });

  // 1. Fetch documentation content
  const docsContent = await fetchDocsContent(tool.docsUrl);
  log.info('agent', 'Fetched chars of documentation', { docsContent: docsContent.length });

  // 2. Generate agent definition using LLM
  const agentDef = await generateAgentDefinition(tool, docsContent);
  log.info('agent', 'Generated agent definition', { agentDef: agentDef.id });
  ...
  log.info('agent', 'Successfully created agent', { agentDef: agentDef.name });
  return agentDef;
}

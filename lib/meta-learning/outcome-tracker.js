...
class OutcomeTracker {
  ...
  async recordOutcome(evaluationId, outcome) {
    const record = {
      ...
      // Suggestion outcomes
      suggestionOutcomes: outcome.suggestions?.map(s => ({
        suggestionId: s.id,
    ...
    if (this.outcomeHistory.length > this.maxHistory) {
      const removed = this.outcomeHistory.pop();
  ...
  calculateAgentAccuracy(agentType, options = {}) {
    ...
    const predictions = outcomes.flatMap(o => 
      o.agentPredictions?.filter(a => a.agentType === agentType) || []
    );
    ...
    const correct = accuracyCounts[ACCURACY_TYPES.TRUE_POSITIVE] + 
                    accuracyCounts[ACCURACY_TYPES.TRUE_NEGATIVE];

...
class QRScanner {
  ...
  async scanFromScreen() {
    try {
      ...
      // Get screen dimensions (use focused window's display for multi-monitor)
      ...
      // Match the source to the detected display (multi-monitor aware)
      // Each source has a display_id that maps to display.id
      let screenSource = sources.find((s) => String(s.display_id) === String(display.id));
      if (!screenSource) {
        // Fallback: first source (primary screen)

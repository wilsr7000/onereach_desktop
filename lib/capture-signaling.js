...
function getGuestPageHTML() {
  if (!_guestPageHTML) {
    try {
      const { buildGuestPageHTML } = require('./meeting/capture-guest-page');
      ...
      try {
        if (global.settingsManager) {
          ...
          if (refreshUrl) kvUrl = refreshUrl.replace('/refresh_token', '/keyvalue2');
        }
      } catch {
        /* no-op */
      }
      if (!kvUrl) {
        kvUrl = `https://em.edison.api.onereach.ai/http/${FALLBACK_ACCOUNT}/keyvalue2`;
      }
  ...
  _getKvUrl() {
    ...
    try {
      if (global.settingsManager) {
        const refreshUrl = global.settingsManager.get('gsxRefreshUrl');
        if (refreshUrl) {
          return refreshUrl.replace('/refresh_token', '/keyvalue2');
        }
      }
    } catch {
      /* no-op */
    }
    return `https://em.edison.api.onereach.ai/http/${FALLBACK_ACCOUNT}/keyvalue2`;
  }

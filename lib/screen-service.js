...
/** Collapsed window size (orb circle + margin + type button to the left) */
const ORB_COLLAPSED_WIDTH = 170;
const ORB_COLLAPSED_HEIGHT = 110;
 ...
 * BrowserWindow (collapsed default 170x110, expands dynamically for chat/tooltip).
 * The orb sits at the bottom of the window, offset to the left or right
 * depending on `side`.
 *
...
function computeHUDPosition(orbWindow, side, hudWidth, hudHeight, spacing = 20) {
  ...
  const orbOnRightHalf = cx > displayCenterX;

  let x;
  if (orbOnRightHalf) {
    x = cx - size / 2 - hudWidth - spacing;
  } else {
    x = cx + size / 2 + spacing;
  }

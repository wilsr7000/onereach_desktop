...
const OP_TYPE = Object.freeze({
  ...
  ASSET_MERGE: 'asset.merge', // emitted by conflict resolution
});
...
/**
 * Cypher for step 3 (asset.upsert / asset.update / asset.merge): upsert
 * :Asset, atomically bump the writer's vector-clock slot, and write
 * :OperationLog. All in a single graph transaction so vc consistency
 * holds against concurrent writes from this same device.
 *
 * The vc bump uses APOC's `apoc.map.setKey` (Aura ships with APOC by
 * default). Pure Cypher map literals don't support dynamic keys; APOC
 * is the standard escape hatch for { [variable]: value } shapes.
 *
 * Param shape:
...
const CYPHER_OP_TX = `
  ...
  ON CREATE SET a.createdAt = datetime($at), a.vc = {}, a.active = true
  SET a.lastUpdatedAt = datetime($at),
      a.contentHash = $contentHash,
      a.active = true,
      a.vc = apoc.map.setKey(
        coalesce(a.vc, {}),
        $deviceId,
        coalesce(a.vc[$deviceId], 0) + 1
      )
  CREATE (op:OperationLog {
    ...
    vcAfter: apoc.convert.toJson(a.vc),
    ackedByDevice: false
  })
  MERGE (op)-[:APPLIED_TO]->(a)
  RETURN op.traceId AS traceId, a.vc AS vcAfter
`;

/**
 * Cypher for asset.delete: atomically bump vc, write the :Tombstone with
 * the post-bump vc as finalVc, and soft-delete the :Asset. Same single-
 * statement atomicity as CYPHER_OP_TX so the no-resurrection invariant
 * (v5 invariant 4) holds even under concurrent writes.
 *
 * Note: the tombstone's finalVc is the asset's vc AFTER the bump, so any
 * pre-delete write whose vc this device had not yet seen would be
 * concurrent with the tombstone -- and concurrent-vs-delete loses to
 * delete (per tombstone.shouldAllowWrite).
 */
const CYPHER_DELETE_TX = `
  MATCH (a:Asset {id: $entityId})
  SET a.vc = apoc.map.setKey(
        coalesce(a.vc, {}),
        $deviceId,
        coalesce(a.vc[$deviceId], 0) + 1
      ),
      ...
      a.deletedBy = $deviceId,
      a.lastUpdatedAt = datetime($at)
  CREATE (t:Tombstone {
    entityId: $entityId,
    deletedAt: datetime($at),
    deletedBy: $deviceId,
    finalVc: apoc.convert.toJson(a.vc)
  })
  CREATE (op:OperationLog {
    traceId: $traceId,
    op: $opType,
...
class SyncEngine {
  ...
  async _processOp(op) {
    try {
      const isDelete = op.opType === OP_TYPE.ASSET_DELETE;

      // Step 1: hash the content if any. Deletes have no body.
      ...
      if (!isDelete) {
        ...
        if (content !== null && content !== undefined) {
          ...
          // Step 2 (deletes skip): blob upload (idempotent, by hash).
          await this._blobStore.upload(contentHash, content, { traceId: op.traceId });
        }
      }

      // Step 3: graph transaction. Branch on opType:
      //   - delete -> CYPHER_DELETE_TX writes the tombstone + bumps vc + soft-deletes
      //   - upsert/update/merge -> CYPHER_OP_TX bumps vc + sets contentHash
      const at = new Date(this._now()).toISOString();
      const cypher = isDelete ? CYPHER_DELETE_TX : CYPHER_OP_TX;
      const result = await this._omniClient.executeQuery(cypher, {
      ...
      // Capture the post-bump vc for the local op record. Phase 4 pull
      // engine will use this to detect ack via heartbeat.
      try {
        const v = result?.[0]?.vcAfter;
        if (v && this._queue.get(op.traceId)) {
          // The op may already be removed by the time we get here in fast
          // tests; safe-guard the assignment.
          this._queue.get(op.traceId).vcAfter = v;
        }
      } catch (_) {
        /* non-fatal */
      }

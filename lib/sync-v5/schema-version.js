...
/**
 * Run the boot-time handshake: read the graph version, compare against
 * compiled-in version, return the compatibility state plus a human-readable
 * banner string for the diagnostics overlay.
 *
 * @param {object} [opts]
 * @param {object} [opts.omniClient] -- test override
 * @returns {Promise<{state:string, deviceVersion:number, graphVersion:number|null, banner:string|null, writeAllowed:boolean}>}
 */
async function handshake(opts = {}) {
  ...
  const writeAllowed = isWriteAllowed(state);

  let banner = null;
  if (state === COMPAT_STATES.UNKNOWN) {
    banner =
  ...
  return { state, deviceVersion, graphVersion, banner, writeAllowed };
}

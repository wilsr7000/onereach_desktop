...
const providers = {
  ...
  pullEngine: null,
  /**
   * @type {null | object} -- Phase 5 (commit B) Replica instance with
   *   .inspect(). Returns the SQLite replica's counts + schemaVersion +
   *   FTS5 availability + cutover-flag state. Returns null when
   *   `syncV5.replica.enabled` is false (the typical case until cutover
   *   completes).
   */
  replica: null,
  /**
   * @type {null | object} -- Phase 5 (commit D) ValidationGate instance
   *   with .evaluate(). Powers the /sync/replica/validation endpoint
   *   that the cutover flag in commit E reads to gate the read-path
   *   flip per §6.6 (≥7 days + per-method invocation thresholds + zero
   *   divergences).
   */
  validationGate: null,
};
...
function setProviders(partial) {
  ...
  log.info('sync-v5', 'diagnostics-endpoints providers updated', {
    ...
    replica: !!providers.replica,
    validationGate: !!providers.validationGate,
  });
}
...
async function handleSyncQueue() {
  ...
  let replica = null;
  if (providers.replica && typeof providers.replica.inspect === 'function') {
    try {
      replica = providers.replica.inspect();
    } catch (err) {
      replica = { error: err.message };
    }
  }
  return {
    status: 200,
    body: {
      ...
      replica: replica || {
        wired: false,
        note: 'Materialised SQLite replica not yet wired (commit B opt-in via syncV5.replica.enabled).',
      },
    },
  };
}
...
/**
 * GET /sync/replica/validation
 *
 * Per docs/sync-v5/replica-shape.md §6.6: returns the cutover-gate
 * evaluation -- per-method invocation counters vs thresholds, wall-
 * clock floor, divergence totals, and the boolean cutoverAllowed
 * that commit E's flag-flip checks. Operators read this with curl
 * to know exactly what's blocking the read-path flip.
 *
 * When the validation gate isn't wired (replica disabled, or
 * shadow-read not yet enabled), returns wired:false so the surface
 * doesn't lie about being ready.
 */
async function handleSyncReplicaValidation() {
  if (!providers.validationGate || typeof providers.validationGate.evaluate !== 'function') {
    return {
      status: 200,
      body: {
        phase: PHASE,
        ...
        note: 'Replica validation gate not wired (syncV5.replica.shadowReadEnabled false, or replica disabled).',
      },
    };
  }
  try {
    const evaluation = providers.validationGate.evaluate();
    return { status: 200, body: { phase: PHASE, wired: true, ...evaluation } };
  } catch (err) {
    return { status: 500, body: { phase: PHASE, wired: true, error: err.message } };
  }
}

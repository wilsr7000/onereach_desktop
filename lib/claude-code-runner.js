...
function getClaudeCodePath() {
  // Null-guard so this function is safe to call from unit tests / CLI probes
  // where the Electron `app` object is undefined. Inside Electron main we
  // still hit the packaged branch normally.
 ...
 */
/** The newest model every recent Claude Code binary accepts (retry target). */
const VERSION_GATE_FALLBACK_MODEL = 'claude-opus-4-8';

/**
 * Pure: is this CLI result the "binary too old for this model" 400?
 * Shape seen live 2026-09-02: `Claude Code 2.1.112 does not support this
 * model; version 2.1.251 or newer is required. Run 'claude update'…`.
 */
function isModelVersionGateError(text) {
  const t = String(text || '');
  return /does not support this model/i.test(t) && /or newer is required/i.test(t);
}
...
async function runClaudeCode(prompt, options = {}) {
  const {
    ...
    maxTurns,
    maxBudget,
    // Default the app's embedded Claude Code to Opus 4.8 at max effort.
    ...
    child.on('close', (code) => {
      ...
      if (streaming) {
        ...
        try {
          ...
          // 2026-09-02 (Fable 5.1 upgrade): a Claude Code binary older than
          // the model requires answers with a 400 naming the required
          // version. Rather than hard-fail the feature on a stale CLI,
          // retry ONCE on the newest model every recent CLI accepts and
          // say loudly how to fix it. The bundled CLI is version-guarded
          // at build time (scripts/download-claude-code.js), so this mostly
          // protects dev machines running the system `claude`.
          if (parsed && parsed.is_error === true && isModelVersionGateError(outputText)) {
            versionGateHit = true;
          }
        } catch (_) {
      ...
      try {
        ...
        budgetManager.trackUsage({
          provider: 'anthropic',
          model: (parsed && parsed.model) || 'claude-fable-5-1',
      ...
      // ---- Version-gate fallback (see isModelVersionGateError) ----
      if (versionGateHit && !options._versionGateRetried && model !== VERSION_GATE_FALLBACK_MODEL) {
        log.warn('app', `Claude Code binary is too old for ${model} — retrying once on ${VERSION_GATE_FALLBACK_MODEL}. Run \`claude update\` (or rebuild to refresh the bundled CLI) to use ${model}.`, {
          model, fallback: VERSION_GATE_FALLBACK_MODEL, detail: String(outputText).slice(0, 200),
        });
        runClaudeCode(prompt, { ...options, model: VERSION_GATE_FALLBACK_MODEL, _versionGateRetried: true }).then(resolve, reject);
        return;
      }
...
module.exports.isModelVersionGateError = isModelVersionGateError;
module.exports.VERSION_GATE_FALLBACK_MODEL = VERSION_GATE_FALLBACK_MODEL;

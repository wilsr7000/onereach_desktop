...
function getClaudeCodePath() {
  ...
  if (app.isPackaged) {
    ...
    if (fs.existsSync(bundledPath)) {
      log.info('app', 'Using bundled binary', { bundledPath: bundledPath });
      return bundledPath;
    }
    ...
    if (fs.existsSync(fallbackPath)) {
      log.info('app', 'Using fallback bundled path', { fallbackPath: fallbackPath });
      return fallbackPath;
    }
  }
...
async function isClaudeCodeAvailable() {
  ...
  if (isBundledBinary) {
    if (fs.existsSync(claudePath)) {
      ...
      log.info('app', 'Bundled binary not found at', { claudePath: claudePath });
    }
  }
...
async function runClaudeCode(prompt, options = {}) {
  ...
  try {
    ...
    if (apiKey) {
      ...
      log.info('app', 'Using API key from settings');
    }
  } catch (e) {
    log.warn('app', 'Could not get API key from settings', { error: e.message });
  }
  
  log.info('app', 'Starting Claude Code...');
  log.info('app', 'Path', { claudePath: claudePath });
  log.info('app', 'CWD', { cwdprocess: cwd || process.cwd() });
  log.info('app', 'Prompt', { prompt: prompt.substring(0, 100) + '...' });
  ...
  return new Promise((resolve) => {
    ...
    if (isBundledBinary) {
      ...
      log.info('app', 'Using bundled binary directly');
    } else {
      ...
      if (nodePath) {
        ...
        log.info('app', 'Found node at', { nodePath: nodePath });
      }
      ...
      log.info('app', 'Using global command', { actualClaudePath: actualClaudePath });
    }
    
    log.info('app', 'Command preview', { shellCommand: shellCommand.substring(0, 200) + '...' });
    ...
    try {
      ...
      execError = err;
      log.info('app', 'Exec completed with error (checking output)', { err: err.message?.substring(0, 100) });
    }
    ...
    try {
      ...
      log.error('app', 'Failed to read output file', { readError: readError.message });
    }
    
    // Check for common error patterns
    if (stdout.includes('env: node: No such file or directory')) {
      log.error('app', 'Node not found in PATH');
      resolve({
        success: false,
    ...
    if (stdout.includes('command not found') || stdout.includes('No such file or directory')) {
      log.error('app', 'Claude CLI not found');
      resolve({
        success: false,
    ...
    if (stdout && stdout.trim().length > 0) {
      ...
      try {
        ...
        log.warn('app', 'Failed to track usage', { trackError: trackError.message });
      }
...
function cancelClaudeCode() {
  ...
  log.info('app', 'Cancelling process...');
  ...
  try {
    ...
    log.error('app', 'Cancel error', { error: error });
    return false;
  }
}
  ...
  try {
    ...
    if (jsonMatch) {
      try {
        ...
        log.error('app', 'Plan JSON parse error', { parseError: parseError.message });
        return {
          success: false,

...
class AnthropicAdapter {
  ...
  async chat(opts) {
    ...
    // Opus 4.7 and later deprecated the `temperature` parameter entirely:
    // Anthropic returns 400 "`temperature` is deprecated for this model."
    // if you send it. We only include temperature for models that still
    // accept it. Claude 4.7 models (and any claude-opus-5+) are temp-less.
    const modelDeprecatesTemperature = /claude-opus-4-7|claude-opus-5/.test(String(model || ''));
  ...
  async *chatStream(opts) {
    ...
    // Same deprecation guard as chat(): Opus 4.7 + newer reject `temperature`.
    const modelDeprecatesTemperature = /claude-opus-4-7|claude-opus-5/.test(String(model || ''));

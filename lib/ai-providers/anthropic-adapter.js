...
class AnthropicAdapter {
   ...
   * @param {string} opts.model - Model name (e.g. 'claude-fable-5-1')
   ...
   *   - true or { type: 'adaptive' } → adaptive thinking (Opus 4.6+, 4.7)
  ...
  async chat(opts) {
    ...
    const thinkingConfig = this._resolveThinking(thinking, model);
    const isThinking = !!thinkingConfig;

    _log(
      'debug',
      `chat REQUEST model=${model} messages=${messages?.length || 0} maxTokens=${maxTokens} jsonMode=${jsonMode} thinking=${isThinking ? thinkingConfig.type : 'off'} effort=${effort || 'default'}`
    );
    ...
    const modelDeprecatesTemperature = this._omitsSamplingParams(model);

    if (isThinking) {
      // Thinking mode still requires temperature=1 on models that support it.
      if (!modelDeprecatesTemperature) body.temperature = 1;
      body.thinking = thinkingConfig;
    } else if (!modelDeprecatesTemperature) {
    ...
    _log(
      'debug',
      `chat RESPONSE model=${data.model || model} tokens=${data.usage?.input_tokens || 0}+${data.usage?.output_tokens || 0} finish=${data.stop_reason} toolCalls=${toolUseBlocks.length}`
    );

    // Strip markdown code fences when jsonMode is true.
    ...
    // Normalize tool_use blocks into a provider-agnostic shape
    ...
    return {
      ...
      toolCalls,
      rawContent: toolCalls ? data.content : undefined,
      usage: {
  ...
  async *chatStream(opts) {
    ...
    _log(
      'debug',
      `chatStream REQUEST model=${model} messages=${messages?.length || 0} thinking=${isThinking ? thinkingConfig.type : 'off'}`
    );
    ...
    // Same deprecation guard as chat(): Fable 5 / Opus 4.7+ reject `temperature`.
    const modelDeprecatesTemperature = this._omitsSamplingParams(model);
    ...
    // Strip markdown code fences when jsonMode is true (streaming path).
    let streamContent = fullContent;
    if (jsonMode && streamContent) {
    ...
    _log(
      'debug',
      `chatStream DONE model=${responseModel} tokens=${usage.promptTokens}+${usage.completionTokens} thinking=${fullThinking ? 'yes' : 'no'} finish=${finishReason}`
    );
  ...
  async vision(opts) {
    ...
    _log(
      'debug',
      `vision REQUEST model=${model} promptLen=${prompt?.length || 0} imageLen=${typeof imageData === 'string' ? imageData.length : 'buffer'}`
    );
  ...
  /**
   * Models with the Claude 4.7+ request surface (Fable 5, Opus 4.7, Opus 4.8,
   * and any future claude-opus-5+) removed the legacy sampling and thinking
   * parameters: sending `temperature`/`top_p`/`top_k` or
   * `thinking: { type: 'enabled', budget_tokens }` returns a 400.
   * Adaptive thinking is the only thinking mode on these models, and on
   * Fable 5 an explicit `{ type: 'disabled' }` also 400s — the param must be
   * omitted entirely (we always omit when thinking is off, so that's safe).
   */
  _omitsSamplingParams(model) {
    return /claude-fable|claude-opus-4-7|claude-opus-4-8|claude-opus-5/.test(String(model || ''));
  }
   ...
   *                                       (coerced to adaptive on models that
   *                                       removed budget_tokens — see
   *                                       _omitsSamplingParams)
   *   falsy                             → null  (thinking disabled; the
   *                                       `thinking` field is omitted from
   *                                       the request body)
   *
   * @param {Object|boolean|null} thinking
   * @param {string} [model] - Target model, used to gate legacy shapes
   * @returns {Object|null}
   */
  _resolveThinking(thinking, model) {
    if (!thinking) return null;
    if (thinking === true) return { type: 'adaptive' };
    if (thinking.type === 'adaptive') return { type: 'adaptive' };
    if (thinking.type === 'enabled') {
      if (this._omitsSamplingParams(model)) {
        _log('info', `Model ${model} does not support budget_tokens; using adaptive thinking instead`);

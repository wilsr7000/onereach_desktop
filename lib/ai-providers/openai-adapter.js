  ...
  async tts(opts) {
    ...
    return new Promise((resolve, reject) => {
      ...
      fetch('http://127.0.0.1:7242/ingest/54746cc5-c924-4bb5-9e76-3f6b729e6870', {
        ...
        body: JSON.stringify({
          location: 'openai-adapter.js:tts-request',
          message: 'TTS API request body',
          data: {
            model: body.model,
            voice: body.voice,
            speed: body.speed,
            instructions: body.instructions?.slice(0, 60),
            inputLen: body.input?.length,
            format: body.response_format,
          },
          timestamp: Date.now(),
          hypothesisId: 'TTS-SPEED',
        }),
      }).catch((err) => console.warn('[openai-adapter] ingest fetch:', err.message));
      // #endregion

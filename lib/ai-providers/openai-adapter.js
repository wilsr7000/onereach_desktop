...
class OpenAIAdapter {
  ...
  async chat(opts) {
    ...
    // Tool definitions (OpenAI format: {type:'function', function:{name, description, parameters}})
    ...
    _log(
      'debug',
      `chat RESPONSE model=${data.model || model} tokens=${data.usage?.prompt_tokens || 0}+${data.usage?.completion_tokens || 0} finish=${choice?.finish_reason} toolCalls=${rawToolCalls.length}`
    );

    // Normalize tool_calls into a provider-agnostic shape
    ...
    return {
      ...
      toolCalls,
      rawMessage: toolCalls ? choice?.message : undefined,
      usage: {
   ...
   *
   * Uses the GA Realtime API 2 endpoint. Bearer-auth only -- the
   * `OpenAI-Beta: realtime=v1` header was preview-only and the GA
   * endpoint rejects it.
   *
   * @param {Object} opts
   * @param {string} opts.apiKey
   * @param {string} [opts.model='gpt-realtime-2']
  ...
  createRealtimeSession(opts) {
    const { apiKey, model = 'gpt-realtime-2', onMessage, onError, onClose } = opts;
    ...
    // Keepalive: an idle realtime WebSocket that traverses a proxy/gateway
    // can be silently dropped, surfacing as an abrupt 1005 close with no
    // frame. A periodic ping keeps the connection warm and lets `ws` detect a
    // half-open socket. Started on open, cleared on close. unref() so it can
    ...
    ws.on('open', () => {
      ...
      keepaliveTimer = setInterval(() => {
        try {
          ...
          /* ping is best-effort; a real failure surfaces via close/error */
        }
      }, KEEPALIVE_MS);

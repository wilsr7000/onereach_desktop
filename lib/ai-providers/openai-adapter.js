...
class OpenAIAdapter {
  ...
  async chat(opts) {
    ...
    _log('debug', `chat REQUEST model=${model} messages=${messages?.length || 0} maxTokens=${maxTokens} jsonMode=${jsonMode}`);
    ...
    _log('debug', `chat RESPONSE model=${data.model || model} tokens=${data.usage?.prompt_tokens || 0}+${data.usage?.completion_tokens || 0} finish=${choice?.finish_reason}`);
  ...
  async *chatStream(opts) {
    ...
    _log('debug', `chatStream DONE model=${responseModel} chunks=${chunkCount} tokens=${usage.promptTokens}+${usage.completionTokens} finish=${finishReason}`);
  ...
  async embed(opts) {
    ...
    _log('debug', `embed RESPONSE model=${data.model || model} vectors=${data.data?.length || 0} tokens=${data.usage?.prompt_tokens || 0}`);
  ...
  async transcribe(opts) {
    ...
    _log('debug', `transcribe REQUEST model=${model} fileSize=${(bufSize / 1024).toFixed(1)}KB lang=${language || 'auto'} format=${responseFormat} granularities=${timestampGranularities?.join(',') || 'none'}`);
    ...
    if (responseFormat === 'json' || responseFormat === 'verbose_json') {
      const data = JSON.parse(res.body);
      _log('debug', `transcribe RESPONSE textLen=${(data.text || '').length} duration=${data.duration || 0}s words=${data.words?.length || 0} segments=${data.segments?.length || 0}`);
  ...
  async tts(opts) {
    ...
    _log('debug', `tts REQUEST model=${model} voice=${voice} inputLen=${input?.length || 0} format=${responseFormat}`);
    ...
    return new Promise((resolve, reject) => {
      ...
      req.write(JSON.stringify({ model, input, voice, response_format: responseFormat }));
      req.end();
    });
  }
  ...
  async imageEdit(opts) {
    ...
    _log('debug', `imageEdit REQUEST model=${model} size=${size} n=${n} imageSize=${(imageBuffer?.length / 1024).toFixed(1)}KB`);
  ...
  async imageGenerate(opts) {
    ...
    _log('debug', `imageGenerate REQUEST model=${model} n=${n} size=${size} quality=${quality} promptLen=${prompt?.length || 0}`);

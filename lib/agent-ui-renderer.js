...
function _renderDayView(spec) {
  ...
  // Contributor sections (weather tip, tasks, tickets, ...): { title, lines[] }.
  ...
  const typeColors = {
    ...
    // Non-meetings: neutral grey so blocks/declined visibly don't count
    // toward the spoken meeting tally.
    Block: 'background:rgba(148,163,184,0.12);color:#cbd5e1;border-color:rgba(148,163,184,0.25)',
    Declined: 'background:rgba(148,163,184,0.12);color:#94a3b8;border-color:rgba(148,163,184,0.2)',
  };
  ...
  const eventsHtml = events.map(ev => {
    let borderStyle = statusBorder[ev.status] || statusBorder.upcoming;
    // Muted rows (blocks / declined): dim the whole card so the timeline
    // visibly decomposes into the spoken count.
    if (ev.muted) borderStyle += ';opacity:0.5';
    const typeStyle = typeColors[ev.type] || typeColors.Work;

    // Conflict marker: an amber ring + explicit "Overlaps" line so a
    // double-booked slot is visible AT the row, not just tallied in a card.
    const conflictsWith = Array.isArray(ev.conflictsWith) ? ev.conflictsWith : [];
    const hasConflict = conflictsWith.length > 0;
    if (hasConflict) {
      borderStyle += ';border-color:rgba(248,113,113,0.55);box-shadow:0 0 0 1px rgba(248,113,113,0.28)';
    }
    const conflictBadge = hasConflict
      ? `<span style="display:inline-block;border-radius:9999px;border:1px solid rgba(248,113,113,0.4);background:rgba(248,113,113,0.12);padding:2px 8px;font-size:10px;font-weight:600;color:#fecaca;">⚠ Conflict</span>`
      : '';
    const conflictNote = hasConflict
      ? `<span style="color:#fca5a5;">Overlaps ${esc(conflictsWith.join(', '))}` +
        (ev.overlapMinutes ? ` · ${esc(String(ev.overlapMinutes))} min` : '') +
        `</span>`
      : '';
    ...
    // Back-to-back marker (distinct from a conflict): an amber "No break"
    // pill on the row that runs straight into the next one, plus the name
    // of what it runs into, so the tally on the insight card is visible AT
    // the transition (2026-09-15: "confused back-to-back with conflicts").
    const tightAfter = typeof ev.tightAfter === 'string' ? ev.tightAfter : '';
    const tightBefore = typeof ev.tightBefore === 'string' ? ev.tightBefore : '';
    const tightBadge = tightAfter && !hasConflict
      ? `<span style="display:inline-block;border-radius:9999px;border:1px solid rgba(251,191,36,0.4);background:rgba(251,191,36,0.12);padding:2px 8px;font-size:10px;font-weight:600;color:#fde68a;">No break</span>`
      : '';
    const tightNote = tightAfter
      ? `<span style="color:#fcd34d;">Straight into ${esc(tightAfter)}</span>`
      : tightBefore
        ? `<span style="color:#fcd34d;">Right after ${esc(tightBefore)}</span>`
        : '';
    // The compressed "who / where" line shared with the spoken brief.
    const whoWhere = [ev.where, ev.who].filter((s) => typeof s === 'string' && s.length > 0);
    const whoWhereHtml = whoWhere.length && !ev.location
      ? whoWhere.map((s) => `<span>${esc(s)}</span>`).join('')
      : (ev.who ? `<span>${esc(ev.who)}</span>` : '');
      ...
      `<div style="display:flex;align-items:flex-start;gap:10px;">` +
        ...
        `<div style="flex:1;min-width:0;">` +
          ...
          `<div style="margin-top:5px;display:flex;flex-wrap:wrap;gap:8px;font-size:11px;color:rgba(255,255,255,0.55);">` +
            (ev.location ? `<span>${esc(ev.where || ev.location)}</span>` : '') +
            whoWhereHtml +
  ...
  // ── Contributor sections (weather tip, tasks, tickets, feed, ...) ──
  const sectionsHtml = sections.length > 0
    ? sections.map((sec) => {
        const lines = Array.isArray(sec.lines) ? sec.lines.filter((l) => typeof l === 'string' && l.length > 0) : [];
        return `<div style="border-radius:20px;border:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.04);padding:14px 16px;box-shadow:0 4px 24px rgba(0,0,0,0.2);">` +
          `<div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:0.02em;">${esc(sec.title)}</div>` +
          (sec.headline ? `<div style="margin-top:6px;font-size:16px;font-weight:600;letter-spacing:-0.01em;">${esc(sec.headline)}</div>` : '') +
          (lines.length
            ? `<div style="margin-top:6px;display:flex;flex-direction:column;gap:4px;font-size:12px;line-height:1.5;color:rgba(255,255,255,0.75);">` +
...
<div class="dv" style="display:flex;flex-direction:column;gap:12px;">
  ...
  ${sectionsHtml ? `<div style="display:flex;flex-direction:column;gap:8px;">${sectionsHtml}</div>` : ''}

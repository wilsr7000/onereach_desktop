...
//
// Completed jobs are kept for history (status queries, UI lookup), but we
// cap the retention so a long-running install doesn't accumulate thousands
// of entries in memory and in the persisted JSON file.
// ---------------------------------------------------------------------------

/** @type {Map<string, Object>} */
const _jobs = new Map();

/** Maximum terminal jobs retained in memory + on disk */
const MAX_TERMINAL_JOBS = 100;

const TERMINAL_STATES = new Set(['completed', 'failed', 'cancelled']);

/**
 * Drop oldest terminal jobs when over MAX_TERMINAL_JOBS. Active jobs
...
function _pruneJobs() {
  ...
  // Sort ascending by completedAt or startedAt so we drop the oldest first
  terminal.sort(([, a], [, b]) => {
    const at = Date.parse(a.completedAt || a.startedAt || 0) || 0;
    const bt = Date.parse(b.completedAt || b.startedAt || 0) || 0;
    return at - bt;
  });
  const drop = terminal.length - MAX_TERMINAL_JOBS;
  for (let i = 0; i < drop; i++) {
    _jobs.delete(terminal[i][0]);
  }
}
...
/**
 * Tags reach us in two shapes: arrays (graph-native) and comma-joined
 * strings (the spaces-sync serialization). Normalize both to a string
 * array — a string `tags` here used to throw `tags.some is not a
 * function` and 500 the whole /playbook-qa route (2026-08-29).
 */
...
function _isPlaybook(item) {
  ...
  const tags = _normalizeTags(item.tags || item.metadata?.tags);
  ...
  // Content markers
  if (meta._playbookNoteId || meta.playbookNoteId) return true;
  if (content.includes('[playbook:')) return true;

  // WISER Playbooks (riff) metadata markers
  if (meta.source === 'riff' || meta.source === 'wiser-playbooks') return true;
  if (meta.type === 'playbook' || meta.contentType === 'playbook') return true;

  // Has structured framework (JSON)
  try {
    const parsed = JSON.parse(item.content);
    if (parsed.framework && parsed.framework.phases) return true;
    if (parsed.steps && Array.isArray(parsed.steps)) return true;
    if (parsed.type === 'playbook' || parsed.kind === 'playbook') return true;
  } catch (_ignored) {
    /* not valid JSON */
  }

  // Has step-like structure (## Steps / ## Phases / numbered list with bold)
  if (content.includes('## steps') && (content.includes('1.') || content.includes('- **'))) return true;
  if (content.includes('## phases') && (content.includes('1.') || content.includes('- **'))) return true;
 ...
 * @param {string}   [opts.query]        - Free-text query used to pick the right
 *                                         playbook when playbookId is omitted
 *                                         ("deploy" -> the deploy playbook, not
...
async function _executeJob(job, opts) {
  ...
  try {
    ...
    if (playbookId) {
      ...
      // With a query, pick the BEST match (title+content ranked) instead of
      // whichever playbook happens to list first.
      let chosenId = playbooks[0].id;
      if (job.query && playbooks.length > 1) {
        try {
          const { searchPlaybooks } = require('./playbook-search');
          const hits = await searchPlaybooks(job.query, { spaceId, limit: 1 });
          if (hits.length > 0) chosenId = hits[0].id;
        } catch (err) {
          log.warn('playbook-executor', 'Query resolution failed; using first playbook', {
            error: err.message,
          });
        }
      }
      playbookItem = await spacesAPI.items.get(spaceId, chosenId);
    }
    ...
    for (const item of assets) {
      try {
        ...
        assetManifest.push({
          ...
          tags: _normalizeTags(item.tags || item.metadata?.tags),
    ...
    // Build MCP config.
    //
    // Security: MCP servers run as child processes of the Claude Code CLI.
    // An attacker who can influence a Space's MCP data-source items could
    // otherwise set `command: 'curl'` (or any arbitrary binary) and
    // exfiltrate data or run code. We:
    //   1. Allow-list the commands we ship/support.
    //   2. Strip any secrets-looking env vars the source tries to inject;
    //      approved MCP servers should get their env from the app, not the
    //      playbook definition.
    //   3. Log every skipped MCP source so issues are visible.
    let mcpConfig = null;
    if (mcpSources.length > 0) {
      const ALLOWED_MCP_COMMANDS = new Set([
        'npx', // most published MCP servers (e.g. @modelcontextprotocol/server-*)
        'node', // locally-bundled servers shipped with the app
        'uvx', // Python MCP servers via uv
        'bunx', // bun-based MCP servers
      ]);

      mcpConfig = { mcpServers: {} };
      for (const mcp of mcpSources) {
        const cmd = typeof mcp.command === 'string' ? mcp.command.trim() : '';
        if (!cmd) {
          log.warn('playbook', 'Skipping MCP source: missing command', { name: mcp.serverName });
          continue;
        }
        if (!ALLOWED_MCP_COMMANDS.has(cmd)) {
          log.warn('playbook', 'Skipping MCP source: command not allow-listed', {
            name: mcp.serverName,
            command: cmd,
          });
          continue;
        }
        // Sanitize env: drop any value that looks like a credential the
        // playbook is trying to smuggle in. The app should inject real
        // credentials via its own credential-manager, not via data-source JSON.
        const cleanEnv = {};
        if (mcp.env && typeof mcp.env === 'object') {
          for (const [k, v] of Object.entries(mcp.env)) {
            if (typeof v !== 'string') continue;
            if (/apikey|secret|token|password/i.test(k)) {
              log.warn('playbook', 'Dropping sensitive-looking env var from MCP source', {
                name: mcp.serverName,
                envKey: k,
              });
              continue;
      ...
      // If everything got filtered, drop the config entirely
...
async function findPlaybooks(spaceId) {
  ...
  const allItems = await spacesAPI.items.list(spaceId, { includeContent: true });
  const playbooks = [];
  const seen = new Set();
...
module.exports = {
  ...
  findPlaybooks,
  onJobProgress,
  // The one playbook-detection heuristic. Exported so playbook-search (and
  // anything else) filters with the SAME rules instead of a drifting copy.
  isPlaybook: _isPlaybook,

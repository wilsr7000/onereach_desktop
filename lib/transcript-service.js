...
class TranscriptService extends EventEmitter {
  ...
  setPending(agentId, context) {
    ...
    // Timestamps live in a side map so the stored context stays exactly what
    // the caller provided (consumers deep-compare it).
    this._pendingSetAt = this._pendingSetAt || new Map();
    this._pendingSetAt.set(agentId, Date.now());
   ...
   */
  hasPending() {
    this._purgeStalePending();
    return this._pending.size > 0;
  }

  /**
   * Stale pending questions must never capture later, unrelated commands.
   * 2026-08-04 live: a ~58s-old pending from the daily brief hijacked
   * "Can you set an alarm for 6:32" and morphed it into a different task.
   * Real answers to a spoken question arrive within seconds.
   */
  _purgeStalePending(ttlMs = 45000) {
    if (!this._pendingSetAt) return;
    const now = Date.now();
    for (const [agentId, setAt] of this._pendingSetAt) {
      if (now - setAt > ttlMs) {
        log.info('voice', 'TranscriptService: pending input expired (stale)', { agentId, ageMs: now - setAt });
        this._pending.delete(agentId);
        this._pendingSetAt.delete(agentId);
      }
    }
  }
  ...
  clearPending(agentId) {
    this._pending.delete(agentId);
    if (this._pendingSetAt) this._pendingSetAt.delete(agentId);
  }

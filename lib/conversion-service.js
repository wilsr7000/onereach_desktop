...
class PipelineResolver {
  ...
  resolve(from, to) {
    ...
    while (queue.length > 0) {
      ...
      for (const [nextFmt, agentId] of neighbors.entries()) {
        if (nextFmt === to) {
          // Found path
...
class ConversionService {
  ...
  async convert(params) {
    ...
    if (agents.length > 0) {
      ...
      if (isAsync) {
        const jobId = this.jobs.create((onProgress) =>
          agent.convert(input, { ...options, onProgress })
        );
    ...
    if (isAsync) {
      const jobId = this.jobs.create((onProgress) =>
        this._executePipeline(input, pipeline, options, onProgress)
      );
      return { jobId, status: 'queued' };
    }

...
const { _app, shell, BrowserWindow, ipcMain } = require('electron');
...
function buildIDWAndGSXMenuItems(idwEnvironments, cachedData) {
  ...
  if (idwEnvironments && idwEnvironments.length > 0) {
    idwEnvironments.forEach((env, index) => {
      // Backfill `environment` for legacy entries (pre-agent-explorer
      // "Add IDW" form). Without this, the entry would be silently
      // dropped from the menu below -- the IDW exists in storage but
      // never reaches the user. menu-data-manager's validator now
      // backfills on save, but in-memory entries from older sessions
      // still pass through here; backfill again at render time so the
      // menu heals without a save cycle.
      if (env && !env.environment) {
        env.environment = 'custom';
      }
      log.info('menu', "Processing IDW Env : Label='', URL='', Environment", {
        index1: index + 1,
        env: env.label,
        env: env.chatUrl,
        env: env.environment,
      ...
      if (gsxLinks && gsxLinks.length > 0) {
        gsxLinks.forEach((link, linkIndex) => {
          log.info('menu', "Adding GSX submenu item : Label='', URL", {
            linkIndex1: linkIndex + 1,
            link: link.label,
            link: link.url,
          });
          gsxSubmenu.push({
            label: link.label,
            click: async () => {
              log.info('menu', 'GSX menu item clicked: (URL: , Env: )', {
                link: link.label,
                link: link.url,
                env: env.environment,
              });
...
function filterGSXLinksForEnvironment(allGsxLinks, env) {
  return allGsxLinks.filter((link) => {
    ...
    if (link.custom === true) {
      ...
      log.info('menu', "Custom link IDW match check: '' === ''", {
        linkIdwId: linkIdwId,
        envId: envId,
        idMatch: idMatch,
      });
    ...
    if (link.environment && env.environment && link.environment.toLowerCase() === env.environment.toLowerCase()) {
      if (link.idwId) {
        if (env.id && link.idwId === env.id) {
          log.info('menu', 'Standard link matches by idwId: (env=, idwId=)', {
            link: link.label,
        ...
        log.info('menu', 'Legacy link matches by environment only: (env=)', {
          link: link.label,
          link: link.environment,
        });
        return true;
    ...
    try {
      ...
      if (env.environment && url.hostname.includes(env.environment)) {
        log.info('menu', "GSX link URL hostname includes environment: hostname='', env", {
          url: url.hostname,
          env: env.environment,
        });
        match = true;
      ...
      if (
        ...
        log.info('menu', "GSX URL '' has no environment prefix, including it for all environments", {
          url: url.hostname,
        });
        match = true;
      }

      log.info('menu', "Checking GSX link: URL='', Host='', Matches Env=''?", {
        link: link.url,
        url: url.hostname,
        env: env.environment,
        match: match,
    ...
    if (audioByCategory[key].length > 0) {
      idwMenuItems.push({
        ...
        submenu: audioByCategory[key].map((generator) => ({
          ...
          click: async () => {
            log.info('menu', 'Opening audio generator in tab: at', {
    ...
    submenu: [
      {
        ...
        click: async () => {
          ...
          try {
            ...
            const envPrefix =
              result.summary.environment && result.summary.environment !== 'production'

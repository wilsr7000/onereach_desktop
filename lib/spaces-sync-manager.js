...
class SpacesSyncManager {
    ...
    if (triggerEvent === 'space:deleted') {
      ...
      if (triggerEvent === 'item:deleted' && triggerData?.itemId) {
        ...
        try {
          ...
          // Specialize WISER meeting artifacts: give the meeting/transcript/
          // recording distinct graph labels and draw the meeting->artifact
          // edges, so agents can traverse a meeting to its transcript +
          // recording instead of parsing ids out of JSON. Runs AFTER
          // upsertAsset so the meeting node exists to MATCH. Never fatal.
          await this._syncMeetingGraph(graph, spaceId, triggerData.item || {}, triggerData.itemId);
        } catch (e) {
  ...
  // Give a synced WISER meeting its distinct graph shape: the meeting item
  // becomes an :Asset:Meeting, its transcript/recording become :Transcript /
  // :Recording, and meeting-[:HAS_TRANSCRIPT|HAS_RECORDING]->artifact edges are
  // drawn. The meeting item is the single trigger because it already knows all
  // its artifact ids (meeting.post), so we don't have to detect transcripts /
  // recordings independently. Edge first (MERGEs the artifact node), then the
  // kind label (MATCHes it) -- order-independent w.r.t. when the artifact syncs.
  // Best-effort: any failure is logged, never thrown (this runs on every write).
  async _syncMeetingGraph(graph, spaceId, item, fallbackItemId) {
    const itemId = item.id || fallbackItemId;
    if (!itemId || typeof graph.setAssetKind !== 'function') return;

    // Gate on the meeting discriminator. `tags` is always carried on the sync
    // item (upsertAsset above reads it); `source` is checked too for safety.
    const tags = Array.isArray(item.tags) ? item.tags : [];
    const src = item.source || (item.metadata && item.metadata.source) || '';
    const isMeeting = src === 'wiser-meeting' || tags.includes('wiser-meeting');
    if (!isMeeting) return;

    try {
      await graph.setAssetKind(itemId, 'Meeting');

      // The meeting JSON (with post.transcriptItemId / recordingItemIds) may
      // not be in the lightweight sync event -- fetch the item if needed.
      let meeting = null;
      try {
        meeting = item.content ? JSON.parse(item.content) : null;
      } catch (_e) { /* not JSON in the event payload */ }
      if (!meeting || !meeting.post) {
        try {
          const { getSpacesAPI } = require('../spaces-api');
          const full = await getSpacesAPI().items.get(spaceId, itemId);
          if (full && full.content) meeting = JSON.parse(full.content);
        } catch (_e) { /* item fetch/parse failed -- labels applied, edges skipped */ }
      }
      const post = meeting && meeting.post;
      if (!post) return;

      if (post.transcriptItemId) {
        await graph.linkMeetingArtifact(itemId, post.transcriptItemId, 'HAS_TRANSCRIPT');
        await graph.setAssetKind(post.transcriptItemId, 'Transcript');
      }
      for (const recId of Array.isArray(post.recordingItemIds) ? post.recordingItemIds : []) {
        if (!recId) continue;
        await graph.linkMeetingArtifact(itemId, recId, 'HAS_RECORDING');
        await graph.setAssetKind(recId, 'Recording');
      }
    } catch (e) {
      log.warn('sync-manager', 'Meeting graph specialization failed', { itemId, error: e.message });
    }
  }

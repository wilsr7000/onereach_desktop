...
class LogEventQueue extends EventEmitter {
  ...
  enqueue(event) {
    ...
    // Flood dedupe (2026-08-14): the 08-12 incident wrote ~90 MB of one
    // repeating error line in 6.5s, rotating every older log away and
    // destroying the evidence of the failure that CAUSED it. Identical
    // (level|category|message) lines beyond the first 5 inside a rolling
    // 2s window are counted, not stored; the count is flushed as one
    // summary line when the storm breaks (different line or window
    // lapse). Span/step variety keeps normal operation un-deduped —
    // only true floods repeat an identical message this fast.
    {
      const key = level + '|' + (event.category || 'app') + '|' + String(event.message || '');
      const now = Date.now();
      const d = this._dedupe || (this._dedupe = { key: null, count: 0, firstAt: 0, sample: null });
      if (d.key === key && now - d.firstAt < 2000) {
        d.count += 1;
        if (d.count > 5) return null; // suppressed; summarized on flush
      } else {
        if (d.count > 5 && d.sample) {
          const suppressed = d.count - 5;
          // Recursion-safe: the summary line has a distinct message, so
          // it can never itself be deduped into silence.
          this._emitEntry({
            level: d.sample.level,
            category: d.sample.category,
            message: `${d.sample.message} [log-dedupe: repeated ×${suppressed} more in ${now - d.firstAt}ms]`,
            data: {},
          });
        }
        d.key = key;
        d.count = 1;
        d.firstAt = now;
        d.sample = { level, category: event.category || 'app', message: String(event.message || '') };
      }
    }

    return this._emitEntry({ ...event, level });
  }

  /** Build + fan out one entry (the pre-dedupe tail of enqueue()). */
  _emitEntry(event) {
    const entry = {
      id: this._generateId(),
  ...
  query(opts = {}) {
    ...
    if (search) {
      ...
      results = results.filter(
        (e) => e.message.toLowerCase().includes(term) || JSON.stringify(e.data).toLowerCase().includes(term)
      );
    }
    if (since) {
  ...
  async export(opts = {}) {
    ...
    if (format === 'text') {
      return results
        .map((e) => `[${e.timestamp}] ${e.level.toUpperCase()} [${e.category}] ${e.message} ${JSON.stringify(e.data)}`)
        .join('\n');
    }
    return results;
  }
          ...
          if (extraData) {
            ...
            if (serialized.length > 65536) {
              extraData = {
                ...
                _reason: 'entry exceeded 64 KB file-write limit',
              };
            }
          }
  ...
  /**
   * Capture lightweight situational context for a log entry.
   * Uses a cached snapshot refreshed at most every 2 seconds to avoid
   * per-log overhead and circular-require issues with gsx-flow-context.
   */
  _captureContext() {
    const now = Date.now();
    if (this._contextCache && now - this._contextCacheTime < 2000) {
      return { ...this._contextCache };
    }

    const ctx = { focusedWindow: null, flowId: null, stepId: null };
    try {
      if (global.windowRegistry) {
        const list = global.windowRegistry.list();
        const focused = list.find(w => w.focused && w.alive);
        ctx.focusedWindow = focused ? focused.name : null;
      }
    } catch (_) { /* ignore */ }
    try {
      if (!this._flowCtxModule) {
        this._flowCtxModule = require('./gsx-flow-context');
      }
      const c = this._flowCtxModule.get();
      if (c) {
        ctx.flowId = c.flowId || null;
        ctx.stepId = c.stepId || null;
      }
    } catch (_) { /* ignore */ }

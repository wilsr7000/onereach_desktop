...
const WRITE_KEYWORD_RE = /\b(CREATE|MERGE|SET|DELETE|REMOVE|DROP|DETACH|FOREACH|LOAD\s+CSV|CALL\s*\{)\b/i;

/** Strip string literals so a title like "Set up" cannot trip the write check. */
function stripStringLiterals(cypher) {
  return String(cypher).replace(/'(?:[^'\\]|\\.)*'|"(?:[^"\\]|\\.)*"/g, "''");
}

/** True when the statement carries no write keyword outside string literals. */
function isReadOnlyCypher(cypher) {
  if (typeof cypher !== 'string' || !cypher.trim()) return false;
  const bare = stripStringLiterals(cypher).replace(/\/\*[\s\S]*?\*\//g, '');

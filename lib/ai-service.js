...
// Agent identity resolver -- lazy-loads budget-manager for a shared
// derivation between the cost tracker and the ai-service layer. Keeps the
// dependency optional so unit tests can load this module without Electron.
let _budgetModule = null;
function _resolveAgentIdentity(opts = {}) {
  try {
    if (!_budgetModule) _budgetModule = require('../budget-manager');
    if (_budgetModule && typeof _budgetModule.deriveAgent === 'function') {
      return _budgetModule.deriveAgent(opts);
    }
  } catch (_err) {
    // Ignore; fall through to inline fallback.
  }
  const rawId = (opts.agentId || '').trim();
  if (rawId) return { agentId: rawId, agentName: (opts.agentName || rawId).trim() };
...
// AI pause kill switch -- lazy-loaded to avoid circular requires.
// Any caller (IPC or in-process) hits this guard before a provider call.
let _aiPauseMod = null;
function _checkAiPause() {
  try {
    if (!_aiPauseMod) _aiPauseMod = require('./ai-pause');
    if (_aiPauseMod.isPaused()) {
      const err = new Error(`AI traffic is paused (${_aiPauseMod.getStatus().reason || 'manual'})`);
      err.code = 'AI_PAUSED';
      err.paused = true;
      throw err;
    }
  } catch (e) {
    if (e && e.code === 'AI_PAUSED') throw e;
    // Module not available yet (e.g. during early init); fail open.
  }
}
...
const DEFAULT_MODEL_PROFILES = {
  // fast = the auction's bidder and other utility calls. It has to land
  // inside the 6 s bid window (lib/exchange/voice-coordinator.js): Fable 5.1
  // measured 5.2-6.4 s per bid on 2026-09-15 and the first voice request
  // after launch closed with "no bids"; Haiku 4.5 answers in ~2-3 s.
  fast: {
    provider: 'anthropic',
  ...
  large: {
    provider: 'openai',
    model: 'gpt-4o',
    fallback: { provider: 'anthropic', model: 'claude-fable-5-1' },
  },
  vision: {
    provider: 'anthropic',
    model: 'claude-fable-5-1',
...
class AIService {
   ...
   */
  _recordUsage(provider, model, usage, feature, profileName, duration, success = true, agentIdentity = null) {
    ...
    if (tracker) {
      try {
        ...
        tracker[trackFn]({
          ...
          agentName: agentIdentity?.agentName || null,
          success,
          duration,
        });
      } catch (err) {
  ...
  async _executeWithResilience(callType, profile, primaryOpts, buildOpts) {
    ...
    const agentIdentity = _resolveAgentIdentity(primaryOpts);
    ...
    if (fallback && !primaryOpts.noFallback) {
      ...
      if (!fbCircuit || !fbCircuit.isOpen()) {
        try {
          ...
          const duration = Date.now() - startTime;
          const cost = this._recordUsage(fbProvider, fbModel, result.usage || {}, feature, profileName, duration, true, agentIdentity);
  ...
  /**
   * Chat with automatic tool-use loop.
   *
   * Calls chat() with tool definitions. When the model returns toolCalls instead
   * of text, executes each tool via the onToolCall callback, appends results,
   * and loops until the model returns a final text response.
   *
   * @param {Object} opts
   * @param {Array} opts.messages - Conversation messages
   * @param {Array} opts.tools - Tool definitions [{name, description, inputSchema, execute?}]
   * @param {Function} opts.onToolCall - async (name, input) => string|object -- executes the tool
   * @param {string} [opts.system] - System prompt
   * @param {string} [opts.profile] - Model profile
   * @param {number} [opts.maxToolRounds=10] - Safety cap on tool-use rounds
   * @param {number} [opts.maxTokens] - Max tokens per LLM call
   * @param {number} [opts.temperature] - Sampling temperature
   * @param {string} [opts.feature] - Cost tracking feature name
   * @param {Function} [opts.onProgress] - Called with {round, toolName, toolInput} on each tool call
   * @returns {Promise<Object>} Final response with {content, usage, model, provider, toolCallLog}
   */
  async chatWithTools(opts) {
    ...
    if (!onToolCall) throw new Error('chatWithTools requires an onToolCall callback');
    if (!tools || tools.length === 0) throw new Error('chatWithTools requires at least one tool');

    // Build LLM-facing tool definitions (strip execute functions)
    ...
    for (let round = 0; round < maxToolRounds; round++) {
      const response = await this.chat({
        ...
        maxTokens,
        temperature,
        feature: feature ? `${feature}:tool-round-${round}` : undefined,
      });
      ...
      // Anthropic: append the assistant's full response (with tool_use blocks) to messages
      if (response.rawContent) {
        messages.push({ role: 'assistant', content: response.rawContent });
      } else if (response.rawMessage) {
      ...
      for (const tc of response.toolCalls) {
        if (onProgress) {
          try { onProgress({ round, toolName: tc.name, toolInput: tc.input }); } catch (_) { /* ignore */ }
        }

        let toolResult;
        try {
          const raw = await onToolCall(tc.name, tc.input);
          toolResult = typeof raw === 'string' ? raw : JSON.stringify(raw);
        } catch (err) {
          toolResult = `Tool error: ${err.message}`;
        }

        toolCallLog.push({ round, id: tc.id, name: tc.name, input: tc.input, result: toolResult.slice(0, 2000) });

        // Append tool result in the provider-appropriate format
        if (response.provider === 'anthropic' || response.rawContent) {
          messages.push({
            role: 'user',
  ...
  async transcribe(audioBuffer, opts = {}) {
    ...
    const duration = Date.now() - startTime;
    this._recordUsage(provider, model, {}, feature, profileName, duration, true, _resolveAgentIdentity(opts));
  ...
  async tts(text, opts = {}) {
    ...
    const duration = Date.now() - startTime;
    this._recordUsage('openai', model, {}, opts.feature || 'tts', 'tts', duration, true, _resolveAgentIdentity(opts));
  ...
  async imageEdit(imageBuffer, prompt, opts = {}) {
    ...
    const duration = Date.now() - startTime;
    this._recordUsage('openai', opts.model || 'gpt-image-1', {}, feature, 'image-edit', duration, true, _resolveAgentIdentity(opts));
  ...
  async imageGenerate(prompt, opts = {}) {
    ...
    const duration = Date.now() - startTime;
    this._recordUsage('openai', model, {}, feature, 'image-generation', duration, true, _resolveAgentIdentity(opts));

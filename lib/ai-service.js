...
class CircuitBreaker {
  ...
  onFailure(error) {
    ...
    if (this.failureCount >= this.failureThreshold) {
      this.state = 'open';
      console.error(`[AIService] Circuit breaker OPEN for ${this.name} after ${this.failureCount} consecutive failures`);
    }
  }
...
class AIService {
  ...
  _getApiKey(provider) {
    ...
    if (provider === 'anthropic') {
      ...
      const rawKey = settings.get('anthropicApiKey') ||
    ...
    try {
      ...
      if (result.blocked) {
        ...
        throw new BudgetExceededError(
          `Budget limit reached. ${result.warnings?.map(w => w.message).join('. ')}`
        );
      }

      if (result.warnings?.length > 0) {
        _log('warn', 'budget', `Budget warning for ${provider}/${model}: ${result.warnings.map(w => w.message).join('; ')}`);
        const logger = this._getLogger();
  ...
  _recordUsage(provider, model, usage, feature, profileName, duration, success = true) {
    ...
    try {
      ...
      _log('info', 'usage', `${provider}/${model} profile=${profileName} feature=${feature} ` +
    ...
    for (let attempt = 0; attempt <= retryConfig.maxRetries; attempt++) {
      ...
      try {
        ...
        if (err.statusCode === 429) {
          _log('error', 'rate-limit', `RATE LIMIT HIT (${opts.model || 'unknown'}): retrying in ${delay}ms. Retry-After: ${err.retryAfter || 'none'}. This should not happen routinely -- if frequent, reduce concurrency or upgrade API tier.`);
          // Also push to structured log so it shows on dashboards
          try {
            const logger = this._getLogger();
            if (logger?.error) logger.error('AI rate limit hit', {
  ...
  async _executeWithResilience(callType, profile, primaryOpts, buildOpts) {
    ...
    _log('debug', 'resilience', `${callType} -> ${primaryProvider}/${primaryModel} (profile=${profileName}, feature=${feature})`);
    // Log to app log queue for observability
    this._getLogger().info(`AI ${callType}: ${primaryProvider}/${primaryModel}`, { event: 'ai-service:call', profile: profileName, provider: primaryProvider, model: primaryModel, feature });
    ...
    if (!primaryCircuit || !primaryCircuit.isOpen()) {
      try {
        ...
        const cost = this._recordUsage(primaryProvider, primaryModel, result.usage || {}, feature, profileName, duration);
        ...
        _log('warn', 'resilience', `Primary ${primaryProvider}/${primaryModel} failed: ${err.message} (circuit=${primaryCircuit?.state})`);
   ...
   */
  async chat(opts) {
    _log('debug', 'chat', `messages=${opts.messages?.length || 0} feature=${opts.feature || 'none'} profile=${opts.profile || 'default'}`);
    const profile = this._resolveProfile(opts);

    // Budget check (estimate)
    const msgText = opts.messages?.map(m => typeof m.content === 'string' ? m.content : JSON.stringify(m.content)).join(' ') || '';
  ...
  async *chatStream(opts) {
    _log('debug', 'chatStream', `messages=${opts.messages?.length || 0} feature=${opts.feature || 'none'} profile=${opts.profile || 'default'}`);
    const profile = this._resolveProfile(opts);
  ...
  async vision(imageData, prompt, opts = {}) {
    _log('debug', 'vision', `promptLen=${prompt?.length || 0} feature=${opts.feature || 'none'} imageDataLen=${typeof imageData === 'string' ? imageData.length : 'buffer'}`);
  ...
  async embed(input, opts = {}) {
    ...
    this._recordUsage(provider, model, { promptTokens: result.usage?.promptTokens || 0 }, feature, profileName, duration);
  ...
  async transcribe(audioBuffer, opts = {}) {
    ...
    _log('debug', 'transcribe', `audioSize=${(bufSize / 1024).toFixed(1)}KB lang=${opts.language || 'auto'} format=${opts.responseFormat || 'json'} feature=${opts.feature || 'none'}`);
    ...
    _log('info', 'transcribe', `Completed in ${duration}ms textLen=${result.text?.length || 0} words=${result.words?.length || 0}`);
    return result;
  }
   ...
   */
  async tts(text, opts = {}) {
    _log('debug', 'tts', `textLen=${text?.length || 0} voice=${opts.voice || 'alloy'} feature=${opts.feature || 'none'}`);
  ...
  async imageEdit(imageBuffer, prompt, opts = {}) {
    _log('debug', 'imageEdit', `promptLen=${prompt?.length || 0} imageSize=${imageBuffer?.length || 0} feature=${opts.feature || 'none'}`);
    const adapter = this._adapters.openai;
  ...
  async imageGenerate(prompt, opts = {}) {
    _log('debug', 'imageGenerate', `prompt="${_truncate(prompt, 60)}" model=${opts.model || 'dall-e-3'} size=${opts.size || '1024x1024'} quality=${opts.quality || 'standard'} feature=${opts.feature || 'none'}`);
    const adapter = this._adapters.openai;
  ...
  async planAgent(description, availableTemplates = {}, opts = {}) {
    _log('debug', 'planAgent', `description="${_truncate(description, 60)}" templates=${Object.keys(availableTemplates).length}`);

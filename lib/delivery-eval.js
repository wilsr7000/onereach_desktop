...
function computeVerdict(f) {
  ...
  if (f.speakResult !== true) return 'silent-failure';
  // Speech confirmed -- but if a modal panel was promised and never spawned,
  // the user heard "brief on screen" with no window. Grade it (only when the
  // caller supplied panel facts; absent facts skip panel grading).
  if (f.panelPromised === true && f.panelShown === false) return 'partial-no-panel';
  return 'delivered';
}
...
function evaluateDelivery(facts = {}) {
  ...
  try {
    ...
    const data = {
      ...
      panelPromised: facts.panelPromised === true,
      panelShown: facts.panelShown !== false,
    ...
    if (facts.taskId && !_fallbackSpokenFor.has(facts.taskId)) {
      ...
      if (speaker && typeof speaker.speak === 'function') {
        const fallbackText =
          verdict === 'partial-no-panel'
            ? "Sorry — I couldn't open the results window. The answer is in the chat panel."
            : "Sorry — I finished your request but couldn't deliver the answer. It's in the chat panel.";
        Promise.resolve(
          speaker.speak(fallbackText, { taskResult: true, skipAffectMatching: true })
        )
          .then((ok) =>

  ...
  async optimizePosition(newTask, existingTasks) {
    ...
    if (this.useAI) {
      try {
        ...
        console.error('AI optimization failed, using heuristics:', error);
      }
    }
...
${existingTasks.slice(0, 10).map((t, i) => `${i}: ${t.description} (priority: ${t.queueMetadata?.priority}, type: ${t.type})`).join('\n')}

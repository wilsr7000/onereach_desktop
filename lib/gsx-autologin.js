...
async function attemptGSXAutoLogin(gsxWindow, url, state, environment) {
  ...
  try {
    ...
    if (!gsxWindow || gsxWindow.isDestroyed()) {
      logger.warn('[Auth GSX] Window destroyed before login', { event: 'auth:gsx-window-destroyed', environment, feature: 'auto-login' });
      return;
    }
    ...
    if (!rateCheck.allowed) {
      logger.info('[Auth GSX] Rate limited', { event: 'auth:gsx-rate-limited', environment, waitMs: rateCheck.waitMs, feature: 'auto-login' });
    ...
    if (!credentials || !credentials.email || !credentials.password) {
      logger.warn('[Auth GSX] No credentials configured', { event: 'auth:gsx-no-credentials', environment, feature: 'auto-login' });
    ...
    logger.info('[Auth GSX] Page type detected', { event: 'auth:gsx-page-type', environment, isLogin: !!pageType.isLoginPage, is2FA: !!pageType.is2FAPage, reason: pageType.reason, feature: 'auto-login' });
    ...
    if (pageType.isLoginPage) {
      ...
      logger.info('[Auth GSX] Login form fill result', { event: 'auth:gsx-form-fill', success: !!(result && result.success), environment, feature: 'auto-login' });
      
      // Verify fill succeeded
      if (!result || !result.success) {
        logger.warn('[Auth GSX] Form fill verification failed', { event: 'auth:gsx-fill-failed', environment, feature: 'auto-login' });
        return 'no_form'; // Signal retry
      }
      
      // Click submit button in separate step for reliability
      await sleep(100);
      await safeFrameExecute(gsxWindow, authFrame, `
        (function() {
    ...
    if (err instanceof WindowDestroyedError) {
      logger.info('[Auth GSX] Window closed during login', { event: 'auth:gsx-window-closed', environment, feature: 'auto-login' });
      return;
    }
    
    logger.error('[Auth GSX] Login error', { event: 'auth:gsx-error', environment, error: err.message, feature: 'auto-login' });
...
async function handleGSX2FA(gsxWindow, state, environment, attempt = 0) {
  ...
  try {
    ...
    if (attempt >= MAX_ATTEMPTS) {
      logger.warn('[Auth GSX] 2FA max attempts exhausted', { event: 'auth:gsx-2fa-exhausted', environment, maxAttempts: MAX_ATTEMPTS, feature: 'auto-login' });
      recordLoginAttempt(environment, false);
    ...
    logger.info(`[Auth GSX] 2FA attempt ${attempt + 1}/${MAX_ATTEMPTS}`, { event: 'auth:gsx-2fa-attempt', environment, attempt: attempt + 1, maxAttempts: MAX_ATTEMPTS, feature: 'auto-login' });
    ...
    if (!authFrame) {
      logger.info('[Auth GSX] Auth frame gone - login likely succeeded', { event: 'auth:gsx-2fa-frame-lost', environment, feature: 'auto-login' });
    ...
    if (formCheck.hasRateLimit) {
      logger.warn('[Auth GSX] Server rate limit detected', { event: 'auth:gsx-rate-limit-server', environment, feature: 'auto-login' });
      recordLoginAttempt(environment, false);
    ...
    if (formCheck.hasError && attempt > 0) {
      logger.warn('[Auth GSX] Invalid 2FA code, waiting for fresh code', { event: 'auth:gsx-2fa-invalid', environment, attempt, feature: 'auto-login' });
    ...
    if (!totpSecret) {
      logger.warn('[Auth GSX] No TOTP secret configured', { event: 'auth:gsx-no-totp', environment, feature: 'auto-login' });
    ...
    if (timeRemaining < 5) {
      logger.info('[Auth GSX] TOTP near expiration, waiting for fresh code', { event: 'auth:gsx-totp-wait', environment, timeRemaining, feature: 'auto-login' });
    ...
    try {
      ...
      logger.info('[Auth GSX] TOTP code generated', { event: 'auth:gsx-totp-generated', environment, feature: 'auto-login' });
    } catch (err) {
      logger.error('[Auth GSX] TOTP generation failed', { event: 'auth:gsx-totp-error', environment, error: err.message, feature: 'auto-login' });
    ...
    logger.info('[Auth GSX] 2FA fill result', { event: 'auth:gsx-2fa-fill', environment, success: !!(result && result.success), verified: result?.verified, reason: result?.reason, feature: 'auto-login' });
    
    // Verify fill succeeded before clicking submit
    if (!result || !result.success) {
      logger.warn('[Auth GSX] 2FA fill failed', { event: 'auth:gsx-2fa-fill-failed', environment, reason: result?.reason, feature: 'auto-login' });
      await sleep(500);
      if (!gsxWindow.isDestroyed()) {
        await handleGSX2FA(gsxWindow, state, environment, attempt + 1);
      }
    ...
    if (!gsxWindow.isDestroyed()) {
      ...
      if (stillOnAuth.stillOnPage && stillOnAuth.hasError) {
        logger.warn('[Auth GSX] 2FA error on page, retrying', { event: 'auth:gsx-2fa-rejected', environment, attempt, feature: 'auto-login' });
        await handleGSX2FA(gsxWindow, state, environment, attempt + 1);
        return;
      }
    }
    ...
    // Success!
    logger.info('[Auth GSX] 2FA completed, login successful', { event: 'auth:gsx-complete', environment, attempts: attempt + 1, feature: 'auto-login' });
    recordLoginAttempt(environment, true);
    await hideStatusOverlay(gsxWindow, true);
    state.complete = true;
    state.inProgress = false;
    ...
    if (err instanceof WindowDestroyedError) {
      logger.info('[Auth GSX] Window closed during 2FA', { event: 'auth:gsx-2fa-window-closed', environment, feature: 'auto-login' });
      return;
    }
    
    logger.error('[Auth GSX] 2FA error', { event: 'auth:gsx-2fa-error', environment, error: err.message, attempt, feature: 'auto-login' });
    
    // Retry on transient errors
    if (attempt < MAX_ATTEMPTS - 1 && !gsxWindow.isDestroyed()) {
      logger.info('[Auth GSX] Retrying 2FA after error', { event: 'auth:gsx-2fa-retry', environment, attempt: attempt + 1, feature: 'auto-login' });
      await sleep(500);
      await handleGSX2FA(gsxWindow, state, environment, attempt + 1);
      return;
    }
...
async function openGSXLargeWindow(url, title, windowTitle, loadingMessage = 'Loading...', idwEnvironment = null) {
  ...
  if (idwEnvironment) {
    ...
    if (injectionResult.success) {
      log.info('app', 'Token injection successful: cookies on', { injectionResult: injectionResult.cookieCount, injectionResult: injectionResult.domains.join(', ') });
    } else if (injectionResult.error) {
      log.warn('app', 'Token injection issue: - user may need to login manually', { injectionResult: injectionResult.error });
    }
  }
    ...
    gsxWindow.webContents.on('did-navigate', async (event, navUrl) => {
      ...
      if (navUrl && navUrl.includes('auth.') && navUrl.includes('onereach.ai')) {
        ...
        const attemptWithRetries = async (attempt = 0) => {
          ...
          log.info('app', 'Attempt / after ms delay', { attempt1: attempt + 1, maxAttempts: maxAttempts, delay: delay });
  ...
  gsxWindow.webContents.on('did-fail-load', (event, errorCode, errorDescription, validatedURL) => {
    log.info('app', 'Load failed: - for', { errorCode: errorCode, errorDescription: errorDescription, validatedURL: validatedURL });

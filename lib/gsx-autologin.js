...
// Shared selectors for auth form detection
 ...
 */
async function waitForAuthFrame(gsxWindow, timeoutMs = 5000) {
...
async function waitForAuthForm(gsxWindow, frame, timeoutMs = 10000) {
  const totpSel = JSON.stringify(AUTH_SELECTORS.totp);
  const emailSel = JSON.stringify(AUTH_SELECTORS.email);
  const pwSel = JSON.stringify(AUTH_SELECTORS.password);

  const script = `
    new Promise(function(resolve) {
      var TOTP_SEL = ${totpSel};
      var EMAIL_SEL = ${emailSel};
      var PW_SEL = ${pwSel};
 ...
 */
async function waitForPostSubmitTransition(gsxWindow, originalAuthFrame, timeoutMs = 8000) {
  const start = Date.now();
  await sleep(200);
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    try {
      const state = await safeFrameExecute(
        ...
        `
        (function() {
          var totpInput = document.querySelector(${JSON.stringify(AUTH_SELECTORS.totp)});
          var errorEl   = document.querySelector(${JSON.stringify(AUTH_SELECTORS.error)});
...
async function attemptGSXAutoLogin(gsxWindow, url, state, environment) {
  ...
  try {
    ...
    if (pageType.isLoginPage) {
      ...
      // Wait for page transition after submit (2FA page, error, or redirect)
      if (!gsxWindow.isDestroyed()) {
        const postSubmit = await waitForPostSubmitTransition(gsxWindow, authFrame);
...
async function handleGSX2FA(gsxWindow, state, environment, attempt = 0) {
  ...
  try {
    ...
    // Wait for 2FA submission result (auth frame disappears on success)
    const postSubmit = await waitForPostSubmitTransition(gsxWindow, authFrame);

...
async function buildAgentWithClaudeCode(userRequest, opts = {}) {
  ...
  try {
    ...
    try {
      if (opts.playbookMarkdown && String(opts.playbookMarkdown).trim()) {
        // USER-AUTHORED playbook (created in WISER Playbooks and saved to
        // Spaces): it IS the authoritative spec — use it verbatim, don't
        // compose over it or re-save a duplicate.
        const markdown = String(opts.playbookMarkdown);
        const heading = markdown.match(/^#\s+(.+)$/m);
        result.playbook = {
          markdown,
          title: (heading && heading[1].trim()) || 'User Playbook',
          saved: true, // lives where the user authored it
          ref: opts.playbookRef || null,
          authored: true,
        };
      } else {

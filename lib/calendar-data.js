...
function analyzeDay(events, targetDate, now) {
  ...
  return {
    date: dayStart,
    // LOCAL day key: dayStart is local midnight; a UTC slice would name the
    // previous day in UTC-positive timezones.
    dateStr: require('./calendar-format').localDateKey(dayStart),
    label,
    isToday,
    isPast: isPastDay,

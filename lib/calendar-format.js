...
// ────────────────────────────────────────────────────────────────────────────
// Truthful meeting counting — ONE vocabulary for voice AND UI
// ────────────────────────────────────────────────────────────────────────────
// The 2026-08 "said 3, showed 7" mismatch: the spoken brief filtered blocks/
// declined events and counted what's LEFT today, while the dayView glance
// card and timeline used the raw timed-event tally. Both channels now derive
// their numbers from this single classifier.

// The vocabulary itself (block titles, the classifier, conflict vs
// back-to-back, per-event compression) lives in lib/brief-items.js so the
// store, this layer and the agents all share ONE definition. Re-exported
...
/**
 * LOCAL calendar-day key (YYYY-MM-DD). Never use toISOString().slice(0,10)
 * for day identity: that's the UTC day, which after 5 PM Pacific names
 * TOMORROW — the "today is 2026-08-04" evening bug class.
 */
 ...
 */
function buildDayViewSpec(briefData, briefingText, extras = {}) {
  const now = new Date();
  // Extra sections (weather tip, tasks, tickets, ...) come from the other
  // brief contributors as { title, lines[] } and render as compact cards, so
  // what the voice says about them is also on screen.
  const sections = Array.isArray(extras.sections) ? extras.sections.filter((s) => s && s.title) : [];

  if (!briefData || !briefData.timeline || briefData.timeline.length === 0) {
    // "Clear day" is only true when the calendar ANSWERED with no events. When
    // it failed or timed out the card must say so, not claim a free day.
    const unavailable = !!extras.calendarUnavailable;
    return {
      type: 'dayView',
      ...
      insightCards: [
        unavailable
          ? { title: 'Today at a glance', value: 'Calendar unavailable', sub: 'Could not read your calendar' }
          : { title: 'Today at a glance', value: 'Clear day', sub: 'No meetings scheduled' },
      ],
      sections,
      briefing: briefingText ? _splitBriefing(briefingText) : [unavailable ? 'Your calendar could not be read.' : 'Your calendar is clear today.'],
  ...
  // "Up next" is the next real meeting -- a hold ("Don't book") or a
  // declined invite is never up next.
  const nextIdx = timeline.findIndex(
    ev => ev.status === 'upcoming' && !BLOCK_TITLE_RE.test(ev.title || '') && !ev.selfDeclined
  );

  // Conflict + back-to-back map: which timeline rows collide (and with
  // what) and which run straight into the next one. briefData.conflicts is
  // [{ event1: <title>, event2: <title>, overlapMinutes, time }] and
  // briefData.backToBack is [{ first, second, transitionTime }]. Before
  // this, conflicts existed ONLY as a summary insight card while the
  // timeline rendered the colliding meetings as ordinary rows (2026-08-05:
  // "the visual for my daily brief did not handle conflicts"); back-to-backs
  // were a bare count with no row marker at all (2026-09-15).
  const flagIndex = indexFlagsByTitle(briefData);
  const conflictsByTitle = flagIndex.conflicts;
  const items = compressTimeline(timeline, { flags: briefData, selfEmail: extras.selfEmail });
  ...
  const events = timeline.map((ev, i) => {
    ...
    // Blocks and declined events stay ON the timeline (it shows the whole
    // day) but are labeled and muted so the rows visibly decompose into
    // the spoken count: "3 left" + done + blocks/declined = all rows.
    const isBlock = BLOCK_TITLE_RE.test(ev.title || '');
    const isDeclined = !isBlock && !!ev.selfDeclined;
    const type = isBlock ? 'Block' : isDeclined ? 'Declined' : _inferEventType(ev, guestCount);

    // A block or a meeting you declined isn't a real double-booking, and
    // never "runs into" anything.
    const conflict = isBlock || isDeclined ? null : conflictsByTitle.get(ev.title) || null;
    const tightAfter = isBlock || isDeclined ? null : flagIndex.b2bFirst.get(ev.title) || null;
    const tightBefore = isBlock || isDeclined ? null : flagIndex.b2bSecond.get(ev.title) || null;
    const item = items[i];
    ...
    return {
      ...
      status,
      note,
      muted: isBlock || isDeclined,
      conflictsWith: conflict ? conflict.others : undefined,
      overlapMinutes: conflict ? conflict.overlapMinutes : undefined,
      // Back-to-back markers: this row runs straight into `tightAfter` /
      // starts the moment `tightBefore` ends. Distinct from a conflict.
      tightAfter: tightAfter || undefined,
      tightBefore: tightBefore || undefined,
      // The one-line compression shared with the spoken brief.
      line: item ? item.line : undefined,
      spoken: item ? item.spoken : undefined,
      where: item ? item.where : undefined,
      who: item ? item.who : undefined,
    };
  });
...
function _buildInsightCards(briefData) {
  ...
  // The glance card speaks the SAME language as the spoken brief
  // (classifyBriefTimeline): real meetings left/done, with blocks and
  // declined events called out separately instead of inflating the count.
  const cls = classifyBriefTimeline(briefData.timeline || []);
  const isToday = briefData.isToday !== false;
  const left = cls.upcoming.length;
  const done = cls.completed.length;
  const extras = cls.blocks.length + cls.declined.length;

  let glanceValue;
  if (cls.meetings.length === 0) {
    glanceValue = 'No meetings';
  } else if (!isToday) {
    glanceValue = `${cls.meetings.length} meeting${cls.meetings.length !== 1 ? 's' : ''}`;
  } else if (left === 0) {
    glanceValue = `All ${done} done`;
  } else {
    glanceValue = `${left} meeting${left !== 1 ? 's' : ''} left`;
  }

  const totalHrs = freeTime.busyHours || 0;
  const freeSlots = (freeTime.freeSlots || []).length;
  const subParts = [];
  if (isToday && left > 0 && done > 0) subParts.push(`${done} done`);
  if (extras > 0) subParts.push(`${extras} block${extras !== 1 ? 's' : ''}/declined`);
  subParts.push(`${totalHrs} hrs scheduled \u00B7 ${freeSlots} free slot${freeSlots !== 1 ? 's' : ''}`);
  ...
  // Conflict = overlap; back-to-back = no break between (NOT a conflict).
  // Both are named on the card so the screen says exactly what the voice
  // says, with the pairs spelled out.
  const conflicts = briefData.conflicts || [];
  const backToBack = briefData.backToBack || [];
  const b2bNames = backToBack
    .map((p) => {
      const a = typeof p.first === 'string' ? p.first : p.first && p.first.title;
      const b = typeof p.second === 'string' ? p.second : p.second && p.second.title;
      return a && b ? `${a} \u2192 ${b}` : null;
    })
    .filter(Boolean);
  const b2bText = backToBack.length > 0
    ? `${backToBack.length} back-to-back${b2bNames.length ? ` (${b2bNames.join('; ')})` : ''}`
    : '';
  if (conflicts.length > 0) {
    const named = conflicts.map((c) => {
      const a = typeof c.event1 === 'string' ? c.event1 : c.event1 && c.event1.title;
      const b = typeof c.event2 === 'string' ? c.event2 : c.event2 && c.event2.title;
      return `${a} / ${b}${c.overlapMinutes ? ` (${c.overlapMinutes} min)` : ''}`;
    });
    cards.push({
      title: 'Conflict check',
      value: `${conflicts.length} overlap${conflicts.length > 1 ? 's' : ''}`,
      sub: [named.join(', '), b2bText].filter(Boolean).join(' \u00B7 '),
    });
  } else {
...
module.exports = {
  ...
  // Truthful counting (shared by spoken brief + dayView glance card)
  classifyBriefTimeline,
  BLOCK_TITLE_RE,

  // Local-day identity (never UTC-slice a local calendar day)
  localDateKey,

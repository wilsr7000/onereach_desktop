 ...
 *   meetings  = real meetings (not blocks, not declined)
 *   upcoming  = meetings still ahead (includes in-progress)
 *   completed = meetings already finished
 */
function classifyBriefTimeline(timeline) {
  const entries = Array.isArray(timeline) ? timeline : [];
  const isBlock = (e) => BLOCK_TITLE_RE.test(e.title || '');
  const blocks = entries.filter(isBlock);
  const declined = entries.filter((e) => !isBlock(e) && e.selfDeclined);
  const meetings = entries.filter((e) => !isBlock(e) && !e.selfDeclined);
...
function buildDayViewSpec(briefData, briefingText) {
  ...
  // Conflict map: which timeline rows actually collide, and with what.
  // briefData.conflicts is [{ event1: <title>, event2: <title>,
  // overlapMinutes, time }]. Before this, conflicts existed ONLY as a summary
  // insight card ("2 conflicts — A / B") while the timeline rendered the
  // colliding meetings as ordinary sequential rows — the user could not see
  // WHERE the day double-books (2026-08-05: "the visual for my daily brief
  // did not handle conflicts").
  const conflictsByTitle = new Map();
  for (const c of briefData.conflicts || []) {
    const pairs = [
      [c.event1, c.event2],
      [c.event2, c.event1],
    ];
    for (const [self, other] of pairs) {
      ...
      const entry = conflictsByTitle.get(self) || { others: [], overlapMinutes: 0 };
      ...
      conflictsByTitle.set(self, entry);
    }
  }
  ...
  const events = timeline.map((ev, i) => {
    ...
    // A block or a meeting you declined isn't a real double-booking.

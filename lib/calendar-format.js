...
function buildDayUISpec(dayAnalysis, label) {
  ...
  // Show current + remaining events first, then past
  const ordered = [...dayAnalysis.current, ...dayAnalysis.remaining, ...dayAnalysis.past];
...
function spokenDaySummary(dayAnalysis, label) {
  ...
  // Current meeting
  if (dayAnalysis.current.length > 0) {
    const curr = dayAnalysis.current[0];
    parts.push(`You're currently in "${curr.event.summary}".`);
  }

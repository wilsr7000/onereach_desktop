...
const { AgentMemoryStore, getAgentMemory } = require('./agent-memory-store');
...
class UserProfileStore {
  ...
  _ensureSections() {
    ...
    if (!sections.includes('Key Facts')) {
      this._store.updateSection('Key Facts', '*No facts learned yet. Agents will populate this as they learn about you.*');
    }
  ...
  updateFact(key, value) {
    ...
    log.info('settings', 'Updated >', { section: section, normalizedKey: normalizedKey, normalizedValue: normalizedValue });
  }
  ...
  _routeFactToSection(key) {
    ...
    // Location keys
    if (lower.includes('home') || lower.includes('work') || lower.includes('location') ||
        lower.includes('city') || lower.includes('address') || lower.includes('timezone')) {
      return 'Locations';
    }
    // Preference keys
    if (lower.includes('unit') || lower.includes('format') || lower.includes('prefer') ||
        lower.includes('style') || lower.includes('theme') || lower.includes('language')) {
      return 'Preferences';
    }
  ...
  getContextString() {
    ...
    const meaningful = Object.entries(facts).filter(([_, v]) =>
      v && !v.includes('not yet learned') && !v.startsWith('*')
    );

...
/**
 * Prompt suffix appended to uiCapable agents so the LLM knows how to
 * return a declarative UI spec that the HUD will render.
 */
const UI_PROMPT_SUFFIX = `

When the user's request can benefit from a visual response (lists, confirmations,
selections), you MAY return a JSON object wrapped in a json code fence.
Supported types:

{ "type": "info", "title": "...", "body": "..." }
{ "type": "confirm", "title": "...", "body": "...", "yesLabel": "...", "noLabel": "..." }
{ "type": "select", "title": "...", "options": [{ "label": "...", "value": "..." }] }

If a visual response is not appropriate, respond with plain text as usual.`;

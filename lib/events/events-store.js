...
// UC-18: dependency-down must speak, not shrug. Map raw graph-client
// config errors to guidance the voice can say verbatim.
const GRAPH_CONFIG_ERR_RE = /not configured|unconfigured|not ready|password not configured|endpoint not configured/i;

function isGraphConfigError(err) {
  return GRAPH_CONFIG_ERR_RE.test((err && err.message) || String(err || ''));
}

function friendlyGraphError(err) {
  if (isGraphConfigError(err)) {
    return 'The graph database is not set up on this Mac yet — open Settings and add your Neo4j Aura credentials, then try again.';
  }

...
function _parseClassifierCacheLine(text) {
  const trimmed = String(text || '').trim();
  if (!trimmed.startsWith('{')) return null;
  try {
    const parsed = JSON.parse(trimmed);
...
*No learning notes yet.*`;
  ...
  /**
   * Phase 3: cheap content hash of a section. Used by the meeting-classifier
   * to build a composite cacheVersion -- when Aliases changes, hashSection
   * flips and the classifier knows to recompute.
   */
  hashSection(name) {
    const content = this.getSectionRaw(name) || '';
    return _shortHash(content);
  }
  ...
  // ── Classifier Cache (Phase 3) ──────────────────────────────────────

  /**
   * Read the cached classifier verdict for an event. Returns
   * `{ tier12, tier3 }` where each is `{ verdict|research, cacheVersion,
   * expiresAt }` or null. The two tiers cache independently because
   * different inputs invalidate them (e.g. an Aliases write invalidates
   * tier12 but not tier3).
   *
   * Returns `null` if no cache entry exists for this eventId.
   */
  readClassifierCache(eventId) {
    if (!eventId) return null;
    if (!this._loaded) return null;
    const entries = this.readEntries('Classifier Cache');
    for (const e of entries) {
      const parsed = _parseClassifierCacheLine(e.text);
      if (parsed && parsed.id === eventId) return parsed;
    }
    return null;
  }

  /**
   * Upsert a classifier cache entry for an eventId. Caller may pass either
   * tier12, tier3, or both -- omitted tiers preserve the prior cached value.
   *
   * Per Phase 3, each tier carries its own cacheVersion + expiresAt so
   * partial invalidation works (Aliases changes -> recompute tier12, leave
   * tier3 untouched until its independent 48h TTL expires).
   */
  async writeClassifierCache(eventId, { tier12, tier3 } = {}) {
    if (!eventId) return false;
    if (!this._loaded) await this.load();

    return _withSectionLock('Classifier Cache', async () => {
      const existing = this.readEntries('Classifier Cache');
      const now = Date.now();
      const surviving = [];
      ...
      for (const e of existing) {
        const parsed = _parseClassifierCacheLine(e.text);
        if (!parsed) continue;
        // Drop expired tiers entirely.
        if (parsed.tier12?.expiresAt && new Date(parsed.tier12.expiresAt).getTime() < now) {
          parsed.tier12 = null;
        }
        if (parsed.tier3?.expiresAt && new Date(parsed.tier3.expiresAt).getTime() < now) {
          parsed.tier3 = null;
        }
        if (parsed.id === eventId) {
          mergedExisting = parsed;
          continue;
        }
        // Drop rows whose both tiers are now empty.
        if (parsed.tier12 || parsed.tier3) surviving.push(parsed);
      }

      const entry = {
        id: eventId,
        tier12: tier12 !== undefined ? tier12 : (mergedExisting?.tier12 || null),
        tier3: tier3 !== undefined ? tier3 : (mergedExisting?.tier3 || null),
      };

      if (entry.tier12 || entry.tier3) surviving.push(entry);

      const lines = [`<!-- schemaVersion: ${SECTION_VERSIONS['Classifier Cache']} -->`];
      for (const s of surviving) lines.push(`- ${JSON.stringify(s)}`);
      this._store.updateSection('Classifier Cache', lines.join('\n'));
      await this._store.save();
      return true;
    });
  }

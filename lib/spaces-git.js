...
class SpacesGit {
  ...
  /**
   * Stage and commit only files belonging to a specific space.
   * Does NOT use checkout -- stays on the current branch (safe for concurrent use).
   *
   * Scopes to: spaces/<spaceId>/** and items that have spaceId in their metadata path.
   *
   * @param {string} spaceId - Space to commit
   * @param {Object} [opts]
   * @param {string} [opts.message]
   * @param {string} [opts.authorName]
   * @param {string} [opts.authorEmail]
   * @param {string[]} [opts.itemIds] - Optional: specific item IDs that changed (avoids full scan)
   * @returns {Promise<{sha: string|null, filesChanged: number, paths: string[]}>}
   */
  async commitSpace(spaceId, opts = {}) {
    this._ensureInit();

    const { message = `Update space ${spaceId}`, authorName, authorEmail, itemIds } = opts;
    const statusMatrix = await git.statusMatrix({ fs, dir: this.dir });
    ...
    for (const [filepath, head, workdir, stage] of statusMatrix) {
      ...
      const isSpaceFile = filepath.startsWith(spacePrefix);
      const isSpaceItem = itemIds
        ? itemIds.some(id => filepath.startsWith(`items/${id}/`))
        : false;

      if (!isSpaceFile && !isSpaceItem) continue;
    ...
    if (changedPaths.length === 0) {
      return { sha: null, filesChanged: 0, paths: [] };
    }
    ...
    return { sha, filesChanged: changedPaths.length, paths: changedPaths };
  }
  ...
  writeV3Marker() {
    ...
    fs.writeFileSync(path.join(this.dir, SPACES_VERSION_FILE), JSON.stringify(data, null, 2));
  }

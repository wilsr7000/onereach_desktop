...
(function () {
  ...
  function dispatchCue(cue) {
    ...
    // Instrument + respect the global OrbAudio mute kill-switch so
    // debugging "noises from the orb" sees every sound path, and mute
    // silences agent sound cues along with tones / TTS.
    try {
      window.OrbAudio?.recordSound?.('dispatchCue', {
        cueType: cue.type,
        cueName: cue.name,
        volume: cue.volume,
      });
      if (window.OrbAudio?.isMuted?.()) return;

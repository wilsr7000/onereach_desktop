...
(function () {
  ...
  function start(orbAPI, handlers, config = {}) {
    ...
    function shouldRejectInput(event, S) {
      // Raw-transcript gate only. Committed requests (function_call_transcript)
      // never reach here -- they're delivered before this gate (see the handler
      // below) because they are the model's structured request, not mic input.
      //
      // TTS cooldown: ignore echo after speaking. Scaled to the length of
      // what was just spoken -- a short confirmation ("OK", ~0.5s) only
      // freezes input for 1s, while long narration keeps the full
      // TTS_COOLDOWN_MS ceiling. Falls back to the fixed ceiling when no
      // duration was recorded.
      const ttsEndTime = S.get('ttsEndTime') || 0;
      if (ttsEndTime > 0) {
        const lastTtsMs = S.get('lastTtsDurationMs') || 0;
        const cooldownMs = lastTtsMs > 0
          ? Math.min(TTS_COOLDOWN_MS, Math.max(1000, Math.round(lastTtsMs * 0.8)))
          : TTS_COOLDOWN_MS;
        if (Date.now() - ttsEndTime < cooldownMs) {
          return 'cooldown';
        }
      }
    ...
    _unsubscribe = orbAPI.onEvent((event) => {
      ...
      // COMMITTED: function_call_transcript is the realtime model's structured
      // handle_user_request tool call -- the authoritative user request, NOT raw
      // mic input. Deliver it to the handler ALWAYS: never drop it on orb phase,
      // TTS cooldown, noise, or router dedup. Every one of those gates exists to
      // filter raw mic transcripts (echo, jibberish, repeats), and every one of
      // them has, at some point, silently eaten a committed request right after
      // the orb spoke a short reply -- the recurring "daily brief did nothing".
      // The handler (handleFunctionCallTranscript) runs its own same-turn dedup
      // against the transcript event, so double-dispatch is still prevented.
      if (event.type === 'function_call_transcript') {
        if (handlers[event.type]) handlers[event.type](event);
        return;
      }

      // INPUT phase gate (raw mic input only).
      if (!S.canAcceptInput()) {
        ...
        // All other non-committed input during non-accepting phases is dropped
        return;
      }

      // Secondary noise gate (raw transcript events only).

...
(function () {
  ...
  function start(orbAPI, handlers, config = {}) {
    ...
    _unsubscribe = orbAPI.onEvent((event) => {
      ...
      // INPUT phase gate. function_call_transcript is EXEMPT: it's the realtime
      // model's structured handle_user_request tool call derived from the
      // user's speech -- an authoritative request, not raw mic input -- and must
      // not be dropped just because the orb is briefly speaking a short ack or
      // has returned to idle (the "daily brief did nothing" race). It still
      // passes through the noise/dedup gate below, and the handler runs its own
      // same-turn dedup, so double-dispatch is still prevented.
      if (event.type !== 'function_call_transcript' && !S.canAcceptInput()) {

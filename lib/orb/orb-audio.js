...
(function () {
  ...
  async function playWAVAudio(base64Audio) {
    ...
    try {
      ...
      source.onended = () => {
        ...
        _currentSource = null;
        // Record how long the utterance was so the event router can scale
        // its echo-suppression cooldown (short "OK" doesn't deserve the
        // full 2.5s input freeze).
        try { window.OrbState?.set('lastTtsDurationMs', Math.round(audioBuffer.duration * 1000)); } catch (_) { /* optional */ }
        if (_onSpeakingEnd) _onSpeakingEnd();
      };
  ...
  async function playTTSAudio() {
    ...
    try {
      ...
      // Same duration record as playWAVAudio -- used to scale the echo cooldown.
      try { window.OrbState?.set('lastTtsDurationMs', Math.round((combined.length / 24000) * 1000)); } catch (_) { /* optional */ }
      ttsAudioChunks = [];
    } catch (e) {

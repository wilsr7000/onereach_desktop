...
class GenerativeFilterEngine {
  ...
  _parseScores(response, batch, filters) {
    try {
      ...
      for (let i = 0; i < batch.length; i++) {
        ...
        for (const filter of filters) {
          ...
          if (parsed[i] && typeof parsed[i][key] === 'number') {
            ...
            itemScores[key] = 0; // Unknown scores default to 0 (not passing)
          }
        }
      ...
      return batch.map(() => filters.reduce((acc, f) => ({ ...acc, [f.id]: 0 }), {}));
    }
  }
  ...
  _rankAndFilter(scoredItems, filters) {
    ...
    const itemsWithComposite = scoredItems.map((item) => {
      ...
      if (filters.length === 0 && typeof scores.score === 'number') {
        return {
          ...
          _search: {
            ...
            scores,
            reason,
            passesThresholds: scores.score >= 30,
          },
        };
      }
      ...
      for (const filter of filters) {
        ...
        const threshold = filter.threshold || 30; // Minimum 30% relevance required

...
const { FILTER_PROMPTS, buildEvaluationPrompt } = require('./filter-prompts');
...
class GenerativeFilterEngine {
  ...
  async search(options) {
    ...
    if (cached) {
      console.log('[GenerativeSearch] Returning cached results');
      return cached;
    }
    ...
    console.log(`[GenerativeSearch] Evaluating ${items.length} items with ${filters.length} filters`);
    
    const scoredItems = await this.batchProcessor.process(
      preparedItems,
      async (batch) => {
        return this._evaluateBatch(batch, prompt, filters, apiKey);
      }
    );
   ...
   */
  async _evaluateBatch(batch, prompt, filters, apiKey) {
    ...
    try {
      const response = await this.openaiAPI.callAPI(fullPrompt, apiKey, {
      ...
      console.error('[GenerativeSearch] Batch evaluation error:', error);
  ...
  _parseScores(response, batch, filters) {
    // #region agent log
    try { require('fs').appendFileSync('/Users/richardwilson/Onereach_app/.cursor/debug.log', JSON.stringify({location:'GenerativeFilterEngine:_parseScores:start',message:'Parsing LLM response',data:{responseType:typeof response,responseLength:response?.length,responsePreview:String(response)?.substring(0,500),batchLength:batch?.length,filterIds:filters?.map(f=>f.id)},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'H-SCORE'})+'\n'); } catch(e){}
    // #endregion
    try {
      let parsed;
      ...
      // #region agent log
      try { require('fs').appendFileSync('/Users/richardwilson/Onereach_app/.cursor/debug.log', JSON.stringify({location:'GenerativeFilterEngine:_parseScores:error',message:'Score parsing failed',data:{errorMessage:error?.message,errorName:error?.name,responsePreview:String(response)?.substring(0,500)},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'H-SCORE'})+'\n'); } catch(e){}
      // #endregion
      console.error('[GenerativeSearch] Score parsing error:', error);
      // Return default scores
      return batch.map(() => 
        filters.reduce((acc, f) => ({ ...acc, [f.id]: 50 }), {})
      );
    }
  }

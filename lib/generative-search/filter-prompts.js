...
- 0-29: Low priority for revisiting`,
    buildPrompt: () => `Identify items that appear to be valuable recent work worth revisiting. Consider quality, completeness, and apparent importance.`
  },
...
- 0-29: No actionable insights`,
    buildPrompt: () => `Evaluate which items contain actionable insights - things that can be directly acted upon or implemented.`
  },
...
- 0-29: Seems complete/fine as-is`,
    buildPrompt: () => `Identify items that need attention - incomplete work, items that could use improvement, or things that should be revisited.`
  },
...
- 30-49: Some similarity
- 0-29: Distinct items`,
    buildPrompt: () => `Identify items that appear to be duplicates or variations of other items in the set. High scores indicate items that might be consolidated.`
  }
};
...
function buildEvaluationPrompt(filters, context = {}) {
  const filterInstructions = filters.map(filter => {

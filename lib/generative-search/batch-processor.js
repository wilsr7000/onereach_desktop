...
class BatchProcessor {
  ...
  async process(items, processBatch) {
    ...
    log.info('app', 'Processing items in batches (concurrency: )', { items: items.length, batches: batches.length, this: this.concurrency });
    ...
    return new Promise((resolve, reject) => {
      const processNextBatch = async () => {
        ...
        if (batchQueue.length === 0 && activeBatches.size === 0) {
          ...
          log.info('app', 'Completed in ms', { this: this.stats.endTime - this.stats.startTime });
        ...
        while (batchQueue.length > 0 && activeBatches.size < this.concurrency) {
          ...
          const batchPromise = this._processBatchWithRetry(batch, idx, processBatch)
            ...
            .catch(error => {
              log.error('app', 'Batch failed', { idx: idx, error: error });
  ...
  async _processBatchWithRetry(batch, batchIndex, processBatch) {
    ...
    for (let attempt = 0; attempt <= this.retryAttempts; attempt++) {
      try {
        if (attempt > 0) {
          log.info('app', 'Retrying batch (attempt )', { batchIndex: batchIndex, attempt1: attempt + 1 });

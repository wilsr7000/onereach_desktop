...
// Transport lives in lib/kv-client (ADR-070 KV chokepoint); only the
// collection binding and the test seam (opts.fetchImpl) remain here.
async function kvRequest(method, key, value, opts = {}) {
  const kvOpts = { fetchImpl: opts.fetchImpl };
  if (method === 'PUT') {
    await kv.kvPut(PRESENCE_LOG_COLLECTION, key, value, kvOpts);
    return null;
  }
  return kv.kvGet(PRESENCE_LOG_COLLECTION, key, kvOpts);
}

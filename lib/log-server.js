...
class LogServer {
  ...
  _handleHTTP(req, res) {
    ...
    try {
      ...
      // Sync v5 -- diagnostics endpoints (Phase 1+ scaffold).
      // Read-only, safe to add early. Reports the schema-version handshake,
      // deviceId, heartbeat reporter state, queue/DLQ surface (when wired by
      // Phase 2+ boot), and the canned graph-side health queries.
   ...
   * { status, body }; this method awaits the promise and renders. Centralises
   * error handling so the route table stays free of inline async boilerplate.
   */

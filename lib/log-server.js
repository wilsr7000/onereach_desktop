...
// Loopback-only admission (2026-09-02 Lite security review). The server
// binds to 127.0.0.1, but that never stopped a web page: with the old
// `Access-Control-Allow-Origin: *` any page in a browser tab could READ
// every log line cross-origin (URLs with OAuth query strings included),
// a no-cors POST could hit /app/restart, and DNS rebinding sidestepped
// the bind address altogether. Two rules, both cheap:
//   - the Host header must name a loopback host (defeats rebinding);
//   - a browser Origin, when present, must itself be loopback — anything
//     else is refused with a 403 and no CORS header.
// Callers with no Origin (curl, Node tools, the MCP layer, the tests) are
// unchanged.
const LOOPBACK_HOST_RE = /^(127\.0\.0\.1|localhost|\[::1\])(:\d+)?$/i;
function isLoopbackHost(host) {
  return LOOPBACK_HOST_RE.test(String(host || '').trim());
}
function isLoopbackOrigin(origin) {
  try {
    const u = new URL(String(origin));
    return (u.protocol === 'http:' || u.protocol === 'https:') && LOOPBACK_HOST_RE.test(u.host);
  } catch (_e) {
    return false; // includes the literal "null" origin of sandboxed frames
  }
}
function isAdmitted(req) {
  const origin = req.headers.origin;
  return isLoopbackHost(req.headers.host) && (origin === undefined || isLoopbackOrigin(origin));
}

const PORT = 47292;
const MAX_BODY_SIZE = 1 * 1024 * 1024; // 1 MB max request body
const MAX_WS_PAYLOAD = 256 * 1024; // 256 KB max WebSocket frame
/** How many ports above the preferred one start() will try before giving up. */
const MAX_PORT_FALLBACKS = 10;

class LogServer {
  /**
   * @param {LogEventQueue} logQueue
   * @param {{port?: number}} [options] - port defaults to 47292 (full app);
   *   lite passes { port: 47392 } so both apps can run side-by-side.
   */
  constructor(logQueue, options = {}) {
    this.queue = logQueue;
  ...
  async start() {
    ...
    // Port fallback: when the preferred port is taken (the installed app,
    // a dev instance, a scratch instance…), walk up through the next few
    // ports and give up with a rejection after MAX_PORT_FALLBACKS. The
    // listeners are registered ONCE, outside the retry — the previous
    // version re-entered its own 'error' handler on the fallback's
    // EADDRINUSE and called listen() forever on the same port, growing a
    // listener per attempt at 100% CPU until V8 ran out of heap
    // (2026-09-05: installed app on 47392 + one dev instance on 47393 =
    // every further instance dead in ~6 minutes).
    ...
    return new Promise((resolve, reject) => {
      ...
      this.server.once('listening', () => {
        ...
        if (candidate === basePort) {
          this.queue.info('app', 'Log server started', { port: this.port, url: `http://127.0.0.1:${this.port}` });
        } else {
          this.queue.info('app', 'Log server started on fallback port', { port: this.port, preferred: basePort });
        }
        resolve();
      });

      this.server.on('error', (error) => {
        if (this._started || error.code !== 'EADDRINUSE' || attempt >= MAX_PORT_FALLBACKS) {
          reject(error);
          return;
        }
        attempt += 1;
        this.queue.warn('app', `Log server port ${candidate} already in use, trying ${candidate + 1}`);
        candidate += 1;
  ...
  _handleHTTP(req, res) {
    ...
    // CORS headers for browser access -- only ever echoing a loopback
    // origin (see isAdmitted), never `*`.

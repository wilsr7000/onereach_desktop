    ...
    return new Promise((resolve, reject) => {
      ...
      this.server.on('error', (error) => {
        if (error.code === 'EADDRINUSE') {
          this.queue.warn('app', `Log server port ${this.port} already in use, retrying...`);
          // Try next port
          const fallbackPort = this.port + 1;
          this.server.listen(fallbackPort, '127.0.0.1', () => {
            this._started = true;
            this.port = fallbackPort;
            this.queue.info('app', 'Log server started on fallback port', { port: fallbackPort });
            resolve();
          });
        } else {
          reject(error);

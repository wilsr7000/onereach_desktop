...
function parseMarkdownSections(markdown) {
  ...
  for (const line of lines) {
    ...
    if (match) {
      ...
      // Skip top-level titles (# Title) in the header -- rebuildMarkdown adds its own
  ...
  async load() {
    try {
      ...
      try {
        ...
        log.info('agent', 'loadItem failed for , will create new', {
          this: this.agentId,
      ...
      if (existingItem && existingItem.content) {
        ...
        this._loaded = true;
        this._dirty = false;
        log.info('agent', `Loaded memory for ${this.agentId || 'unknown'} from Spaces`, { agentId: this.agentId });
        return true;
      }

      // Item exists in index but content is empty/missing -- try reading content.md directly
      if (existingItem && !existingItem.content) {
        try {
          const _contentPath = path.join(
            path.dirname(require.resolve('../clipboard-storage-v2')),
            '..',
            'Documents',
            'OR-Spaces',
            'items',
          ...
          const directPath = path.join(homeDir, 'Documents', 'OR-Spaces', 'items', this.itemId, 'content.md');

          if (fs.existsSync(directPath)) {
            const directContent = fs.readFileSync(directPath, 'utf8');
            if (directContent && directContent.trim().length > 0) {
              log.warn('agent', 'Recovered memory from disk for (Spaces returned empty content)', {
                this: this.agentId,
              });
              this._raw = directContent;
              this._sections = parseMarkdownSections(this._raw);
              this._loaded = true;
              this._dirty = true; // Mark dirty to re-save to Spaces
              return true;
            }
          }
        } catch (diskErr) {
          log.warn('agent', 'Disk recovery failed for', { this: this.agentId, error: diskErr?.message });
        }
        log.warn('agent', 'Item exists but content empty for -- creating fresh', { this: this.agentId });
      }

      // Create new item (only if item truly doesn't exist)

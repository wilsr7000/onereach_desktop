...
const KNOWN_AGENT_ISSUES = [
  ...
  // ── Phase 8 calendar-specific patterns (calendar agent overhaul) ────────

  {
    id: 'KAI-CAL-001',
    title: 'calendar-query-agent: repeated "couldn\'t find an event matching" -- alias proposal candidate',
    detect: (ctx) => {
      if (ctx.agent?.id !== 'calendar-query-agent') return false;
      if (ctx.interactions.length < 5) return false;
      const misses = ctx.interactions.filter(
        (i) => i.message && /couldn'?t find an event matching/i.test(i.message)
      );
      return misses.length >= 3;
    },
    // Fix is null -- escalate to alias-proposal flow via userQueue. The
    // learning loop's transcript-reviewer will surface the candidate.
    fix: null,
  },

  {
    id: 'KAI-CAL-002',
    title: 'calendar-query-agent: brief reports 0 meetings while Omnical health is fine -- merge regression',
    detect: (ctx) => {
      if (ctx.agent?.id !== 'calendar-query-agent') return false;
      if (ctx.interactions.length < 3) return false;
      // Look for the briefing-contribution-empty pattern when the user has
      // a non-trivial calendar (the broader system's health check would
      // contradict). This is a heuristic; the detector raises the issue
      // and the engine inspects logs.
      const emptyBriefs = ctx.interactions.filter(
        (i) => i.message && /no meetings (scheduled|today|this week)/i.test(i.message)
      );
      return emptyBriefs.length >= 3;
    },
    fix: null,
  },
  ...
  {
    id: 'KAI-CAL-003',
    title: 'calendar-query-agent: rapid user correction after answer ("no, the meeting at 3") -- alias hint',
    detect: (ctx) => {
      if (ctx.agent?.id !== 'calendar-query-agent') return false;
      if (ctx.interactions.length < 4) return false;
      // Pairs where a user follow-up within 5 seconds starts with "no" or
      // "actually" or contains "I meant" -- a strong signal the agent picked
      // the wrong event and a specific alias would help.
      let corrections = 0;
      for (let i = 1; i < ctx.interactions.length; i++) {
        const prev = ctx.interactions[i - 1];
        const cur = ctx.interactions[i];
        const dt = new Date(cur.ts || 0).getTime() - new Date(prev.ts || 0).getTime();
        if (dt < 0 || dt > 5000) continue;
        const userInput = (cur.userInput || '').toLowerCase();
        if (/^(no|actually|i meant|nope|not that)/i.test(userInput)) corrections += 1;
      }
      return corrections >= 2;
    },
    // Promote the most recent corrected event ID to the Aliases section
    // via the user-queue review flow. Fix is null -- escalate.
    fix: null,
  },
];

...
class EvaluationConsolidator {
   ...
   * @returns {Object[]}
   */
  generateSuggestions(evaluations, conflicts) {
    const suggestions = [];
  ...
  generateRationale(data) {
    ...
    // Score explanation
    parts.push(`Overall score of ${Math.round(aggregateScore)} reflects weighted consensus of ${agentScores.length} evaluating agents.`);

...
class CalendarStore {
  ...
  generateMorningBrief(date, externalEvents) {
    ...
    const backToBack = this._findBackToBack(timedEvents);

    // Free time
    const balance = this.getDayBalance(d, externalEvents);
    const freeSlots = this.getFreeSlots(d, 30, externalEvents);
    ...
    return {
      ...
      dayLabel,
      dayOfWeek,
      greeting: this._briefGreeting(d),
  ...
  renderBriefForSpeech(brief) {
    ...
    // Recurring breakdown
    if (brief.summary.recurringCount > 0 && brief.summary.oneOffCount > 0) {
      parts.push(
    ...
    if (brief.conflicts.length > 0) {
      ...
      parts.push(`Heads up: you have ${brief.conflicts.length} scheduling ${conflictWord}.`);
    ...
    // Free time
    if (brief.freeTime.totalFreeHours > 0 && brief.summary.timedEvents > 0) {
      parts.push(`You have about ${brief.freeTime.totalFreeHours} hours of free time during working hours.`);

...
const SLOT_STEP_MINUTES = 15; // Resolution for free-slot search
...
class CalendarStore {
  ...
  updateEvent(eventId, changes) {
    ...
    const allowed = [
      ...
      'allDay', 'recurring', 'reminders', 'guests', 'calendar',
      'color', 'exceptions', 'overrides',
    ];
  ...
  _matchesPattern(cursor, baseDate, rule) {
    ...
    switch (rule.pattern) {
      ...
      case 'yearly':
        return cursor.getDate() === baseDate.getDate() &&
               cursor.getMonth() === baseDate.getMonth();
  ...
  getEventsInRange(rangeStart, rangeEnd, externalEvents) {
    ...
    // Merge external events (de-duplicate by title + time)
    if (externalEvents && externalEvents.length > 0) {
      for (const ext of externalEvents) {
        const normalExt = this._normalizeExternalEvent(ext);
        // De-dup: skip if a local event has same title and overlapping time
  ...
  generateMorningBrief(date, externalEvents) {
    ...
    const pastEvents = isToday
      ? timedEvents.filter(e => new Date(e.endTime) <= now)
      : [];
    ...
    const upcomingEvents = isToday
      ? timedEvents.filter(e => new Date(e.startTime) > now)
      : timedEvents; // If not today, all events are "upcoming" (future day)
    ...
    const freeSlots = isToday
      ? allFreeSlots.filter(s => new Date(s.end) > now)
      : allFreeSlots;
    ...
    const longestFree = freeSlots.reduce((max, s) =>
    ...
    return {
      ...
      freeTime: {
        ...
        longestFreeBlock: longestFree.durationMinutes > 0
  ...
  renderBriefForSpeech(brief) {
    ...
    if (brief.currentMeeting) {
      ...
      parts.push(`Right now you're in "${brief.currentMeeting.title}"${locPart}, wrapping up in about ${brief.currentMeeting.minutesRemaining} minutes.`);
    }
    ...
    if (isToday && brief.nextMeeting) {
      ...
      if (brief.nextMeeting.minutesUntil <= 60) {
        parts.push(`Your next meeting is "${brief.nextMeeting.title}" at ${brief.nextMeeting.time}${locPart}, that's in about ${brief.nextMeeting.minutesUntil} minutes.`);
      } else {
    ...
    if (brief.backToBack.length > 0) {
      parts.push(`You have ${brief.backToBack.length} back-to-back transition${brief.backToBack.length > 1 ? 's' : ''} with no break.`);
    }

    // Free time -- show remaining free time for today
    const freeHours = isToday ? (brief.freeTime.remainingFreeHours ?? brief.freeTime.totalFreeHours) : brief.freeTime.totalFreeHours;
  ...
  generateDaySummary(date, externalEvents) {
    ...
    if (allDay.length > 0) {
      parts.push(`${allDay.length} all-day event${allDay.length > 1 ? 's' : ''}: ${allDay.map(e => e.title).join(', ')}.`);
    }
  ...
  generateWeekSummary(externalEvents) {
    ...
    if (busiestDay.count > 0) {
      parts.push(`Your busiest day is ${busiestDay.day} with ${busiestDay.count} meeting${busiestDay.count > 1 ? 's' : ''}.`);
    }
  ...
  getNextEvent(externalEvents) {
    ...
    const events = this.getEventsInRange(now, endOfWeek, externalEvents)
      .filter(e => !e.allDay && new Date(e.startTime) > now);

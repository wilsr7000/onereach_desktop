...
// ONE definition of conflict (overlap) vs back-to-back (no break, no overlap),
// shared with the dayView and the spoken brief. See lib/brief-items.js.
const { deriveScheduleFlags } = require('./brief-items');
...
class CalendarStore {
   ...
   */
  getEventsInRange(rangeStart, rangeEnd, externalEvents, opts = {}) {
    this._ensureLoaded();
    const rs = new Date(rangeStart);
    const re = new Date(rangeEnd);
    const results = [];

    // Live-first policy (2026-09-15): when the caller passes
    // `includeLocal: false` the app-local store is left out of the merge.
    // Nothing in production writes this store any more (calendar-mutate
    // writes Omnical; agent-tools' events_add writes the graph events
    // store), so what it holds is legacy test data -- two "standup" rows at
    // 9:00 every weekday that collided with the user's real 9 AM meeting and
    // produced phantom "3 conflicts" in every morning brief.
    const localEvents = opts.includeLocal === false ? [] : this._events;
    ...
    if (externalEvents && externalEvents.length > 0) {
      ...
      for (const ext of externalEvents) {
        ...
        // Range filter external events too. calendar-fetch always returns a
        // 14-day window (and returns the whole multi-day cache when Omnical is
        // offline), so without this a getEventsForDay(today, cache) merges every
        // cached day into "today" -- the "54 meetings today" fabrication. Match
        // the internal-event check above: only merge events that overlap [rs,re].
        const exs = new Date(normalExt.startTime);
        const exe = new Date(normalExt.endTime);
        if (!overlaps(exs, exe, rs, re)) continue;
    ...
    // Final dedupe pass: the internal store can hold near-identical recurring
    // copies (e.g. three "standup" entries from different source calendars)
    // that survive the incremental checks above because they were added from
    // the same batch. Same normalized title + same start minute = one event.
    // (2026-08-04: five 9 AM standup-ish duplicates inflated the daily-brief
    // count.)
    const seenKeys = new Set();
    const deduped = [];
    for (const ev of results) {
      const key = `${(ev.title || '').toLowerCase().replace(/\s+/g, ' ').trim()}|${new Date(ev.startTime).setSeconds(0, 0)}`;
   ...
   */
  getEventsForDay(date, externalEvents, opts) {
    const d = new Date(date);
    return this.getEventsInRange(startOfDay(d), endOfDay(d), externalEvents, opts);
  }
  ...
  _normalizeExternalEvent(ext) {
    return {
      ...
      // The calendar owner's own RSVP (Google/Omnical mark the user's
      // attendee entry with self: true). A declined event is not the user's
      // meeting -- the brief must not count it.
      selfDeclined: !!(ext.attendees || []).find((a) => a && a.self && a.responseStatus === 'declined'),
   ...
   */
  findDayConflicts(date, externalEvents, opts = {}) {
    const events = this.getEventsForDay(date, externalEvents, opts).filter((e) => !e.allDay);
    // A conflict is an OVERLAP between two real meetings. Blocks ("Don't
    // book") and invites the user declined are not double-bookings and are
    // skipped by deriveScheduleFlags.
    return deriveScheduleFlags(events, { minGapMinutes: this._settings.minGapMinutes }).conflicts.map((c) => ({
      event1: c.first,
      event2: c.second,
   ...
   */
  getFreeSlots(date, minDurationMinutes, externalEvents, opts) {
    ...
    const events = this.getEventsForDay(d, externalEvents, opts)
      .filter((e) => !e.allDay)
   ...
   */
  getDayBalance(date, externalEvents, opts) {
    const d = new Date(date);
    const wh = this._settings.workingHours;
    const totalWorkMinutes = (wh.end - wh.start) * 60;

    const events = this.getEventsForDay(d, externalEvents, opts).filter((e) => !e.allDay);
    let busyMinutes = 0;
    for (const evt of events) {
   ...
   */
  generateMorningBrief(date, externalEvents, opts = {}) {
    ...
    // Conflicts vs back-to-back -- ONE definition (lib/brief-items.js):
    //   conflict     = two real meetings overlap
    //   back-to-back = one ends and the next starts within minGapMinutes,
    //                  with NO overlap
    // The old code counted negative gaps (overlaps) as back-to-back too, so
    // an overlapping pair showed up in BOTH tallies and the composed brief
    // muddled them. Blocks and declined invites count as neither. For today
    // only pairs that are still ahead are reported.
    const flags = deriveScheduleFlags(timedEvents, {
      minGapMinutes: this._settings.minGapMinutes,
      now: isToday ? now : null,
    });
    const upcomingConflicts = flags.conflicts.map((c) => ({
      event1: c.first,
      event2: c.second,
      overlapMinutes: c.overlapMinutes,
      atMs: c.atMs,
    }));
    const backToBack = flags.backToBack;

    // Free time -- for today, only show remaining free time
    const balance = this.getDayBalance(d, externalEvents, opts);
    const allFreeSlots = this.getFreeSlots(d, 30, externalEvents, opts);
    ...
    const tomorrowEvents = this.getEventsForDay(tomorrow, externalEvents, opts);
    ...
    return {
      ...
      timeline: timedEvents.map((e) => {
        ...
        return {
          ...
          // ISO copies so downstream code (brief-items) never has to parse
          // the formatted clock strings back into times.
      ...
      conflicts: upcomingConflicts.map((c) => ({
        ...
        overlapMinutes: c.overlapMinutes,
        // When the overlap starts (the later start of the pair).
        time: formatTime12(c.atMs),
      })),

      backToBack: backToBack.map((pair) => ({
        first: pair.first.title,
        second: pair.second.title,
        transitionTime: formatTime12(pair.first.endTime),
  ...
  /**
   * Find back-to-back meeting pairs: the next real meeting starts within
   * minGapMinutes AFTER this one ends (gap in [0, minGap)). Overlaps are
   * conflicts, not back-to-backs, and are never returned here.
   *
   * @returns {Array<[Object, Object]>} [first, second] event pairs
   */
  _findBackToBack(sortedEvents) {
    return deriveScheduleFlags(sortedEvents, { minGapMinutes: this._settings.minGapMinutes }).backToBack.map(
      (p) => [p.first, p.second]
    );
  }

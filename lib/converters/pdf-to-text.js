...
class PdfToTextAgent extends BaseConverterAgent {
   ...
   * @private
   */
  async _extractWithOcr(pdfBuffer, options = {}) {
    if (!this._ai) {
    ...
    const result = await this._ai.vision(imageData, 
      ...
      {
        profile: 'vision',
        system: 'You are a precise OCR engine. Extract all visible text from the document exactly ' +
    ...
    const text = typeof result === 'string'
      ? result
      : (result?.content || result?.text || String(result));

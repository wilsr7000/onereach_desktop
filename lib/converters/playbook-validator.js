...
function validateStructural(playbook) {
  ...
  if (fw && typeof fw === 'object') {
    for (const [name, spec] of Object.entries(FRAMEWORK_PILLARS)) {
      ...
      if (value === undefined || value === null) {
        errors.push({ field: `framework.${name}`, code: 'MISSING_FIELD', message: `Framework field "${name}" is missing` });
      } else if (spec.type === 'string' && typeof value !== 'string') {
        errors.push({ field: `framework.${name}`, code: 'WRONG_TYPE', message: `Framework field "${name}" should be a string` });
      } else if (spec.type === 'array' && !Array.isArray(value)) {
        errors.push({ field: `framework.${name}`, code: 'WRONG_TYPE', message: `Framework field "${name}" should be an array` });
...
function validateGraphReadiness(playbook) {
  ...
  if (Array.isArray(playbook.keywords) && playbook.keywords.length === 0) {
    errors.push({ field: 'keywords', code: 'EMPTY_KEYWORDS', message: 'Keywords array is empty (needed for graph indexing)' });
  }

  // Framework should exist for graph edges
  if (!playbook.framework || typeof playbook.framework !== 'object') {
    errors.push({ field: 'framework', code: 'MISSING_FRAMEWORK', message: 'Framework is needed for graph edge creation' });
  }

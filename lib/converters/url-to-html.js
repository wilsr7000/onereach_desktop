...
// Bounded so a long-running session with many distinct URLs doesn't grow
// unbounded; when full, oldest entries (insertion order) are evicted.
const _htmlCache = new Map();
const _HTML_CACHE_MAX = 200;

function _setCached(url, entry) {
  if (_htmlCache.size >= _HTML_CACHE_MAX) {
    const oldestKey = _htmlCache.keys().next().value;
    if (oldestKey !== undefined) _htmlCache.delete(oldestKey);
  }
    ...
    switch (strategy) {
      ...
      case 'cached': {
        ...
        if (cached && Date.now() - cached.timestamp < this._cacheTtlMs) {
          ...
          output = await this._fetchHtml(url);
          _setCached(url, { html: output, timestamp: Date.now() });
        }
        break;
      }

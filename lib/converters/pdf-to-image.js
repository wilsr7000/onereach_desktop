...
class PdfToImageAgent extends BaseConverterAgent {
  ...
  async execute(input, strategy, options = {}) {
    ...
    const outputSize = Array.isArray(output) ? output.reduce((sum, buf) => sum + buf.length, 0) : output.length;
  ...
  async _renderAllWithPdftoppm(pdfBuffer, opts) {
    ...
    try {
      ...
      const args = ['-r', String(opts.dpi || 150)];
  ...
  async _renderFallback(pdfBuffer, opts) {
    ...
    return sharp(Buffer.from(svgContent)).resize(width, height).toFormat(format).toBuffer();
  }

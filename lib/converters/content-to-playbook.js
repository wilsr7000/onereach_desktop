...
class ContentToPlaybookAgent extends BaseConverterAgent {
   ...
   * @private
   */
  async _fullAnalysis(content, _options) {
...
Return a single JSON object with all fields above.`;
    ...
    const result = await this._ai.json(`${systemPrompt}\n\n---\n\nContent to analyze:\n\n${content}`, {
      profile: 'standard',
   ...
   * @private
   */
  async _quickAnalysis(content, _options) {
...
Return JSON only. Be specific to the content provided.`;
  ...
  _isFrameworkComplete(fw) {
    if (!fw) return false;
    return !!(fw.who?.primary && fw.why?.coreValue && fw.what?.primaryAction && fw.where?.platform);
  }

...
${contentHtml}
   ...
   * @private
   */
  _buildDashboard(title, contentHtml, framework, doFramework, _keywords) {
...
${contentHtml}
  ...
  _frameworkToTable(fw) {
    ...
    if (fw.who) {
      rows.push(
        `<tr><th>WHO</th><td><strong>${this._esc(fw.who.primary || 'N/A')}</strong><br>${this._esc(fw.who.context || '')}<br>Traits: ${(fw.who.characteristics || []).join(', ') || 'N/A'}</td></tr>`
      );
    }
    if (fw.why) {
      rows.push(
        `<tr><th>WHY</th><td><strong>${this._esc(fw.why.coreValue || 'N/A')}</strong><br>Hook: ${this._esc(fw.why.emotionalHook || 'N/A')}<br>Benefit: ${this._esc(fw.why.practicalBenefit || 'N/A')}</td></tr>`
      );
    }
    if (fw.what) {
      rows.push(
        `<tr><th>WHAT</th><td><strong>${this._esc(fw.what.primaryAction || 'N/A')}</strong><br>Success: ${this._esc(fw.what.successLooksLike || 'N/A')}<br>Failure: ${this._esc(fw.what.failureLooksLike || 'N/A')}</td></tr>`
      );
    }
    if (fw.where) {
      rows.push(
        `<tr><th>WHERE</th><td><strong>${this._esc(fw.where.platform || 'N/A')}</strong> / ${this._esc(fw.where.format || 'N/A')}<br>${this._esc(fw.where.distribution || '')}<br>Constraints: ${(fw.where.constraints || []).join(', ') || 'none'}</td></tr>`
      );
    }
    if (fw.when?.raw) {
  ...
  _personasToHtml(doFramework) {
    ...
    const personas = doFramework.personas
      .map((p) => {

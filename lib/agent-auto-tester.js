...
- Be specific about expected results`;
    ...
    try {
      ...
      log.error('agent', 'Failed to generate execution plan', { error: error });
      return null;
    }
  }
  ...
  async executeAndVerify(agent, testPrompt) {
    ...
    try {
      ...
      log.info('agent', 'Execution plan', { plan: plan.action, data: '->', plan: plan.scriptType });
      ...
      if (this.useAgenticExecution && plan.executionScript) {
        try {
          log.info('agent', 'Using agentic CLI execution');
          const agentPrompt =
            plan.scriptType === 'applescript'
          ...
          const result = await this.claude.executeWithTools(agentPrompt, {
            allowedTools: ['Bash'],
            systemPrompt:
              'You are executing a test command. Run the command using Bash and report the output. Be concise.',
          });
          ...
          this.addTimelineEvent('script-executed', {
            success: result.success,
            output: executionOutput.substring(0, 200),
            agentic: true,
          });
          log.info('agent', 'Agentic execution result', { result: result.success ? 'SUCCESS' : 'FAILED' });
        } catch (error) {
          log.error('agent', 'Agentic execution error', { error: error.message });
        ...
        try {
          ...
          const { stdout } = await execAsync(`osascript -e '${script}'`, { timeout: plan.timeout || 10000 });
        ...
        try {
          const { stdout } = await execAsync(plan.executionScript, { timeout: plan.timeout || 10000 });
          executionOutput = stdout;
          executionResult = { success: true, output: stdout, exitCode: 0 };
          this.addTimelineEvent('script-executed', { success: true, output: stdout.substring(0, 100) });
        } catch (error) {
      ...
      const multiVerification = await this.runMultiVerification(
        plan,
        beforeState,
        afterState,
        stateDiff,
      ...
      this.addTimelineEvent('verification-complete', {
        ...
        results: Object.entries(multiVerification.results)
          .map(([k, v]) => `${k}:${v.passed}`)
          .join(', '),
      });
  ...
  async runMultiVerification(plan, beforeState, afterState, stateDiff, executionResult) {
    ...
    const allPassed = results.primary.passed && results.appState?.passed !== false && results.execution.passed;
    ...
    if (beforeState.type === 'browser') {
      if (action.includes('open') || action.includes('navigate') || action.includes('go to')) {
        ...
        return {
          ...
          details: urlChanged ? `Navigated to: ${afterState.url}` : `URL unchanged: ${afterState.url}`,
   ...
   */
  summarizeState(state, _relevantApps) {
    return {
  ...
  async diagnoseFailure(agent, testPrompt, result) {
    ...
    const stateInfo =
      result.actualState && result.expectedState
...
Respond in JSON format:
{
  ...
  async generateRecommendation(agent, testPrompt) {
    const historyText = this.attemptHistory
      .map((h, i) => `Attempt ${i + 1}: ${h.result.method} - ${h.result.details}`)
      .join('\n');
...
// ==================== V2: Test Scenario Generation & Persistence ====================

/**
 * Generate test scenarios for an agent based on its configuration.
 * Produces prompts that SHOULD match (positive) and prompts that should NOT (negative).
 *
 * @param {Object} agent - Agent configuration (v2 schema)
 * @returns {Promise<{ positive: string[], negative: string[] }>}
 */
async function generateTestScenarios(agent) {
  const ai = require('./ai-service');

  const prompt = `Generate test scenarios for this voice agent:

Name: ${agent.name}
Type: ${agent.executionType}
Keywords: ${(agent.keywords || []).join(', ')}
Categories: ${(agent.categories || []).join(', ')}
Prompt (first 300 chars): ${(agent.prompt || '').substring(0, 300)}

Return JSON with two arrays:
{
  "positive": ["prompt that SHOULD trigger this agent", ...],
  "negative": ["prompt that should NOT trigger this agent", ...]
}

Rules:
- 3-5 positive prompts covering different phrasings/capabilities
- 2-3 negative prompts that should go to other agents (weather, calendar, search, etc.)
- Keep prompts natural and conversational (how a user would actually speak)
- Positive prompts should test different aspects of the agent's capabilities`;

  try {
    const result = await ai.chat({
      profile: 'fast',
      system: 'You generate test scenarios for voice agents. Return only valid JSON.',
    ...
    log.warn('agent', 'Failed to generate test scenarios', { error: error.message });
    // Fallback: generate basic scenarios from keywords
    return {
      positive: (agent.keywords || []).slice(0, 3).map((kw) => `Can you help me with ${kw}?`),
      negative: ['What time is it?', "What's the weather like?"],
    };
  }
}

/**
 * Run a quick validation test: sends a prompt through the agent's LLM path
 * and checks that it returns a non-empty, successful response.
 *
 * @param {Object} agent - Agent configuration
 * @param {string} testPrompt - Prompt to test
 * @returns {Promise<{ passed: boolean, response: string, durationMs: number }>}
 */
async function quickValidate(agent, testPrompt) {
  const ai = require('./ai-service');
  ...
  try {
    const result = await ai.chat({
      ...
      maxTokens: 512,
      temperature: 0.7,
      feature: `test:${agent.id || agent.name}`,
    });
...
/**
 * Run a full test suite for an agent: generate scenarios, execute them,
 * and return a summary. Optionally persists results to agent-store.
 *
 * @param {Object} agent - Agent configuration (v2 schema)
 * @param {Object} options
 * @param {boolean} options.persist - Whether to save results to agent-store
 * @param {Function} options.onProgress - Progress callback
 * @returns {Promise<Object>} Test suite results
 */
async function runTestSuite(agent, options = {}) {
  const { onProgress = () => {} } = options;
  const suiteStart = Date.now();

  onProgress({ type: 'generating-scenarios', message: 'Generating test scenarios...' });
  ...
  // Run positive tests (should succeed)
  for (let i = 0; i < scenarios.positive.length; i++) {
    const prompt = scenarios.positive[i];
    onProgress({
      type: 'testing-positive',
      index: i,
      total: scenarios.positive.length,
      prompt,
      message: `Testing positive scenario ${i + 1}/${scenarios.positive.length}...`,
    });

    const testResult = await quickValidate(agent, prompt);
    results.positive.push({ prompt, ...testResult });
    results.summary.total++;
    if (testResult.passed) results.summary.passed++;
    else results.summary.failed++;
  }

  // Run negative tests (we just check the agent responds -- routing is tested separately)
  for (let i = 0; i < scenarios.negative.length; i++) {
    const prompt = scenarios.negative[i];
    onProgress({
      type: 'testing-negative',
      index: i,
      total: scenarios.negative.length,
      prompt,
      message: `Testing negative scenario ${i + 1}/${scenarios.negative.length}...`,
    });

    // For negative tests, we're just verifying the agent doesn't crash
    const testResult = await quickValidate(agent, prompt);
    results.negative.push({ prompt, ...testResult, note: 'Negative test -- should be routed elsewhere' });
  }

  results.durationMs = Date.now() - suiteStart;
  results.status = results.summary.failed === 0 ? 'passed' : 'failed';

  // Persist results to agent-store if requested
  if (options.persist && agent.id) {
    try {
      const { getAgentStore } = require('../src/voice-task-sdk/agent-store');
      const store = getAgentStore();
      if (store?.initialized) {
        const existingAgent = store.getAgent(agent.id);
        if (existingAgent) {
          await store.updateAgent(
            ...
            'auto-fix',
            `Test suite: ${results.summary.passed}/${results.summary.total} passed`
          );
        }
      }
    } catch (err) {
      log.warn('agent', 'Could not persist test results', { error: err.message });
    }
  }

  onProgress({
    type: 'complete',
    message: `Tests complete: ${results.summary.passed}/${results.summary.total} passed`,
    results,
  });

...
RULES:
1. ALWAYS call meeting_open_setup when invoked, so the Start-a-Meeting modal pops open — and again after EVERY draft change (pass a short notice like "Added Erika ✓"). EXCEPTION: never after meeting_start — once the meeting starts, the setup modal closes itself and must stay closed (reopening it stacks a second window under the meeting).
...
4. "Start it" / "start the meeting now" → meeting_start, then answer by VOICE ONLY (no meeting_open_setup). Report the outcome honestly: meeting window opened in Space X with N participants; the join link is shared from the meeting window itself.
...
async function ensureMeetingStarterAgent(deps = {}) {
  try {
    ...
    const stalePrompt = String(existing.prompt || '') !== MEETING_AGENT_PROMPT;
    if (missingPlaybook || missingTools || stalePrompt) {
      const updates = {};
      if (missingPlaybook) updates.playbook = composePlaybook();
      if (missingTools) updates.tools = [...MEETING_AGENT_TOOLS];
      if (stalePrompt) updates.prompt = MEETING_AGENT_PROMPT;
      const updated = await store.updateAgent(existing.id, updates, 'update', 'ensure-meeting-agent: playbook/tools/prompt patch');

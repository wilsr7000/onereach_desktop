...
function _closeSetupModal() {
  if (_deps && _deps.closeSetupModal) return _deps.closeSetupModal();
  try {
    const { closeAgentUIModal } = require('../agent-ui-modal-manager');
    closeAgentUIModal('meeting-starter');
  } catch (_e) {
    /* best-effort; the meeting window is already up */
  }
}

function _now() {
  return _deps && _deps.now ? _deps.now() : Date.now();
}
...
// Set when a meeting successfully starts. While recent, the setup modal must
// NOT reopen (the agent's reflex is to refresh it after every tool call, which
// is how users ended up with a second "meeting window" stacked under the real
// one). Editing the draft again clears it — that's the user organizing anew.
let _lastStartedAtMs = 0;
const REOPEN_SUPPRESS_MS = 20000;

/**
 * True when a meeting started within `windowMs` (default 20s) and the draft
 * has not been touched since. meeting_open_setup checks this to keep the
 * setup modal closed once the real meeting window is up.
 */
function wasJustStarted(windowMs = REOPEN_SUPPRESS_MS) {
  return _lastStartedAtMs > 0 && _now() - _lastStartedAtMs < windowMs;
}
...
function updateDraft(changes = {}) {
  _lastStartedAtMs = 0; // editing again = organizing a new meeting; modal allowed
...
async function startMeeting(overrides = {}) {
  ...
  clearDraft();
  // The real meeting window is up — the setup modal's job is done. Close it
  // here (code-enforced, not prompt-enforced) and suppress reopens briefly so
  // the agent's refresh habit can't stack a second window under the meeting.
  _lastStartedAtMs = _now();
  _closeSetupModal();
  return { started: true, spaceId: spaceId || null, spaceName, participants, title };
}

module.exports = { getDraft, updateDraft, clearDraft, startMeeting, wasJustStarted, _setTestDeps };

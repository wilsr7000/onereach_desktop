...
class AutoLoginManager {
  constructor(credentialManager) {
    ...
    console.log('[AutoLogin] Manager initialized');
  }
  ...
  setEnabled(enabled) {
    ...
    console.log(`[AutoLogin] ${enabled ? 'Enabled' : 'Disabled'}`);
  }
  ...
  async handleNavigation(webContents, url) {
    ...
    try {
      ...
      console.log(`[AutoLogin] Page type detected: ${pageType} for ${url}`);
      ...
      console.error('[AutoLogin] Navigation handling error:', error.message);
  ...
  async detectPageType(webContents) {
    try {
      ...
      console.error('[AutoLogin] Page detection error:', error.message);
      return 'error';
    }
  }
   ...
   */
  async handleLoginPage(webContents, url) {
    const credentials = await this.credentialManager.getOneReachCredentials();
    
    if (!credentials || !credentials.email || !credentials.password) {
      console.log('[AutoLogin] No credentials configured, skipping auto-login');
      return { handled: false, reason: 'no credentials configured' };
    }

    console.log(`[AutoLogin] Filling login form for ${credentials.email}`);
    ...
    try {
      ...
      console.log('[AutoLogin] Login form result:', result);
      return { handled: true, ...result };
      
    } catch (error) {
      console.error('[AutoLogin] Login form fill error:', error.message);
      return { handled: false, reason: error.message };
    }
  }
   ...
   */
  async handle2FAPage(webContents, url) {
    const credentials = await this.credentialManager.getOneReachCredentials();
    
    if (!credentials || !credentials.totpSecret) {
      console.log('[AutoLogin] No TOTP secret configured, skipping 2FA auto-fill');
    ...
    try {
      ...
      console.log(`[AutoLogin] Generated 2FA code: ${code.slice(0, 2)}****`);
      ...
      console.log('[AutoLogin] 2FA form result:', result);
      return { handled: true, ...result };
      
    } catch (error) {
      console.error('[AutoLogin] 2FA form fill error:', error.message);
      return { handled: false, reason: error.message };
    }
  }

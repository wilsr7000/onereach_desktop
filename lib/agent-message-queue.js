...
class AgentMessageQueue extends EventEmitter {
  ...
  canSpeak() {
    ...
    if (this.canSpeakFn && typeof this.canSpeakFn === 'function') {
      try {
        ...
        log.warn('agent', 'canSpeakFn error', { error: e.message });
      }
    }
  ...
  enqueue(agentId, message, priority = 'normal', options = {}) {
    if (!message || message.trim() === '') {
      log.info('agent', 'Ignoring empty message');
      return null;
    }

    // Convert string priority to number
    const priorityNum = typeof priority === 'string' ? (PRIORITY[priority.toUpperCase()] ?? PRIORITY.NORMAL) : priority;
    ...
    if (this.queue.length >= this.config.maxQueueSize) {
      ...
      log.info('agent', 'Queue full, removed', { removed: removed.id });
    }

    // Insert by priority
    this.insertByPriority(item);

    log.info('agent', 'Enqueued from : "..." (priority: )', {
      agentId: agentId,
  ...
  removeExpired() {
    ...
    this.queue = this.queue.filter((item) => {
      if (item.expiresAt <= now) {
        log.info('agent', 'Message expired', { item: item.id });
    ...
    if (this.queue.length < before) {
      log.info('agent', 'Removed expired messages', { beforethis: before - this.queue.length });
    }
  }
  ...
  async processNext() {
    ...
    if (!this.speakFn) {
      log.warn('agent', 'No speak function configured');
      return;
    }
    ...
    log.info('agent', 'Speaking message from : "..."', { item: item.agentId, item: item.message.slice(0, 40) });
    this.emit('message_speaking', item);

    try {
      // Pass the full item so the speak function can tag proactive agent
      // speech (vs phantom/orphan audio from other paths). Consumers that
      // only want the text can safely ignore the second arg.
      await this.speakFn(item.message, item);

      log.info('agent', 'Message delivered', { item: item.id });
      this.emit('message_delivered', item);
    } catch (error) {
      log.error('agent', 'Speak error', { error: error.message });
  ...
  stopPolling() {
    if (this.pollInterval) {
      ...
      this.pollInterval = null;
      log.info('agent', 'Stopped poll (queue empty)');
    }
  }
  ...
  clearAgent(agentId) {
    ...
    if (removed > 0) {
      log.info('agent', 'Cleared messages from', { removed: removed, agentId: agentId });
    }
  ...
  clearAll() {
    ...
    log.info('agent', 'Cleared all messages', { count: count });
    this.emit('cleared');

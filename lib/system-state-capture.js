...
class SystemStateCapture {
  ...
  async getRunningProcesses() {
    try {
      ...
      const processes = stdout.trim().split(', ').map(p => p.trim().toLowerCase());
      return processes;
    } catch (error) {
      return [];
    }
  ...
  async getWindowList() {
    try {
      ...
      const windows = [];
      const matches = stdout.matchAll(/appName:([^,]+), winTitle:([^,]+), winPosition:\{([^}]+)\}, winSize:\{([^}]+)\}/g);
  ...
  async getFinderState() {
    try {
      ...
      return {
        ...
        selection: selection.trim().split(', ').filter(s => s),
  ...
  async getSystemSettings() {
    try {
      ...
      const { stdout: muted } = await execAsync(
   ...
   */
  generateDiffSummary(changes, unchanged) {
    if (changes.length === 0) {

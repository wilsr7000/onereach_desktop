...
const { matchTemplate, buildAgentPrompt, getTemplates, getTemplate, UI_PROMPT_SUFFIX } = require('./agent-templates');
...
async function generateAgentFromDescription(description, options = {}) {
  ...
  log.info('agent', 'Using template', { matchedTemplate: matchedTemplate.name, data: '(' + matchedTemplate.executionType + ')' });
...
${systemPrompt}`;
    ...
    try {
      ...
      log.error('agent', 'Failed to parse response', { response: response });
    ...
    // If uiCapable, append UI instructions to the agent's prompt
    const isUiCapable = options.uiCapable === true;
    const finalPrompt = isUiCapable
      ? config.prompt.trim() + UI_PROMPT_SUFFIX
      : config.prompt.trim();
    ...
    log.info('agent', 'Generated agent config', { normalizedConfig: normalizedConfig.name, type: '(type:', normalizedConfig: normalizedConfig.executionType + ')' });
    return normalizedConfig;

  } catch (error) {
    log.error('agent', 'Error generating agent', { error: error });
    throw error;
  }
}

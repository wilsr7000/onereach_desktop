...
const DEFAULT_SPACES = {
  ...
  'meeting-agents': {
    ...
    icon: '~',
    color: '#a78bfa',
    agentIds: ['action-item-agent', 'decision-agent', 'meeting-notes-agent', 'time-agent', 'calendar-query-agent'],
  ...
  try {
    ...
    for (const space of allSpaces) {
      try {
        ...
        if (meta.attributes?.category === AGENT_SPACE_CATEGORY || meta.agentSpace) {
...
async function getAgentIdsInSpace(spaceId) {
  ...
  return space.agents.filter((a) => a.enabled !== false).map((a) => a.id);
}

...
class UnifiedTaskQueue extends EventEmitter {
  ...
  complete(taskId, result = {}) {
    ...
    task.queueMetadata.duration = new Date(task.queueMetadata.completedAt) - new Date(task.queueMetadata.addedAt);
    task.result = result;
  ...
  query(filters = {}) {
    ...
    if (filters.sortBy === 'priority') {
      results.sort((a, b) => (a.queueMetadata?.priority ?? 5) - (b.queueMetadata?.priority ?? 5));
    } else {
   ...
   */
  getUndoable() {
    return this.undoQueue.map((id) => this.tasks.get(id)).filter(Boolean);
  }

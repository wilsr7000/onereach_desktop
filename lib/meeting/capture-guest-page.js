...
// v18: big-room grid (7+ scrolls, speaking-first), large-meeting header
// badge with head-count, featured screen share (tap to feature), scrollable
// presentation strip with a 96px tile floor, indefinite wait-for-host.
const GUEST_PAGE_VERSION = 18;
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  html, body {
    height: 100%;
    /* Mobile browsers: 100% tracks the LAYOUT viewport, which is taller than
       the visible area while the URL bar is shown — with overflow:hidden the
       controls bar ends up cut off below the fold (worst in the tall
       screen-share layout). dvh tracks the VISIBLE viewport; browsers without
       dvh support ignore this line and keep the 100% fallback. */
    height: 100dvh;
  ...
  /* Join Panel */
  .lobby-preview-wrap { position: relative; width: 100%; aspect-ratio: 16/9;
    background: #111; border-radius: 10px; overflow: hidden; margin: 10px 0 8px;
    border: 1px solid rgba(255,255,255,0.10); }
  .lobby-preview { width: 100%; height: 100%; object-fit: cover; transform: scaleX(-1); display: block; }
  .lobby-preview-off { position: absolute; inset: 0; display: none; align-items: center;
    justify-content: center; color: rgba(255,255,255,0.45); font-size: 13px; }
  .lobby-preview-wrap.cam-off .lobby-preview { display: none; }
  .lobby-preview-wrap.cam-off .lobby-preview-off { display: flex; }
  .lobby-controls { display: flex; gap: 8px; margin: 0 0 6px; }
  .lobby-toggle { flex: 1; padding: 8px 10px; border-radius: 8px; font-size: 13px;
    border: 1px solid rgba(255,255,255,0.18); background: rgba(255,255,255,0.06);
    color: rgba(255,255,255,0.55); cursor: pointer; }
  .lobby-toggle.on { background: rgba(52,199,89,0.15); border-color: rgba(52,199,89,0.5);
    color: #eaffea; }
  .lobby-hint { font-size: 11px; color: rgba(255,255,255,0.35); margin: 0 0 10px; }
  .lobby-roster { font-size: 12.5px; color: rgba(255,255,255,0.65); margin: 0 0 8px;
    padding: 7px 10px; background: rgba(255,255,255,0.05); border-radius: 8px;
    border: 1px solid rgba(255,255,255,0.08); }
  .lobby-roster .roster-count { color: #7ee2a0; font-weight: 600; }
  .join-panel {
    flex: 1;
    display: flex;
    flex-direction: column;
  ...
  .session-view {
    flex: 1;
    /* Without this the flex item's min-height:auto default lets the session
       view grow to its CONTENT height (share area + strip + panel + controls)
       and overflow .app — body's overflow:hidden then clips the bottom, so
       turning on screen share visually "removed" the mute/camera controls on
       phones (measured: 709px of content in a 624px slot). min-height:0 lets
       the share area compress instead; the controls bar always stays on
       screen. */
    min-height: 0;
    display: none;
    flex-direction: column;
  ...
  /* Big room (7+): tiles keep a legible minimum size and the grid SCROLLS
     instead of compressing everyone into slivers; whoever is speaking
     bubbles to the front of the grid. */
  .video-grid.big-room {
  ...
  .video-grid.big-room .video-cell.speaking {
    order: -1;
  }

  /* Large-meeting signal in the header */
  .badge.badge-bigroom {
    display: none;
  ...
  /* Dedicated screen share container. Click one to feature it — the
     featured share takes most of the area (works for the row layout on
     desktop and the column stack on phones alike). */
  .screen-share-container {
    position: relative;
    width: 100%;
    flex: 1;
  ...
  .screen-share-area .screen-share-container.featured {
    flex: 3 1 0%;
  }
  ...
  .video-grid.presentation-mode .video-cell {
    flex: 0 1 160px;
    /* Floor keeps tiles legible with many participants; the strip scrolls
       horizontally instead of crushing them into slivers. */
    min-width: 96px;
    height: 100%;
  }
  ...
  /* "CC" chip: the meeting agent is transcribing (host broadcasts state) */
  .cc-live {
    display: none;
    align-items: center;
    gap: 5px;
  ...
  /* Live caption bar floated above the controls (host's transcription) */
  .guest-caption {
    position: fixed;
    left: 12px;
    right: 12px;
  ...
  .guest-caption.visible { display: flex; }
  ...
  .guest-caption.interim .guest-caption-text {
...
<div class="app">
  ...
  <div class="header">
    ...
    <div>
      <span class="badge badge-bigroom" id="badgeBigRoom">Large meeting</span>
  ...
  <div class="join-panel" id="joinPanel">
    ...
    <div class="join-form">
      ...
      <!-- Pre-join lobby (v16): see yourself + choose camera/mic BEFORE
           entering. Nothing is transmitted until Join is clicked. -->
      <div class="lobby-preview-wrap" id="lobbyPreviewWrap">
        <video class="lobby-preview" id="lobbyPreview" autoplay playsinline muted></video>
        <div class="lobby-preview-off" id="lobbyPreviewOff">Camera is off</div>
      </div>
      <div class="lobby-controls">
        <button type="button" class="lobby-toggle on" id="lobbyCamBtn" onclick="guest.toggleLobbyCam()"
                aria-pressed="true" title="Turn camera off">🎥 Camera on</button>
        <button type="button" class="lobby-toggle on" id="lobbyMicBtn" onclick="guest.toggleLobbyMic()"
                aria-pressed="true" title="Turn microphone off">🎙 Mic on</button>
      </div>
      <div class="lobby-roster" id="lobbyRoster" style="display:none"></div>
      <div class="lobby-hint">You can change these anytime in the meeting.</div>
  ...
  <div class="session-view" id="sessionView">
    ...
    <!-- Live caption bar (host transcription, broadcast to guests) -->
    <div class="guest-caption" id="guestCaption">
      <div class="guest-caption-text" id="guestCaptionText"></div>
    </div>
    <div class="controls-bar">
      ...
      <div class="cc-live" id="ccLive" title="The meeting agent is transcribing live">
        <span class="cc-live-dot"></span>CC
      </div>
...
const guest = {
  ...
  init() {
    ...
    const hideLobby = () => {
      for (const id of ['lobbyPreviewWrap', 'lobbyCamBtn', 'lobbyMicBtn', 'lobbyRoster']) {
    ...
    // Pre-join lobby (v16): show yourself before you enter. Non-fatal
    // if the camera is denied — the toggle flips off and join still works.
    void this._startLobbyPreview();
    // v17: show who's already in the room while you decide.
    this._startLobbyRoster();
  ...
  // ── Pre-join lobby (v16) ──────────────────────────────────────────
  // Camera/mic choice BEFORE entering. The preview stream is local-only
  // and stopped at join; join() then requests exactly the chosen kinds.
  _lobbyCam: true,
  _lobbyMic: true,
  _lobbyStream: null,

  // ── Lobby roster (v17): who's already in the room ─────────────────
  // Polls the SAME signed KV payload the join uses (host republishes it
  // with a participants list while live). Signature-verified, so a
  // tampered roster reads as "no payload" and simply shows nothing.
  _rosterTimer: null,

  _stopLobbyRoster() {
    if (this._rosterTimer) { clearInterval(this._rosterTimer); this._rosterTimer = null; }
  },

  _startLobbyRoster() {
    this._stopLobbyRoster();
    if (!this._joinKey || !KV_URL) return;
    const poll = async () => {
      try {
        const roomRaw = (document.getElementById('codeInput').value || '').trim().toLowerCase()
          .replace(/[^a-z0-9-]+/g, '-').replace(/^-+|-+$/g, '');
        const el = document.getElementById('lobbyRoster');
        if (!el) return;
        if (!roomRaw) { el.style.display = 'none'; return; }
        const verifyKey = await crypto.subtle.importKey(
          'raw', this._b64uToBytes(this._joinKey),
          { name: 'ECDSA', namedCurve: 'P-256' }, false, ['verify']
        );
        const kvKey = 'wiser-room:' + roomRaw;
        const resp = await fetch(KV_URL + '?id=' + encodeURIComponent(KV_COLLECTION) + '&key=' + encodeURIComponent(kvKey));
        if (!resp.ok) { el.style.display = 'none'; return; }
        const kvData = await resp.json();
        if (kvData.Status === 'No data found.' || !kvData.value) { el.style.display = 'none'; return; }
        const parsed = typeof kvData.value === 'string' ? JSON.parse(kvData.value) : kvData.value;
        const data = await this._verifySignedPayload(parsed, roomRaw, verifyKey);
        if (!data) { el.style.display = 'none'; return; }
        const names = Array.isArray(data.participants)
          ? data.participants.map((n) => String(n).slice(0, 60)).filter((n) => n.length > 0)
          : [];
        el.style.display = '';
        el.textContent = '';
        if (names.length === 0) {
          el.textContent = 'No one is in the room yet — you can be first.';
        } else {
          const count = document.createElement('span');
          count.className = 'roster-count';
          count.textContent = names.length === 1 ? '1 person in the room: ' : names.length + ' in the room: ';
          el.appendChild(count);
          const shown = names.slice(0, 8);
          const extra = names.length - shown.length;
          el.appendChild(document.createTextNode(shown.join(', ') + (extra > 0 ? ' +' + extra + ' more' : '')));
        }
      } catch {
        /* roster is garnish — never break the lobby */
      }
    };
    void poll();
    this._rosterTimer = setInterval(() => { void poll(); }, 5000);
  },

  async _startLobbyPreview() {
    if (!this._lobbyCam) return;
    try {
      this._lobbyStream = await navigator.mediaDevices.getUserMedia({
        video: { facingMode: 'user' }, audio: false
      });
      const v = document.getElementById('lobbyPreview');
      if (v) v.srcObject = this._lobbyStream;
    } catch (err) {
      // Denied / no camera: reflect reality in the toggle, keep join usable.
      this._lobbyCam = false;
      this._renderLobbyToggles();
    }
  },

  _stopLobbyPreview() {
    if (this._lobbyStream) {
      try { this._lobbyStream.getTracks().forEach(t => t.stop()); } catch {}
  ...
  _renderLobbyToggles() {
    const cam = document.getElementById('lobbyCamBtn');
    const mic = document.getElementById('lobbyMicBtn');
    const wrap = document.getElementById('lobbyPreviewWrap');
    if (cam) {
      cam.classList.toggle('on', this._lobbyCam);
      cam.setAttribute('aria-pressed', String(this._lobbyCam));
      cam.textContent = this._lobbyCam ? '🎥 Camera on' : '🎥 Camera off';
      cam.title = this._lobbyCam ? 'Turn camera off' : 'Turn camera on';
    }
    if (mic) {
      mic.classList.toggle('on', this._lobbyMic);
      mic.setAttribute('aria-pressed', String(this._lobbyMic));
      mic.textContent = this._lobbyMic ? '🎙 Mic on' : '🎙 Mic off';
      mic.title = this._lobbyMic ? 'Turn microphone off' : 'Turn microphone on';
    }
    if (wrap) wrap.classList.toggle('cam-off', !this._lobbyCam);
  },

  toggleLobbyCam() {
    this._lobbyCam = !this._lobbyCam;
    this._renderLobbyToggles();
    if (this._lobbyCam) { void this._startLobbyPreview(); }
    else { this._stopLobbyPreview(); }
  },

  toggleLobbyMic() {
    this._lobbyMic = !this._lobbyMic;
    this._renderLobbyToggles();
  },
  ...
  async join() {
    ...
    try {
      ...
      // exactly the kinds chosen in the lobby (v16). Both off = join
      // without media; the in-meeting toggles can enable them later.
      this._stopLobbyPreview();
      this._stopLobbyRoster();
      this.setStep('Requesting camera and microphone...');
      try {
        this.localStream = (this._lobbyCam || this._lobbyMic)
          ? await navigator.mediaDevices.getUserMedia({
              video: this._lobbyCam
                ? { facingMode: 'user', width: { ideal: 1280 }, height: { ideal: 720 } }
                : false,
              audio: this._lobbyMic
            })
          : null;
      } catch (mediaErr) {
        ...
        if (name === 'NotAllowedError' || name === 'PermissionDeniedError') {
          ...
          try {
            this.localStream = await navigator.mediaDevices.getUserMedia({
              video: this._lobbyCam, audio: this._lobbyMic });
          } catch {
  ...
  async _fetchTokensWithWait(kvKey, roomName, verifyKey) {
    // Join-any-time: wait for the host INDEFINITELY (Cancel is always on
    // screen). The old 5-minute cap dead-ended early guests with "the host
    // has not started this meeting yet" — anyone who left the tab open
    // silently missed the start. Polling backs off 5s -> 15s after the
    // first couple of minutes, and pauses while the tab is hidden.
    const POLL_INTERVAL_MS = 5000;
    const SLOW_POLL_INTERVAL_MS = 15000;
    const SLOW_AFTER_POLLS = 24;
    ...
    for (let attempt = 0; ; attempt++) {
      ...
      const waitMs = attempt >= SLOW_AFTER_POLLS ? SLOW_POLL_INTERVAL_MS : POLL_INTERVAL_MS;
      if (document.hidden) {
        // Hidden tab: hold position without hammering the KV endpoint.
  ...
  async _connectWithTokenRetry() {
    ...
    this.room.on(RoomEvent.DataReceived, (data, participant) => {
      try {
        ...
        if (msg.type === 'recording-start' || msg.type === 'recording-stop' ||
            ...
            msg.type === 'live-caption' || msg.type === 'cc-state') {
        ...
        } else if (msg.type === 'live-caption') {
          this._handleLiveCaption(msg);
        } else if (msg.type === 'cc-state') {
          this._setCcState(msg.active);
        }
      } catch {}
    });
    ...
    } else if (this._lobbyCam || this._lobbyMic) {
      // Fallback if localStream was lost — let LiveKit request its own,
      // still honoring the lobby choice.
      if (this._lobbyCam) await this.room.localParticipant.setCameraEnabled(true);
      if (this._lobbyMic) await this.room.localParticipant.setMicrophoneEnabled(true);
    }
  ...
  onDisconnected(state) {
    ...
    // No host connection => no captions
    this._setCcState(false);
  ...
  async sendRecordingToHost() {
    ...
    // A sub-8KB webm is a headerless/empty capture (mic or camera never
    // delivered data). Don't take over the screen with upload UI for a file
    // that contains nothing.
    if (this.recordedBlob.size < 8192) {
      console.warn('[Guest] Recording empty (' + this.recordedBlob.size + ' bytes) -- skipping transfer');
      this.showStatus('Recording was empty — nothing to save', 'error');
    ...
    // If the meeting is already gone -- room disconnected OR the host has
    // left -- a chunk upload is doomed (this is how "started uploading, then
    // failed" happens when a meeting ends abruptly). Checking that the room
    // OBJECT exists is not enough: it survives disconnection. Go straight to
    // the download option with an honest message instead.
    const roomConnected = !!(this.room && this.room.localParticipant && this.room.state === 'connected');
    let hostPresent = false;
    if (roomConnected) {
      try {
        for (const p of this.room.remoteParticipants.values()) {
          if (p.identity === 'host') { hostPresent = true; break; }
        }
      } catch (_e) { /* remoteParticipants unavailable -- treat as absent */ }
    }
    if (!roomConnected || !hostPresent) {
      document.getElementById('transferTitle').textContent = roomConnected
        ? 'Host has left the meeting'
        : 'Meeting connection closed';
      document.getElementById('transferStatus').textContent =
        'Your recording is safe on this device — download it to keep it.';
      this._showTransferActions(true, false);
      return;
    }
  ...
  _showScreenShare(videoElement, trackSid, participantName) {
    ...
    wrapper.title = 'Tap to feature this screen';
    const self = this;
    wrapper.addEventListener('click', function () {
      self._toggleFeaturedShare(trackSid);
    });
  ...
  // Click/tap a share to feature it (it takes most of the share area, in
  // the desktop row and the phone column stack alike); click it again to
  // return to the even split. One share needs no featuring.
  _toggleFeaturedShare(trackSid) {
    if (this._screenShares.size < 2) return;
    const target = this._screenShares.get(trackSid);
    if (!target) return;
    const wasFeatured = target.classList.contains('featured');
    this._screenShares.forEach(function (wrapper) {
      wrapper.classList.remove('featured');
    });
    if (!wasFeatured) target.classList.add('featured');
  },
  ...
  // ── Live captions from the host ─────────────────────────────
  // Transcription runs on the host (it holds the API key + audio mix); the
  // host broadcasts throttled interims + finals plus a cc-state signal so
  // guests can SEE the transcription happening.
  _ccActive: false,
  _captionHideTimer: null,

  _setCcState(active) {
    this._ccActive = !!active;
    const chip = document.getElementById('ccLive');
    if (chip) chip.classList.toggle('visible', this._ccActive);
    if (!this._ccActive) this._hideGuestCaption();
  },

  // The controls bar wraps to variable heights on narrow screens, so the
  // caption bar's offset is measured from the live controls rect rather than
  // hard-coded (a fixed 76px sat inside the wrapped bar on phones).
  _positionGuestCaption() {
    const bar = document.getElementById('guestCaption');
    const ctrl = document.querySelector('.controls-bar');
    if (!bar || !ctrl) return;
    const controlsHeight = window.innerHeight - ctrl.getBoundingClientRect().top;
    if (controlsHeight > 0 && controlsHeight < window.innerHeight) {
      bar.style.bottom = (controlsHeight + 8) + 'px';
    }
  },

  _handleLiveCaption(msg) {
    if (!msg || !msg.text) return;
    if (!this._ccActive) this._setCcState(true); // caption implies CC is live
    const bar = document.getElementById('guestCaption');
    const txt = document.getElementById('guestCaptionText');
    if (!bar || !txt) return;
    this._positionGuestCaption();
    // textContent only -- caption text is remote data, never markup.
    txt.textContent = (msg.speaker ? msg.speaker + ': ' : '') + msg.text;
    bar.classList.toggle('interim', !msg.final);
    bar.classList.add('visible');
    clearTimeout(this._captionHideTimer);
    // Finals linger briefly; interims stay until replaced or CC stops.
    if (msg.final) {
      this._captionHideTimer = setTimeout(() => bar.classList.remove('visible'), 5000);
    }
  },
  ...
  _updateGridLayout() {
    ...
    const bigRoom = count > 6;

    // Large-meeting signal: header badge carries the live head-count.
    const bigBadge = document.getElementById('badgeBigRoom');
    if (bigBadge) {
      bigBadge.classList.toggle('visible', bigRoom);
      if (bigRoom) bigBadge.textContent = 'Large meeting - ' + count + ' people';
    }
    ...
    if (count >= 2 && count <= 6) {
      ...
      // Big room: legible-minimum tiles + a scrolling grid (7+ used to fall
      // into an auto-fill grid that compressed tiles without limit).
      grid.classList.add('big-room');
    }
  },

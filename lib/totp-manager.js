...
class TOTPManager {
  constructor() {
    console.log('[TOTPManager] Initialized with standard TOTP settings (otplib v13)');
  }
  ...
  parseOTPAuthURI(uri) {
    try {
      ...
      if (type !== 'totp') {
        console.warn(`[TOTPManager] Warning: ${type} type detected, only TOTP is fully supported`);
      }
      ...
      console.error('[TOTPManager] Failed to parse OTP URI:', error.message);
  ...
  generateCode(secret) {
    try {
      ...
      console.error('[TOTPManager] Failed to generate code:', error.message);
  ...
  verifyCode(secret, code) {
    try {
      ...
      console.error('[TOTPManager] Verification error:', error.message);
      return false;
    }
  }
  ...
  isValidSecret(secret) {
    try {
      ...
      if (cleanSecret.length < 16) {
        console.warn('[TOTPManager] Secret may be too short:', cleanSecret.length, 'chars');
      ...
      console.error('[TOTPManager] Invalid secret:', error.message);
      return false;
    }
  }

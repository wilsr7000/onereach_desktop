...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Presentation mode: grid becomes a small strip below the screen share */
...
const guest = {
  ...
  _screenShareEl: null,           // dedicated screen share container element
  ...
  async join() {
    ...
    try {
      ...
      const meetingData = await this._fetchTokensWithWait(kvKey, roomName);
  ...
  async _fetchTokensWithWait(kvKey, roomName) {
    ...
    this._waitingForHost = false;
    throw { userMessage: 'The host has not started this meeting yet.', helpText: 'Try again later or ask the host to start the meeting.' };
  },
  ...
  leave() {
    ...
    // Remove screen share container if present
    this._hideScreenShare();
  ...
  _showScreenShare(videoElement, trackSid, participantName) {
    ...
    wrapper.id = 'screenShareContainer';
    ...
    label.textContent = (participantName || 'Participant') + ' is sharing';
    wrapper.appendChild(label);
    const grid = document.getElementById('videoGrid');
    grid.parentNode.insertBefore(wrapper, grid);
    this._screenShareEl = wrapper;
    this._updateGridLayout();
  },

  _hideScreenShare() {
    if (this._screenShareEl) {
      this._screenShareEl.remove();
      this._screenShareEl = null;
      this._updateGridLayout();
    }
  },
  ...
  _showOverlay(overlay) {
    ...
    el.innerHTML = '<div style="flex:1;min-width:0"><div style="font-size:9px;text-transform:uppercase;letter-spacing:0.05em;color:#8e8e93;margin-bottom:2px">' + (overlay.source || 'AI') + '</div><div>' + (overlay.content || '') + '</div></div><button onclick="this.parentElement.remove()" style="background:none;border:none;color:#8e8e93;cursor:pointer;font-size:16px;padding:0">&times;</button>';
    stack.appendChild(el);
    const ttl = (overlay.ttl || 15) * 1000;
    const timer = setTimeout(() => { el.style.opacity = '0'; el.style.transition = 'opacity 0.5s'; setTimeout(() => el.remove(), 500); }, ttl);

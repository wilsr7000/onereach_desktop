...
Does this request need clarification, or can the agent proceed with available information?`;
  ...
  try {
    ...
    log.warn('agent', 'AI clarification check failed', { error: e.message });
  }
...
Did this fulfill the user's intent?`;
  ...
  try {
    ...
    log.warn('agent', 'AI review failed', { error: e.message });
  }
...
async function learnFromInteraction(memory, task, result, context = {}) {
  ...
  if (context.learnedPreferences) {
    ...
    learnedKeys.push(...Object.keys(context.learnedPreferences));
  }
  ...
  if (context.useAILearning && result.success) {
    ...
    if (aiLearnings) {
      ...
      memory.updateSectionAsKeyValue('Learned Preferences', updatedPrefs);
      learnedKeys.push(...Object.keys(aiLearnings));
    }
  }
  ...
  // Disclosure contract (UC-04): report WHAT was durably learned, so the
  // caller can tell the user. History-only appends do not count.
  return learnedKeys.length ? { learnedKeys } : null;
}
...
What preferences should be remembered?`;
  ...
  try {
    ...
    log.warn('agent', 'AI learning extraction failed', { error: e.message });
  }
...
function createThinkingExecute(agent, config = {}) {
  return async function thinkingExecute(task) {
    try {
      ...
      const clarification = await checkPreferencesAndClarify(agent.id, task, memory, config);
      ...
      log.error('agent', 'Thinking execute error', { error: error });
      return {
        success: false,

...
const QUOTE_FACTORS = {
  ...
  MEMORABILITY: 0.2, // Likely to be remembered
...
const QUOTE_PATTERNS = {
  ...
  emotional: [/I (love|hate|believe|feel)/i, /honestly/i, /the truth is/i, /I have to say/i],
};
...
export class QuoteFinder {
  ...
  async findBestQuotes(transcriptSegments, options = {}) {
    const { maxQuotes = this.maxQuotes, minLength = this.minQuoteLength, maxLength = this.maxQuoteLength } = options;
    ...
    try {
      ...
      this.quotes = candidates.sort((a, b) => b.score - a.score).slice(0, maxQuotes);
  ...
  async detectTopics(transcriptSegments) {
    ...
    try {
      ...
      for (let i = 0; i < words.length; i++) {
        ...
        const _speakerChanged = i > 0 && words[i].speaker && words[i].speaker !== words[i - 1].speaker;
  ...
  convertToMarkers(markerManager) {
    ...
    this.quotes.forEach((quote) => {
      markerManager.addSpotMarker(quote.startTime, `💬 ${quote.text.substring(0, 30)}...`, '#8b5cf6', {

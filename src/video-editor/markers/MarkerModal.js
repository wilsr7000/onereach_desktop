...
export class MarkerModal {
  ...
  _updateModalElements(time, editMarker) {
    ...
    if (createdEl) {
      createdEl.textContent = editMarker?.createdAt 
        ? new Date(editMarker.createdAt).toLocaleString() 
        : 'Now';
    }
    if (modifiedEl) {
      modifiedEl.textContent = editMarker?.modifiedAt 
        ? new Date(editMarker.modifiedAt).toLocaleString() 
        : '-';
    }
  ...
  save() {
    ...
    if (this.editingMarkerId) {
      // Update existing
      const marker = manager?.getById(this.editingMarkerId) || 
                     markers.find(m => m.id === this.editingMarkerId);
      if (marker) {

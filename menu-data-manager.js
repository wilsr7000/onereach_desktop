...
class MenuDataManager extends EventEmitter {
  ...
  async setIDWEnvironments(environments, options = {}) {
    ...
    log.info('menu', 'setIDWEnvironments called from ... with ... items', {
      source,
  ...
  async setImageCreators(creators, options = {}) {
    const { skipRefresh = false } = options;
    log.info('menu', 'setImageCreators: ... creators, skipRefresh=...', {
  ...
  async setVideoCreators(creators, options = {}) {
    const { skipRefresh = false } = options;
    log.info('menu', 'setVideoCreators: ... creators, skipRefresh=...', {
      detail: (creators || []).length,
      skipRefresh,
    });
  ...
  async setAudioGenerators(generators, options = {}) {
    const { skipRefresh = false } = options;
    log.info('menu', 'setAudioGenerators: ... generators, skipRefresh=...', {
  ...
  async removeIDWEnvironment(id) {
    ...
    log.info('menu', 'removeIDWEnvironment: ... -> ...', {
  ...
  getOpenableItems() {
    ...
    try {
      ...
      log.info('menu', 'getOpenableItems returned ... items in ...ms', {
        itemsCount: items.length,
        detail: Date.now() - startTime,
      });
      return items;
    } catch (error) {
  ...
  async findMenuItem(query) {
    ...
    try {
      ...
      if (result) {
        log.info('menu', 'findMenuItem matched "..." in ...ms', {
          detail: result.name || result.label,
          detail: Date.now() - startTime,
        });
      } else {
  ...
  _registerIPCHandlers() {
    ...
    ipcMain.handle('menu-data:get-idw-environments', () => {
      logCount: log.info('menu', 'get-idw-environments');
  ...
  _registerLegacyHandlers() {
    ...
    ipcMain.on('save-idw-environments', async (event, environments) => {
      logCount: log.info('menu', 'save-idw-environments (... envs)', { detail: (environments || []).length });

...
Select the appropriate scenes and return JSON.`;
  ...
  async transcribeRange(inputPath, options = {}) {
    ...
    try {
      ...
      // Response format - use verbose_json for word timestamps
      parts.push(Buffer.from(
        `--${boundary}\r\n` +
      ...
      let result;
      try {
        result = JSON.parse(transcription);
      } catch (e) {
        // Fallback if not JSON (shouldn't happen with verbose_json)
        return {
          success: true,
      ...
      // Extract word-level timestamps and adjust for startTime offset
      const words = (result.words || []).map(w => ({
        text: w.word,
        start: (w.start || 0) + startTime,  // Offset by range start time
        end: (w.end || w.start + 0.3) + startTime
      }));

      console.log(`[VideoEditor] Got ${words.length} word timestamps`);

      return {
        success: true,
        transcription: result.text || transcription.trim(),
        words: words,  // Word-level timestamps
        segments: result.segments || [],  // Segment-level timestamps
        startTime,
        endTime: startTime + duration,
        duration,

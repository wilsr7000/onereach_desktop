...
class CopyStyleExtractor {
  ...
  async extract(url, options = {}) {
    ...
    try {
      ...
      const copyGuide = await page.evaluate(() => {
        ...
        const _schemaOrg = document.querySelector('script[type="application/ld+json"]');
        ...
        const heroSubtext = document.querySelector(
          '.hero p, [class*="hero"] p, header p, .tagline, [class*="tagline"]'
        )?.textContent;
        ...
        const actionPatterns =
          /\b(get|start|try|discover|explore|learn|build|create|grow|boost|improve|transform|unlock|achieve|accelerate|simplify|automate|streamline|optimize|scale|launch|join|sign up|subscribe|download|install|upgrade|activate)\b/gi;
        ...
        const emotionalPatterns =
          /\b(free|easy|simple|fast|secure|powerful|beautiful|amazing|incredible|revolutionary|innovative|trusted|reliable|seamless|effortless|intuitive|smart|intelligent|premium|exclusive|unlimited|guaranteed)\b/gi;
        ...
        const socialProofPatterns =
          /\b(trusted by|used by|loved by|preferred by|recommended by|as seen in|featured in|award-winning|#1|leading|top-rated|best-in-class|industry-leading|world-class|enterprise-grade|million|billion|customers|users|companies|teams)\b/gi;
        ...
        const formalIndicators = (
          allText.match(/\b(therefore|furthermore|consequently|regarding|pursuant|hereby|aforementioned)\b/gi) || []
        ).length;
        const casualIndicators = (
          allText.match(/\b(hey|awesome|cool|super|totally|gonna|wanna|yeah|yep|nope|btw)\b/gi) || []
        ).length;
        const technicalIndicators = (
          allText.match(
            /\b(API|SDK|integration|platform|infrastructure|architecture|scalable|deploy|configure|implement)\b/gi
          ) || []
        ).length;
        const emotionalIndicators = (
          allText.match(/\b(love|amazing|incredible|exciting|passionate|inspiring|delightful|wonderful)\b/gi) || []
        ).length;
        const urgentIndicators = (
          allText.match(/\b(now|today|limited|hurry|don't miss|act fast|last chance|expires|deadline)\b/gi) || []
        ).length;
        const friendlyIndicators = (
          allText.match(/\b(we|our|you|your|together|help|support|team|community|welcome)\b/gi) || []
        ).length;
      ...
      console.log(
        `[CopyStyle] Extracted: ${copyGuide.headlines.primary.length} headlines, ${copyGuide.callsToAction.length} CTAs, ${copyGuide.productNames.length} product names`
      );
      return copyGuide;
    } finally {
      await page.close();
  ...
  generateVoiceSummary(guide) {
    const _tones = Object.entries(guide.toneIndicators)
      .sort((a, b) => b[1] - a[1])
      .filter(([_, v]) => v > 0);

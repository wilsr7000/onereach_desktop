...
contextBridge.exposeInMainWorld('claudeCodeAPI', {
  ...
  /**
   * Generate test scenarios for an agent (positive + negative prompts)
   * @param {Object} agent - The agent configuration
   * @returns {Promise<{ success: boolean, scenarios?: { positive: string[], negative: string[] } }>}
   */
  generateTestScenarios: (agent) => ipcRenderer.invoke('gsx-create:generate-scenarios', agent),

  /**
   * Run full test suite: scenario generation + execution + result persistence
   * @param {Object} agent - The agent configuration
   * @returns {Promise<{ success: boolean, results?: Object }>}
   */
  runTestSuite: (agent) => ipcRenderer.invoke('gsx-create:test-suite', agent),
  ...
  compareVersions: (agentId, versionA, versionB) =>
    ipcRenderer.invoke('agents:compare-versions', agentId, versionA, versionB),

  // ==================== V2 Agent Config ====================

  /**
   * Get available voice personalities with descriptions
   * @returns {Promise<Object>} Map of voice name to description
   */
  getAvailableVoices: () => ipcRenderer.invoke('agent-composer:voices'),

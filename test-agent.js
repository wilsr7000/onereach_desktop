...
${JSON.stringify(testableElements, null, 2)}`;
  ...
  extractTestableElements(html) {
    ...
    // Find inputs
    const inputRegex =
      /<input[^>]*(?:id=["']([^"']+)["'])?[^>]*(?:type=["']([^"']+)["'])?[^>]*(?:name=["']([^"']+)["'])?[^>]*(?:placeholder=["']([^"']+)["'])?[^>]*/gi;
      ...
      switch (test.action) {
        case 'click':
          ...
          await this.page
            ...
            .catch((err) => console.warn('[test-agent] waitForLoadState networkidle:', err.message));
          break;
  ...
  async runPerformanceTest(htmlFilePath, options = {}) {
    ...
    try {
      ...
      const _response = await this.page.goto(pathToFileURL(htmlFilePath).href, {

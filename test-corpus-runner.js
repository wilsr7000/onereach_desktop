    ...
    async ({ q, t }) => {
      ...
      return new Promise((resolve) => {
        ...
        const timer = setTimeout(() => {
          unsub();
          resolve({ timedOut: true, result: resultData });
        }, t);

        window.agentHUD
          .submitTask(q, { toolId: 'corpus-test', skipFilter: true })
          .then((sub) => {
...
(async () => {
  ...
  const _mainWindow = await electronApp.firstWindow();
  ...
  for (let i = startIdx; i < CORPUS.length; i++) {
    ...
    const _success = result.result?.success;

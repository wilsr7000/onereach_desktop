...
async function runTests() {
  ...
  try {
    ...
    const setTagsResult = await api.items.setTags(testSpaceId, testItemId, 
      ['replaced-tag-1', 'replaced-tag-2', 'replaced-tag-3']
    );
    ...
    assert(spaceSearchResults.every(item => item.spaceId === testSpaceId), 'All results from specified space');
    ...
    assert(typeSearchResults.every(item => item.type === 'text'), 'Type filter works in search');
    ...
    const unsubscribe = api.on('space:created', (data) => {
      eventReceived = true;
    });

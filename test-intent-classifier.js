...
const { classifyIntent, _AGENT_DEFINITIONS } = require('./packages/agents/intent-classifier');
...
async function runTests() {
  ...
  for (const test of testCases) {
    ...
    try {
      ...
      const matchesExpected = test.isSystem ? result.isSystemCommand === true : result.agentId === test.expectedAgent;
      ...
      if (matchesExpected) {
        ...
        console.log(
          `✗ Expected: ${test.expectedAgent || 'system'}, Got: ${result.agentId} (${result.confidence.toFixed(2)})`
        );
        failed++;
      }
    } catch (error) {

  ...
  async checkWebsite(monitorId) {
    ...
    try {
      ...
      if (monitor.consecutiveErrors >= 3) {
        ...
        console.log(`[WebsiteMonitor] Auto-paused ${monitor.name} after ${monitor.consecutiveErrors} consecutive errors`);
      }
  ...
  isLikelyNoise(changedText) {
    ...
    if (trimmed.length < 20) {
      ...
      return phrases.every(phrase => 
        WebsiteMonitor.NOISE_PATTERNS.some(pattern => pattern.test(phrase))
      );
    }

...
class StylePromptGenerator {
    ...
    generateDesignPrompt(styleGuide, options = {}) {
        ...
        if (styleGuide.cssVariables && Object.keys(styleGuide.cssVariables).length > 0) {
            ...
            Object.entries(styleGuide.cssVariables).slice(0, 15).forEach(([name, value]) => {
                prompt += `${name}: ${value};\n`;
            });
            prompt += `\`\`\`\n\n`;
        }
    ...
    generateFullPrompt(styleGuide, copyGuide, options = {}) {
        ...
        if (styleGuide?.colors) {
            ...
            if (styleGuide.colors.accents?.length > 0) {
                prompt += `- **Accent Colors:** ${styleGuide.colors.accents.slice(0, 3).map(c => c.value).join(', ')}\n`;
            }
            prompt += `\n`;
        }
        ...
        if (copyGuide?.callsToAction?.length > 0) {
            prompt += `## CTA Style\n`;
            prompt += `Examples: ${copyGuide.callsToAction.slice(0, 3).map(c => `"${c.text}"`).join(', ')}\n\n`;
        }
...
- Professional signature`
    ...
    generateMultimodalPrompt(options = {}) {
        ...
        if (styleGuide) {
            ...
            if (styleGuide.colors?.accents?.length > 0) {
                textPrompt += `- **Accent Colors:** ${styleGuide.colors.accents.slice(0, 3).map(c => c.value).join(', ')}\n`;
            }
            if (styleGuide.buttons?.[0]) {
...
`
    ...
    generateImageGenPrompts(styleGuide, copyGuide, options = {}) {
        const {
            ...
            purpose = '',
            targetAudience = '',
            includeText = false,  // Whether to include text in the image
            variations = 3
        } = options;
    ...
    _generateMidjourneyStylePrompt(styleGuide, copyGuide, options = {}) {
        const { subject, mood, purpose } = options;
    ...
    _generateStableDiffusionPrompt(styleGuide, options = {}) {
        ...
        // Negative prompt
        const negative = 'blurry, low quality, distorted, ugly, bad anatomy, ' +
            'bad composition, watermark, signature, text, logo';
     ...
     */
    _generateDALLEVariations(basePrompt, styleGuide) {
        const variations = [];
    ...
    _formatStyleData(styleGuide) {
        ...
        if (styleGuide.colors) {
            ...
            if (styleGuide.colors.accents?.length > 0) {
                data += `- Accents: ${styleGuide.colors.accents.slice(0, 3).map(c => c.value).join(', ')}\n`;
            }
        }

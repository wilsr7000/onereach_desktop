...
- [ ] **Email Agent (IMAP)** - Real IMAP/SMTP connectivity, guided setup wizard for Gmail/Outlook/Yahoo/iCloud, threaded conversation tracking with AI-powered triage scoring (urgency, importance, conversation weighting)
- [x] **Agent Self-Learning System** - Fully automatic self-improvement for user-defined agents. Monitors interactions, detects frustration/gaps/opportunities, improves prompts/UI/routing/reliability silently, LLM-as-judge verification (never deploys if degraded), three-layer budget control, audit trail via Agent Product Manager space. Adapted from cranky-boyd deploy-test-loop pattern.

  ...
  try {
    ...
    app.exportStoryBeatsTranscript = function () {
      ...
      if (editor.dialogueBlocks?.length > 0) {
        ...
        editor.dialogueBlocks.forEach((block, _idx) => {
    ...
    app.applyStoryBeatsEdits = async function () {
      ...
      const message =
        `Apply ${summary.deletionCount + summary.gapCount + summary.replacementCount} edit(s)?\\n\\n` +
    ...
    app.setZoomToTime = function (seconds) {
      ...
      console.log(
        `[Zoom] Set to ${displayLabel} (${newZoom.toFixed(2)}x zoom for ${videoDuration.toFixed(0)}s video, showing ${seconds}s)`
      );
    };
    ...
    app.deserializeAudioTracks = function (savedTracks) {
      ...
      console.log(
        '[deserializeAudioTracks] Restored',
        app.audioTracks.length,
        'tracks:',
    ...
    app.restoreAudioTracks = function (savedTracks) {
      ...
      if (savedTracks && Array.isArray(savedTracks) && savedTracks.length > 0) {
        ...
        if (newTracks.length === 0 && app.audioTracks && app.audioTracks.length >= savedTracks.length) {
          ...
          console.log(
            '[RestoreAudioTracks] All tracks already loaded, skipping restore:',
            app.audioTracks.map((t) => t.id)
          );
        } else {
          ...
          console.log(
            '[RestoreAudioTracks] Restored audioTracks array:',
            app.audioTracks.map((t) => t.id),
            'nextTrackId:',
            app.nextTrackId
          );
      ...
      const tracksToRender = audioTracks.filter((track) => track.id !== 'A1' && !renderedTrackIds.has(track.id));
        ...
        tracksToRender.forEach((track) => {
          if (track.clips && track.clips.length > 0) {
            ...
            if (
              track.type === 'speaker' &&
              app.adrManager &&
              typeof app.adrManager._renderSpeakerClips === 'function'
            ) {
      ...
      app.loadProjectData = function (projectData) {
        ...
        setTimeout(() => {
          ...
          const savedTracks =
            ...
            projectData?.tracks ||
            app.currentProject?.audioTracks ||
            app.currentProject?.tracks;
    ...
    app.updateUndoRedoUI = function () {
      ...
      if (undoBtn) {
        ...
        undoBtn.title = status.canUndo ? `Undo: ${status.undoDescription || 'Last action'}` : 'Nothing to undo';
      }

      if (redoBtn) {
        redoBtn.disabled = !status.canRedo;
        redoBtn.style.opacity = status.canRedo ? '1' : '0.4';
        redoBtn.title = status.canRedo ? `Redo: ${status.redoDescription || 'Last undone action'}` : 'Nothing to redo';
      }
    };
    ...
    app.initDeploymentVersions = function () {
      ...
      app.deploymentVersionManager = new DeploymentVersionManager(app.videoPath, video.duration, {

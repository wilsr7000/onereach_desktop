...
class TestRunner {
    ...
    initializeTestCases() {
        ...
        this.testCases.set('clipboard-text', {
            ...
            description: 'Verifies that the application correctly monitors and captures text content copied to the system clipboard.',
            steps: [
        ...
        this.testCases.set('source-detection', {
            ...
            description: 'Tests the intelligent content classification system that automatically identifies URLs, code snippets, emails, and plain text.',
            steps: [
        ...
        this.testCases.set('export-pdf', {
            title: 'Export to PDF',
            description: 'Verifies that the app can export clipboard content and reports to PDF format for sharing and archiving.',
            steps: [
    ...
    showTestResultPopup(testResult) {
        ...
        logsContainer.innerHTML = testResult.logs.map(log => {
            const timestamp = new Date(log.timestamp).toLocaleTimeString();
            let className = '';
            switch (log.type) {
                case 'error': className = 'log-fail'; break;
                case 'warn': className = 'log-warn'; break;
                case 'success': className = 'log-pass'; break;
    ...
    initializeTests() {
        ...
        this.tests.set('source-detection', {
            ...
            category: 'core',
            useCase: 'Tests the intelligent content classification system that identifies URLs, code snippets, emails, and plain text when manually captured.',
            instructions: [
        ...
        this.tests.set('search-function', {
            ...
            category: 'core',
            useCase: 'Tests the search feature that allows users to quickly find items in their manually saved clipboard history.',
            instructions: [
        ...
        this.tests.set('edit-idw', {
            ...
            async run() {
                ...
                const updatedEnvironments = environments.map(env => 
                    env.id === idwToEdit.id ? editedIDW : env
                );
            ...
            async run() {
                ...
                for (const env of environments) {
                    for (const linkType of expectedLinkTypes) {
                        const link = gsxLinks.find(l => 
                            l.idwId === env.id && l.label === linkType
                        );
        ...
        this.tests.set('export-pdf', {
            name: 'Export to PDF',
            category: 'export',
            useCase: 'Verifies that the app can export clipboard content and reports to PDF format for sharing and archiving.',
            instructions: [
            ...
            async run() {
                ...
                try {
                    ...
                    const smartExportAvailable = await window.electron.ipcRenderer.invoke('check-smart-export');
    ...
    renderLogsInTestRunner() {
        ...
        container.innerHTML = this.logFilteredLogs.map(log => {
